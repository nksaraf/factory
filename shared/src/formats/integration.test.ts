...
import {
  DockerComposeFormatAdapter,
  resolveComposeEnvVar,
} from "./docker-compose.adapter"
...
describe("resolveComposeEnvVar", () => {
  ...
  it("resolves multiple vars in one string", () => {
    expect(
      resolveComposeEnvVar("postgres://${DB_HOST}:${PORT:-5432}/db", env)
    ).toBe("postgres://mydb:8080/db")
  })

  it("resolves port mapping string", () => {
    expect(resolveComposeEnvVar("${INFRA_POSTGRES_PORT:-5432}:5432", env)).toBe(
      "5432:5432"
    )
  })
...
describe("docker-compose integration: multi-service", () => {
  ...
  it("parses multiple ports with correct names", () => {
    ...
    expect(pgPorts[0]).toMatchObject({
      name: "postgres",
      port: 5432,
      protocol: "tcp",
    })
    ...
    expect(apiPorts[0]).toMatchObject({
      name: "http",
      port: 3000,
      protocol: "http",
    })
    expect(apiPorts[1]).toMatchObject({
      name: "metrics",
      port: 9090,
      protocol: "http",
    })
    ...
    expect(rmqPorts[0]).toMatchObject({
      name: "amqp",
      port: 5672,
      protocol: "tcp",
    })
    expect(rmqPorts[1]).toMatchObject({
      name: "management-ui",
      port: 15672,
      protocol: "http",
    })
  ...
  it("preserves healthcheck on resources", () => {
    const { system } = adapter.parse(rootDir)
    expect(system.resources.postgres.spec.healthcheck).toBe("pg_isready -U app")
  })
  ...
  it("infers database connections from DATABASE_URL env", () => {
    const { system } = adapter.parse(rootDir)
    const dbConns = system.connections.filter(
      (c) => c.envVar === "DATABASE_URL"
    )
  ...
  it("infers redis connections from REDIS_URL env", () => {
    const { system } = adapter.parse(rootDir)
    const redisConns = system.connections.filter(
      (c) => c.envVar === "REDIS_URL"
    )
  ...
  it("infers rabbitmq connections from RABBITMQ_URL env", () => {
    const { system } = adapter.parse(rootDir)
    const rmqConns = system.connections.filter(
      (c) => c.envVar === "RABBITMQ_URL"
    )
...
describe("docker-compose integration: repo root", () => {
  ...
  it("resolves env var interpolation in environment values", () => {
    ...
    const auth =
      system.components["infra-auth"] ?? system.resources["infra-auth"]
...
const authServiceDir = join(
  import.meta.dirname,
  "../../../.dx/pkg-repos/auth-service"
)
describe.skipIf(!existsSync(authServiceDir))(
  "docker-compose integration: auth-service",
  () => {
    const rootDir = authServiceDir
    const adapter = new DockerComposeFormatAdapter()
