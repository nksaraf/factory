...
const hasCatalogInfo = existsSync(join(repoRoot, "catalog-info.yaml"))
...
describe("factory docker-compose.yaml (original)", () => {
  ...
  it("infers connections from env var URLs", () => {
    ...
    const dbConns = system.connections.filter(
      (c) => c.targetComponent === "infra-postgres"
    )
...
describe.skipIf(!hasCatalogInfo)("factory catalog-info.yaml", () => {
  ...
  it("dependency references use entity ref format", () => {
    ...
    const docs = entities.find(
      (e: any) => e.metadata.name === "factory-api-docs"
    )
  ...
  it("factory-api has image, build, ports, env, volumes, healthchecks", () => {
    ...
    expect(api.spec.volumes[0].containerPath).toBe(
  ...
  it("factory-postgres has image, port, env, volumes, healthcheck", () => {
    ...
    expect(pg.spec.ports[0]).toMatchObject({
      name: "postgres",
      port: 5432,
      protocol: "tcp",
    })
  ...
  it("factory-auth has image, port mapping, env with connection strings, healthcheck", () => {
    ...
    expect(auth.spec.environment.AUTH_DATABASE_URL).toMatch(
      /factory-postgres:5432/
    )
    expect(auth.spec.volumes).toHaveLength(1)
    expect(auth.spec.volumes[0].containerPath).toBe(
      "/app/config/application.yml"
    )
  ...
  it("factory-reverse-proxy has image, port mapping, volumes, command", () => {
    const entities = loadEntities()
    const rp = entities.find(
      (e: any) => e.metadata.name === "factory-reverse-proxy"
    )
  ...
  it("factory-api-docs has image, port, volume", () => {
    ...
    expect(docs.spec.volumes[0].containerPath).toBe(
