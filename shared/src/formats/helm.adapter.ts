...
export class HelmFormatAdapter implements CatalogFormatAdapter {
  ...
  parse(rootDir: string): CatalogParseResult {
    ...
    const compute =
      valLimits || valRequests
        ? {
            min: valRequests
              ? { cpu: valRequests.cpu, memory: valRequests.memory }
              : undefined,
            max: valLimits
              ? { cpu: valLimits.cpu, memory: valLimits.memory }
              : undefined,
          }
        : undefined

    const componentType = chart.type === "library" ? "library" : "service"
  ...
  generate(
    ...
    const appVersion = (helmExt.appVersion as string) ?? "1.0.0"
    const chartVersion = (helmExt.chartVersion as string) ?? "0.1.0"
    ...
    const chartYaml: Record<string, unknown> = {
      ...
      description:
        ...
        `A Helm chart for ${system.metadata.name}`,
      type: "application",
    ...
    if (componentEntries.length > 0) {
      ...
      if (comp.spec.compute) {
        ...
        if (comp.spec.compute.max) {
          ...
          if (comp.spec.compute.max.memory)
            limits.memory = comp.spec.compute.max.memory
          if (Object.keys(limits).length) res.limits = limits
        }
        if (comp.spec.compute.min) {
          const requests: Record<string, string> = {}
          if (comp.spec.compute.min.cpu)
            requests.cpu = comp.spec.compute.min.cpu
          if (comp.spec.compute.min.memory)
            requests.memory = comp.spec.compute.min.memory
