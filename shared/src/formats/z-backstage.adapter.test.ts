...
const { BackstageFormatAdapter } = await import("./backstage.adapter.js")
...
`
    ...
    it("correctly parses entity references", () => {
      ...
      expect(result.system.resources.postgres.spec.dependencyOf).toEqual([
        "api",
      ])
  ...
  describe("generate", () => {
    ...
    it("does not warn about connections when there are none", () => {
      ...
      expect(result.warnings.some((w) => w.includes("connections"))).toBe(false)
    })
  })
  ...
  describe("round-trip", () => {
    it("generate -> parse preserves key data", () => {
      ...
      expect(parsed.system.components.web.spec.providesApis).toEqual([
        "web-api",
      ])
