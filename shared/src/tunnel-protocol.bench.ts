...
function benchResponseAssembly() {
  ...
  for (const { label, totalSize, numChunks } of chunkSizes) {
    ...
    console.log(
      `    streaming: ${fmt(iterations, msStreaming)} (${speedup}% faster)`
    )
  }
}
...
function benchMemory() {
  ...
  console.log(
    `  Streaming approach peak overhead: ~${fmtMem(chunkSize)} (one chunk in flight)`
  )
  console.log(
    `  Memory reduction: ~${(bufferedRss / chunkSize).toFixed(0)}x for 10MB response`
  )
}
...
function benchE2EThroughput() {
  ...
  const reqPayload = {
    method: "GET",
    url: "/api/v1/data",
    headers: { host: "test.tunnel.dx.dev", accept: "application/json" },
  }
  const resPayload = {
    status: 200,
  ...
  console.log(
    `  Full round-trip (encode+decode req+res+4KB body): ${fmt(iterations, ms)}`
  )
}
