...
function buildBranchRegex(config: ConventionsConfig): RegExp | null {
  ...
  const ticketGroup = require_ticket ? "([A-Z][A-Z0-9]*-\\d+)" : "([^/]+)"
  ...
  if (config.commits.require_scope) {
    ...
    if (!scopeRequired.test(m)) {
      return {
        valid: false,
        violations: [
          "Conventions require a scope in parentheses, e.g. feat(api): ...",
        ],
...
export function checkConnectionPolicy(
  ...
  if (rules.length === 0) {
    const isProduction = targetKind === "production" || targetKind === "prod"
    return {
      allowed: true,
      forceReadonly: false,
  ...
  if (forceReadonly && !readonly) {
    violations.push(`Connections to "${targetKind}" require --readonly flag.`)
  }

  const isProduction = kind === "production" || kind === "prod"
  const requireReason = isProduction && connections.production_require_reason
