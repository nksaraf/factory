...
import { preview, systemDeployment, workspace, site } from "../db/schema/ops";
...
import type { PreviewSpec, SystemDeploymentSpec, SiteSpec } from "@smp/factory-shared/schemas/ops";
...
export class PreviewReconciler {
  ...
  private async handleBuilding(prev: PreviewRow): Promise<void> {
    ...
    // Workspace has a direct runtimeId FK
    if (!wks.runtimeId) return;

    const rt = await this.loadRuntime(wks.runtimeId);
    if (!rt) return;
    ...
    try {
      ...
      logger.info({ previewId: prev.id, imageRef: fullImageRef }, "Preview build complete, deploying");
    } catch (err) {
  ...
  private async handleDeploying(prev: PreviewRow): Promise<void> {
    ...
    // In v2, preview can reference a runtime directly via spec.runtimeId
    // or via spec.systemDeploymentId → systemDeployment.runtimeId
    const runtimeId = spec.runtimeId ?? await this.resolveRuntimeId(spec.systemDeploymentId);
    if (!runtimeId) {
      await this.failPreview(prev, "No runtime for deployment");
      return;
    }

    const rt = await this.loadRuntime(runtimeId);
    if (!rt) {
      await this.failPreview(prev, "Runtime not found");
      return;
    }
    const rtSpec = (rt.spec ?? {}) as Record<string, any>;
    if (!rtSpec.kubeconfigRef) {
      await this.failPreview(prev, "No kubeconfig for runtime");
      return;
    }
    ...
    if (prev.siteId) {
      const [siteRow] = await this.db.select({ spec: site.spec })
        .from(site).where(eq(site.id, prev.siteId)).limit(1);
    ...
    await this.updateDeploymentStatus(prev, "in_progress", `Deploying ${spec.imageRef}`);
    ...
    try {
      ...
      if (spec.systemDeploymentId) {
        const sdExisting = await this.db
          ...
          .limit(1)
          .then((r) => r[0]);
        const sdSpec: SystemDeploymentSpec = { ...(sdExisting?.spec ?? {} as SystemDeploymentSpec), status: "active" };
        await this.db
      ...
      logger.info({ previewId: prev.id, domain: previewDomain }, "Preview deployed and active");
  ...
  private async upsertPRComment(prev: PreviewRow, body: string): Promise<void> {
    ...
    try {
      if (spec.githubCommentId) {
        await this.gitHost.updatePRComment(spec.repo, spec.githubCommentId, fullBody);
      } else {
        const comments = await this.gitHost.listPRComments(spec.repo, prev.prNumber);
        const existing = comments.find((c) => c.body.includes(marker));

        if (existing) {
          await this.gitHost.updatePRComment(spec.repo, existing.commentId, fullBody);
          await this.db
            .update(preview)
            .set({ spec: { ...spec, githubCommentId: existing.commentId } })
            .where(eq(preview.id, prev.id));
        } else {
          const { commentId } = await this.gitHost.postPRComment(spec.repo, prev.prNumber, fullBody);
          await this.db
            .update(preview)
      ...
      logger.warn({ previewId: prev.id, error: err }, "Failed to upsert PR comment");
    }
  }
  ...
  private async ensureGitHubDeployment(prev: PreviewRow): Promise<void> {
    ...
    try {
      ...
      logger.warn({ previewId: prev.id, error: err }, "Failed to create GitHub deployment");
    }
  }
  ...
  private async updateDeploymentStatus(
    ...
    try {
      await this.gitHost.createDeploymentStatus(spec.repo, spec.githubDeploymentId, {
        state,
        description,
        environmentUrl,
      });
    } catch (err) {
      logger.warn({ previewId: prev.id, error: err }, "Failed to update deployment status");
    }
  }
  ...
  private async createOrUpdateCheck(
    ...
    try {
      ...
      logger.warn({ previewId: prev.id, error: err }, "Failed to create check run");
      return null;
    }
  }
  ...
  private async completeCheck(
    ...
    try {
      ...
      logger.warn({ previewId: prev.id, error: err }, "Failed to complete check run");
    }
  }
  ...
  private async onPreviewActive(prev: PreviewRow, previewUrl: string): Promise<void> {
    ...
    await emitEvent(this.db, "preview.ready", {
      ...
      logger.warn({ previewId: prev.id, error: err }, "Failed to emit preview.ready event");
    });
  }
  ...
  private async updateStatus(previewId: string, phase: string, statusMessage?: string): Promise<void> {
    const existing = await this.loadPreview(previewId);
    const existingSpec: PreviewSpecStored = existing?.spec as PreviewSpecStored ?? {} as PreviewSpecStored;
    await this.db
      .update(preview)
      .set({
        phase,
        spec: { ...existingSpec, ...(statusMessage !== undefined ? { statusMessage } : {}) },
        updatedAt: new Date(),
      })
  ...
  private async loadRuntime(runtimeId: string) {
    const [row] = await this.db
      .select()
      .from(runtime)
  ...
  /** Resolve runtimeId from a systemDeployment's runtimeId FK. */
  private async resolveRuntimeId(systemDeploymentId?: string): Promise<string | null> {
