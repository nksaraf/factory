...
export class KubernetesStrategy implements RuntimeStrategy {
  ...
  async reconcile(ctx: ReconcileContext, db: Database): Promise<ReconcileResult> {
    const runtimeId = ctx.target.clusterId; // v2: clusterId maps to runtimeId
    if (!runtimeId) throw new Error(`Kubernetes target ${ctx.target.deploymentTargetId} has no runtime`);
    ...
    if (!kubeconfigRef) throw new Error(`Runtime ${runtimeId} has no kubeconfig`);
    ...
    if (ctx.component.kind === "service" || ctx.component.kind === "server" || ctx.component.kind === "worker" || ctx.component.kind === "database" || ctx.component.kind === "gateway" || ctx.component.kind === "agent" || ctx.component.kind === "cache" || ctx.component.kind === "queue" || ctx.component.kind === "storage" || ctx.component.kind === "search") {
