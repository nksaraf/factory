...
import type { WorkspaceSpec, WorkspaceSnapshotSpec, ComponentDeploymentSpec, SystemDeploymentSpec } from "@smp/factory-shared/schemas/ops";
import type { RuntimeSpec } from "@smp/factory-shared/schemas/infra";
import { route as v2Route } from "../db/schema/infra-v2";
import { resolveRouteTargets, drizzleDbReader } from "../modules/infra/route-resolver";
...
import { componentDeployment, workspace, systemDeployment, preview, workspaceSnapshot } from "../db/schema/ops";
...
import { expireStale, updateWorkspaceHealth } from "../modules/fleet/workspace.service";
import { getSnapshot, updateSnapshotStatus } from "../modules/fleet/snapshot.service";
...
export class Reconciler {
  ...
  async reconcileAll(): Promise<{ reconciled: number; errors: number }> {
    ...
    const allWorkspaces = await this.db.select().from(workspace).where(
      and(isNull(workspace.systemTo), isNull(workspace.validTo))
    );
    const activeWorkspaces = allWorkspaces.filter((w) => {
      return w.spec?.runtimeType === "container" && w.spec?.lifecycle !== "destroyed";
    });
    logger.debug({ total: allWorkspaces.length, active: activeWorkspaces.length }, "Workspace reconciliation candidates");
    ...
    for (const ep of expiredPreviews) {
      try {
        ...
        const [rt] = await this.db
          .select()
          .from(runtime)
          .where(eq(runtime.id, sd.runtimeId))
          .limit(1);
        ...
        await this.db
          ...
          .set({ spec: { ...sd.spec, status: "destroyed" as const }, updatedAt: new Date() })
  ...
  async reconcileWorkspace(workspaceId: string): Promise<void> {
    ...
    // 2. Skip if runtimeType !== 'container' (VM workspaces managed directly)
    if (wksSpec.runtimeType !== "container") return;

    // 3. Load runtime directly from workspace.runtimeId
    const runtimeId = wks.runtimeId;
    if (!runtimeId) throw new Error(`Workspace ${workspaceId} has no runtime`);

    const [rt] = await this.db
      .select()
      .from(runtime)
      .where(eq(runtime.id, runtimeId));
    if (!rt) throw new Error(`Runtime not found: ${runtimeId}`);
    const rtSpec: RuntimeSpec = rt.spec ?? {} as RuntimeSpec;
    if (!rtSpec.kubeconfigRef) throw new Error(`Runtime ${runtimeId} has no kubeconfig`);
    ...
    if (wksSpec.lifecycle === "destroying") {
      ...
      await this.kube.remove(kubeconfig, "PersistentVolumeClaim", ns, `${podName}-data`).catch(() => {});
      await this.kube.remove(kubeconfig, "PersistentVolumeClaim", ns, `${podName}-docker`).catch(() => {});
      await this.kube.remove(kubeconfig, "Namespace", "", ns);
      await this.db
        .update(workspace)
        .set({ spec: { ...wksSpec, lifecycle: "destroyed" }, updatedAt: new Date() })
        .where(eq(workspace.id, workspaceId));
      return;
    }
    ...
    try {
      ...
      if (existingPod) {
        ...
        const podEnv = ((existingPod as any).spec?.containers?.[0]?.env ?? []) as Array<{ name: string; value: string }>;
        const podWorkspaceId = podLabels["dx.dev/workspace"] ?? podLabels["dx.dev/workspace-id"] ?? podEnv.find((e) => e.name === "DX_WORKSPACE_ID")?.value ?? podEnv.find((e) => e.name === "WORKSPACE_ID")?.value;
        if (podWorkspaceId && podWorkspaceId !== wks.id) {
          logger.info(
            { ns, staleWorkspaceId: podWorkspaceId, currentWorkspaceId: wks.id },
          ...
          logger.info({ ns }, "Stale namespace cleaned up. Will re-provision on next reconcile cycle.");
          return;
        }
      }
    } catch (err) {
      // Non-fatal — if we can't check, just proceed with apply (it may succeed)
      logger.debug({ ns, err }, "Could not check for stale resources, proceeding");
    }
    ...
    try {
      ...
      if (nsResource) {
        ...
        if (nsPhase === "Terminating") {
          logger.debug({ ns }, "Namespace is Terminating — skipping reconcile cycle");
          return;
        }
      }
    } catch {
    ...
    const wksSpecExtra = wksSpec as WorkspaceSpec & { devcontainerImage?: string };
    const resources = generateSandboxResources({
      ...
      repos: (wksSpec.repos ?? []).map((r) => ({ ...r, branch: r.branch ?? "main" })),
    ...
    const primaryPort = WORKSPACE_PRIMARY_ENDPOINT === "terminal"
      ? (webPort ?? 8080)
      : (webIdePort ?? 8081);
    ...
    if (!existingRoute) {
      ...
      logger.info({ domain: workspaceDomain }, "Created bare domain route for workspace");
    } else if (existingRoute.targetPort !== primaryPort || existingRoute.targetService !== runtimeEndpoint) {
      await updateRoute(this.db, existingRoute.routeId, { targetPort: primaryPort, targetService: runtimeEndpoint });
      logger.info({ domain: workspaceDomain, targetPort: primaryPort }, "Updated bare domain route for workspace");
    }

    // 11. Create named endpoint routes from devcontainer config
    const dxConfig = (wksSpec.devcontainerConfig as Record<string, unknown>)?.customizations as Record<string, unknown> | undefined;
    ...
    for (const [name, config] of Object.entries(endpoints)) {
      ...
      const targetPort = name === "terminal" ? (webPort ?? 8080) : name === "ide" ? (webIdePort ?? 8081) : config.port;
      ...
      if (!existingEndpointRoute) {
        ...
        logger.info({ domain: endpointDomain, port: targetPort }, "Created named endpoint route");
      } else if (existingEndpointRoute.targetPort !== targetPort || existingEndpointRoute.targetService !== runtimeEndpoint) {
        await updateRoute(this.db, existingEndpointRoute.routeId, { targetPort, targetService: runtimeEndpoint });
        logger.info({ domain: endpointDomain, port: targetPort }, "Updated named endpoint route");
      }
    }
    ...
    if (wksSpec.lifecycle === "provisioning") {
      ...
      if (podCheck) {
        const podStatus = (podCheck as KubeResource & { status?: Record<string, unknown> }).status;
        const phase = podStatus?.phase as string | undefined;
        const containerStatuses = (podStatus?.containerStatuses ?? []) as Array<{ name: string; ready: boolean; state?: Record<string, unknown> }>;
        ...
        if (phase === "Running" && containerStatuses.length > 0 && containerStatuses.every((c) => c.ready)) {
          ...
          logger.warn({ workspaceId, phase }, "Pod entered terminal state during provisioning");
        } else {
          // Detect stuck container states that won't self-resolve
          const terminalReasons = new Set(["ImagePullBackOff", "ErrImagePull", "InvalidImageName", "CrashLoopBackOff"]);
          const stuckReason = containerStatuses
            .map((c) => (c.state?.waiting as Record<string, unknown> | undefined)?.reason as string | undefined)
            .find((r) => r && terminalReasons.has(r));
          if (stuckReason) {
            logger.warn({ workspaceId, podName, reason: stuckReason }, "Pod has container in terminal waiting state");
          }
        }
      }
    }
    ...
    if (newLifecycle === "active" && wksSpec.lifecycle !== "active") {
      await emitEvent(this.db, "workspace.ready", { workspaceId, status: "active" }).catch((err) => {
        logger.warn({ workspaceId, err }, "Failed to emit workspace.ready event");
      });
    }
  }
  ...
  async reconcileWorkspaceHealth(workspaceId: string): Promise<{
    ...
    if (!pod) {
      ...
      const podWithStatus = pod as typeof pod & { status?: Record<string, unknown> };
    ...
    await updateWorkspaceHealth(this.db, workspaceId, healthStatus, statusMessage);
  ...
  async reconcileSnapshotCreate(snapshotId: string): Promise<void> {
    ...
    const { wks, kubeconfig, ns } = await this.loadWorkspaceContext(snap.workspaceId);
    ...
    try {
      ...
      if (ready) {
        ...
        logger.info({ snapshotId, sandboxId: snap.workspaceId }, "Snapshot created successfully");
      } else {
  ...
  async reconcileSnapshotRestore(sandboxId: string, snapshotId: string): Promise<void> {
    const snap = await getSnapshot(this.db, snapshotId);
    if (!snap) throw new Error(`Snapshot not found: ${snapshotId}`);
    const snapSpec: WorkspaceSnapshotSpec = snap.spec ?? {} as WorkspaceSnapshotSpec;
    if (snapSpec.status !== "ready") throw new Error(`Snapshot ${snapshotId} is not ready (status: ${snapSpec.status})`);
  ...
  async reconcileWorkload(componentDeploymentId: string): Promise<void> {
    ...
    if (!cd) throw new Error(`Component deployment not found: ${componentDeploymentId}`);
    const cdSpec: ComponentDeploymentSpec = cd.spec ?? {} as ComponentDeploymentSpec;
    ...
    if (!sd) throw new Error(`System deployment not found: ${cd.systemDeploymentId}`);
    ...
    const cdSpecExtra = cdSpec as ComponentDeploymentSpec & { desiredArtifactUri?: string | null };
    const ctx: ReconcileContext = {
      ...
      component: {
        name: comp.name,
        kind: comp.type,
        ports: (compSpec.ports ?? []) as Array<{ name: string; port: number; protocol: string }>,
        healthcheck: (compSpec.healthcheck as { path: string; portName: string; protocol: string } | null) ?? null,
        // TODO: fix type — isPublic is not in ComponentSpec union; add when schema is updated
        isPublic: (compSpec as Record<string, unknown> & { isPublic?: boolean }).isPublic ?? false,
        stateful: (compSpec as Record<string, unknown> & { stateful?: boolean }).stateful ?? false,
        defaultCpu: (compSpec as Record<string, unknown> & { defaultCpu?: string }).defaultCpu ?? "100m",
        defaultMemory: (compSpec as Record<string, unknown> & { defaultMemory?: string }).defaultMemory ?? "128Mi",
        defaultReplicas: (compSpec as Record<string, unknown> & { defaultReplicas?: number }).defaultReplicas ?? 1,
      },
      target: {
    ...
    await this.db
      ...
      .set({
        spec: {
          ...
          ...(result.actualImage != null ? { actualImage: result.actualImage } : {}),
  ...
  > {
    ...
    return drifted.map((cd) => {
      const spec: ComponentDeploymentSpec = cd.spec ?? {} as ComponentDeploymentSpec;
      return {
        workloadId: cd.id,
  ...
  async reconcileRoutes(): Promise<{ resolved: number; errors: number }> {
    ...
    const staleRoutes = await this.db.select().from(v2Route).where(
      or(
    ...
    for (const r of staleRoutes) {
      try {
        const newStatus = await resolveRouteTargets(r.spec?.targets ?? [], reader);

        await this.db.update(v2Route)
          .set({
  ...
  startOperationRunner(
    ...
    const { createOperationRunner } = require("../lib/operations") as typeof import("../lib/operations");
