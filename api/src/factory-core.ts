...
import { gitHostProvider, pipelineRun, repo } from "./db/schema/build"
...
import { domain, route as gwRoute } from "./db/schema/gateway"
import { cluster, provider } from "./db/schema/infra"
import { runtime, substrate } from "./db/schema/infra-v2"
import { dnsDomain, route as infraRoute } from "./db/schema/infra-v2"
import {
  componentDeployment,
  preview as opsPreview,
  site,
  systemDeployment,
  workspace,
} from "./db/schema/ops"
...
export async function seedDemoData(db: Database) {
  ...
  await db.insert(orgTeam).values([
    {
      ...
      slug: "lepton",
      type: "business-unit",
      description: "Lepton Technologies",
    },
    {
      ...
      slug: "engineering",
      type: "product-area",
      parentTeamId: teamIds.root,
      description: "All engineering",
    },
    {
      teamId: teamIds.platform,
      name: "Platform",
      slug: "platform",
      type: "product-area",
      parentTeamId: teamIds.root,
      description: "Infrastructure & DevEx",
    },
    {
      teamId: teamIds.networkAccess,
      name: "Network Access",
      slug: "team-network-access",
      type: "team",
      parentTeamId: teamIds.engineering,
    },
    {
      teamId: teamIds.smartInventory,
      name: "Smart Inventory",
      slug: "team-smart-inventory",
      type: "team",
      parentTeamId: teamIds.engineering,
    },
    {
      teamId: teamIds.smartOps,
      name: "SmartOps",
      slug: "team-smartops",
      type: "team",
      parentTeamId: teamIds.engineering,
    ...
    {
      teamId: teamIds.smartSignal,
      name: "Smart Signal",
      slug: "team-smart-signal",
      type: "team",
      parentTeamId: teamIds.engineering,
    },
    {
      teamId: teamIds.smartMarket,
      name: "SmartMarket",
      slug: "team-smartmarket",
      type: "team",
      parentTeamId: teamIds.engineering,
    },
    {
      teamId: teamIds.trafficure,
      name: "Trafficure",
      slug: "team-trafficure",
      type: "team",
      parentTeamId: teamIds.engineering,
    },
    {
      teamId: teamIds.neo360,
      name: "Neo360",
      slug: "team-neo360",
      type: "team",
      parentTeamId: teamIds.engineering,
  ...
  const insertedSubstrates = await db
    .insert(substrate)
    .values(substrateRows)
    .returning({ id: substrate.id, slug: substrate.slug })
  ...
  const runtimeRows: Array<{
  ...
  }> = [
    {
      ...
      slug: "prod-us-east",
      type: "k8s-cluster",
      spec: { status: "ready", endpoint: "10.0.1.10:6443" },
    },
    {
      name: "prod-eu-west",
      slug: "prod-eu-west",
      type: "k8s-cluster",
      spec: { status: "ready", endpoint: "10.0.2.10:6443" },
    },
    {
      name: "staging-us",
      slug: "staging-us",
      type: "k8s-cluster",
      spec: { status: "ready", endpoint: "10.0.1.20:6443" },
    },
    {
      name: "dev-sandbox",
      slug: "dev-sandbox",
      type: "k8s-cluster",
      spec: { status: "ready", endpoint: "10.0.1.30:6443" },
    },
    {
      name: "preview-us-east",
      slug: "preview-us-east",
      type: "k8s-cluster",
      spec: {
        status: "ready",
        endpoint: "eks-preview.us-east-1.amazonaws.com",
      },
    },
    {
      name: "edge-latam",
  ...
  const insertedRuntimes = await db
    .insert(runtime)
    .values(runtimeRows)
    .returning({ id: runtime.id, slug: runtime.slug })
  ...
  const insertedProviders = await db
    .insert(provider)
    .values(providerRows)
    .returning({ providerId: provider.providerId, slug: provider.slug })
  ...
  const clusterRows = [
    {
      name: "prod-us-east",
      slug: "prod-us-east",
      providerId: providerMap["proxmox-dc1"],
    ...
    },
    {
      name: "prod-eu-west",
      slug: "prod-eu-west",
      providerId: providerMap["proxmox-dc2"],
      status: "ready" as const,
      endpoint: "10.0.2.10:6443",
    },
    {
      name: "staging-us",
      slug: "staging-us",
    ...
    },
    {
      ...
      providerId: providerMap["proxmox-dc1"],
      status: "ready" as const,
      endpoint: "10.0.1.30:6443",
    },
    {
      name: "preview-us-east",
      slug: "preview-us-east",
      providerId: providerMap["aws-us-east"],
      status: "ready" as const,
      endpoint: "eks-preview.us-east-1.amazonaws.com",
    },
    {
  ...
  const insertedClusters = await db
    .insert(cluster)
    .values(clusterRows)
    .returning({ clusterId: cluster.clusterId, slug: cluster.slug })
  ...
  const moduleRows = [
    {
      name: "Network Access API",
      slug: "network-access-api",
      teamId: teamIds.networkAccess,
      product: "network-access",
      description: "Core auth & session management",
    },
    {
      name: "Network Access Portal",
      slug: "network-access-portal",
      teamId: teamIds.networkAccess,
      product: "network-access",
      description: "Captive portal frontend",
    },
    {
      name: "Smart Inventory Core",
      slug: "smart-inventory-core",
      teamId: teamIds.smartInventory,
      product: "smart-inventory",
      description: "Inventory tracking engine",
    },
    {
      ...
      teamId: teamIds.smartInventory,
      product: "smart-inventory",
      description: "Async inventory sync jobs",
    },
    {
      name: "SmartOps Collector",
      slug: "smartops-collector",
      teamId: teamIds.smartOps,
      product: "smartops",
    ...
    },
    {
      ...
      slug: "smartops-dashboard",
      teamId: teamIds.smartOps,
      product: "smartops",
      description: "Operations dashboard UI",
    },
    {
      name: "Smart Signal Processor",
      slug: "smart-signal-processor",
      teamId: teamIds.smartSignal,
      product: "smart-signal",
      description: "Signal analysis pipeline",
    },
    {
      name: "SmartMarket Engine",
      slug: "smartmarket-engine",
      teamId: teamIds.smartMarket,
      product: "smartmarket",
      description: "Marketplace pricing engine",
    },
    {
      name: "SmartMarket Storefront",
      slug: "smartmarket-storefront",
      teamId: teamIds.smartMarket,
      product: "smartmarket",
      description: "Storefront web app",
    },
    {
      name: "Trafficure Gateway",
      slug: "trafficure-gateway",
      teamId: teamIds.trafficure,
      product: "trafficure",
      description: "Traffic management gateway",
    },
    {
      name: "Trafficure Analytics",
      slug: "trafficure-analytics",
      teamId: teamIds.trafficure,
      product: "trafficure",
      description: "Traffic analytics service",
    },
    {
      name: "Neo360 Platform",
      slug: "neo360-platform",
      teamId: teamIds.neo360,
      product: "neo360",
      description: "Customer 360 platform",
    },
    {
      name: "Factory CLI",
      slug: "factory-cli",
  ...
  const insertedModules = await db
    ...
    .values(moduleRows)
    .returning({ moduleId: productModule.moduleId, slug: productModule.slug })
  ...
  const repoRows = [
    {
      name: "lepton/network-access",
      slug: "network-access",
      kind: "product-module" as const,
      moduleId: moduleMap["network-access-api"],
      teamId: teamIds.networkAccess,
      gitUrl: "git@github.com:lepton/network-access.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      name: "lepton/smart-inventory",
      slug: "smart-inventory",
      kind: "product-module" as const,
      moduleId: moduleMap["smart-inventory-core"],
      teamId: teamIds.smartInventory,
      gitUrl: "git@github.com:lepton/smart-inventory.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      name: "lepton/smartops",
      slug: "smartops",
      kind: "product-module" as const,
      moduleId: moduleMap["smartops-collector"],
      teamId: teamIds.smartOps,
      gitUrl: "git@github.com:lepton/smartops.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      name: "lepton/smartmarket",
      slug: "smartmarket",
      kind: "product-module" as const,
      moduleId: moduleMap["smartmarket-engine"],
      teamId: teamIds.smartMarket,
      gitUrl: "git@github.com:lepton/smartmarket.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      name: "lepton/trafficure",
      slug: "trafficure",
      kind: "product-module" as const,
      moduleId: moduleMap["trafficure-gateway"],
      teamId: teamIds.trafficure,
      gitUrl: "git@github.com:lepton/trafficure.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      ...
      slug: "neo360",
      kind: "product-module" as const,
      moduleId: moduleMap["neo360-platform"],
      teamId: teamIds.neo360,
      gitUrl: "git@github.com:lepton/neo360.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      name: "lepton/factory",
      slug: "factory",
      kind: "platform-module" as const,
      moduleId: moduleMap["factory-api"],
      teamId: teamIds.platform,
      gitUrl: "git@github.com:lepton/factory.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
    {
      ...
      kind: "infra" as const,
      teamId: teamIds.platform,
      gitUrl: "git@github.com:lepton/infra.git",
      defaultBranch: "main",
      gitHostProviderId: ghp.gitHostProviderId,
    },
  ]
  const insertedRepos = await db
    .insert(repo)
    .values(repoRows)
    .returning({ repoId: repo.repoId, slug: repo.slug })
  ...
  await db.insert(pipelineRun).values([
    {
      ...
      completedAt: new Date(now - 3450_000),
    },
    {
      repoId: repoMap["smart-inventory"],
      triggerEvent: "pull_request",
      triggerRef: "refs/heads/feat/barcode-scan",
      commitSha: "e4f5a6b",
      status: "running",
      triggerActor: "bob",
    ...
    {
      repoId: repoMap["smartops"],
      triggerEvent: "push",
      triggerRef: "refs/heads/main",
      commitSha: "c7d8e9f",
      status: "failure",
      triggerActor: "charlie",
      startedAt: new Date(now - 7200_000),
      completedAt: new Date(now - 7080_000),
      errorMessage: "Test suite failed: 3 assertions",
    },
    {
      repoId: repoMap["trafficure"],
      triggerEvent: "push",
      triggerRef: "refs/heads/release/2.14",
      commitSha: "1a2b3c4",
      status: "success",
      triggerActor: "diana",
      startedAt: new Date(now - 1800_000),
      completedAt: new Date(now - 1650_000),
    },
    {
      repoId: repoMap["factory"],
      triggerEvent: "push",
      triggerRef: "refs/heads/main",
      commitSha: "5d6e7f8",
      status: "success",
      triggerActor: "nikhil",
      startedAt: new Date(now - 900_000),
      completedAt: new Date(now - 780_000),
    },
    {
      repoId: repoMap["smartmarket"],
      triggerEvent: "manual",
      triggerRef: "refs/heads/main",
      commitSha: "9a0b1c2",
      status: "pending",
    ...
    {
      repoId: repoMap["neo360"],
      triggerEvent: "push",
      triggerRef: "refs/heads/feat/customer-360",
      commitSha: "3d4e5f6",
      status: "running",
      triggerActor: "frank",
    ...
    },
    {
      repoId: repoMap["network-access"],
      triggerEvent: "pull_request",
      triggerRef: "refs/heads/fix/session-timeout",
      commitSha: "7a8b9c0",
      status: "success",
      triggerActor: "grace",
      startedAt: new Date(now - 5400_000),
      completedAt: new Date(now - 5280_000),
    },
  ])
  ...
  const siteRows = [
    {
      name: "Verizon Network Access (US)",
      slug: "verizon-network-access",
      product: "network-access",
      clusterId: clusterMap["prod-us-east"],
      status: "active" as const,
      currentManifestVersion: 42,
      lastCheckinAt: new Date(now - 30_000),
    },
    {
      name: "Verizon Network Access (EU)",
      slug: "verizon-network-access-eu",
      product: "network-access",
      clusterId: clusterMap["prod-eu-west"],
      status: "active" as const,
      currentManifestVersion: 41,
      lastCheckinAt: new Date(now - 45_000),
    },
    {
      name: "Walmart Smart Inventory",
      slug: "walmart-smart-inventory",
      product: "smart-inventory",
      clusterId: clusterMap["prod-us-east"],
      status: "active" as const,
      currentManifestVersion: 38,
      lastCheckinAt: new Date(now - 60_000),
    },
    {
      name: "BMW Trafficure",
      slug: "bmw-trafficure",
      ...
      clusterId: clusterMap["prod-eu-west"],
      status: "active" as const,
      currentManifestVersion: 27,
      lastCheckinAt: new Date(now - 120_000),
    },
    {
      name: "Target SmartMarket",
      slug: "target-smartmarket",
      product: "smartmarket",
      clusterId: clusterMap["prod-us-east"],
      status: "active" as const,
      currentManifestVersion: 15,
      lastCheckinAt: new Date(now - 90_000),
    },
    {
      name: "Lepton Staging",
      slug: "lepton-staging",
      product: "platform",
      clusterId: clusterMap["staging-us"],
      status: "active" as const,
      currentManifestVersion: 156,
      lastCheckinAt: new Date(now - 15_000),
    },
    {
      name: "Lepton Prod (US)",
      slug: "lepton-prod-us",
      ...
      clusterId: clusterMap["prod-us-east"],
      status: "active" as const,
      currentManifestVersion: 144,
      lastCheckinAt: new Date(now - 20_000),
    },
    {
      name: "Deutsche Telekom SmartOps",
      slug: "dt-smartops",
      product: "smartops",
      clusterId: clusterMap["prod-eu-west"],
      status: "active" as const,
      currentManifestVersion: 33,
      lastCheckinAt: new Date(now - 180_000),
    },
    {
      name: "Acme Neo360",
      slug: "acme-neo360",
    ...
    {
      name: "Sprint Smart Signal",
      slug: "sprint-smart-signal",
      product: "smart-signal",
      clusterId: clusterMap["prod-us-east"],
      status: "active" as const,
      currentManifestVersion: 21,
      lastCheckinAt: new Date(now - 300_000),
    },
  ]
  const insertedSites = await db
    .insert(fleetSite)
    .values(siteRows)
    .returning({ siteId: fleetSite.siteId, slug: fleetSite.slug })
  ...
  const previewDefs = [
    ...
    {
      name: "fix-inventory-sync",
      slug: "prev-fix-inv-sync",
      branch: "fix/inventory-sync",
      repo: "lepton/smart-inventory",
      pr: 87,
      owner: "bob",
    ...
    {
      name: "feat-dashboard-v2",
      slug: "prev-dashboard-v2",
      branch: "feat/dashboard-v2",
      repo: "lepton/smartops",
      pr: 201,
      owner: "charlie",
      site: "dt-smartops",
      status: "active" as const,
    ...
    {
      name: "perf-traffic-routing",
      slug: "prev-perf-traffic",
      branch: "perf/traffic-routing",
      repo: "lepton/trafficure",
      owner: "diana",
      site: "bmw-trafficure",
  ...
  await db.insert(gwRoute).values([
    {
      siteId: siteMap["verizon-network-access"],
      clusterId: clusterMap["prod-us-east"],
      kind: "ingress",
      domain: "na.verizon.lepton.io",
      pathPrefix: "/",
      targetService: "network-access-api",
      targetPort: 8080,
      status: "active",
      createdBy: "system",
    },
    {
      siteId: siteMap["walmart-smart-inventory"],
      clusterId: clusterMap["prod-us-east"],
      kind: "ingress",
      domain: "inventory.walmart.lepton.io",
      pathPrefix: "/",
      targetService: "smart-inventory-core",
      targetPort: 8080,
      status: "active",
      createdBy: "system",
    },
    {
      siteId: siteMap["bmw-trafficure"],
      clusterId: clusterMap["prod-eu-west"],
      kind: "ingress",
      domain: "trafficure.bmw.lepton.io",
      pathPrefix: "/",
      targetService: "trafficure-gateway",
      targetPort: 8080,
      status: "active",
      createdBy: "system",
    },
    {
      siteId: siteMap["target-smartmarket"],
      clusterId: clusterMap["prod-us-east"],
      kind: "ingress",
      domain: "market.target.lepton.io",
      pathPrefix: "/",
      targetService: "smartmarket-engine",
      targetPort: 8080,
      status: "active",
      createdBy: "system",
    },
    {
      siteId: siteMap["lepton-staging"],
      clusterId: clusterMap["staging-us"],
      kind: "ingress",
      domain: "staging.lepton.io",
      pathPrefix: "/api",
      targetService: "factory-api",
      targetPort: 4000,
      status: "active",
      createdBy: "system",
    },
    {
      siteId: siteMap["lepton-prod-us"],
      clusterId: clusterMap["prod-us-east"],
      kind: "ingress",
      domain: "app.lepton.io",
      pathPrefix: "/",
      targetService: "factory-api",
      targetPort: 4000,
      status: "active",
    ...
    {
      clusterId: clusterMap["dev-sandbox"],
      kind: "tunnel",
      domain: "alice-3000.tunnel.lepton.io",
      pathPrefix: "/",
    ...
    },
    {
      clusterId: clusterMap["dev-sandbox"],
      kind: "sandbox",
      domain: "sbx-bob.sandbox.lepton.io",
      pathPrefix: "/",
      targetService: "sbx-bob-staging",
      targetPort: 8080,
      status: "active",
      createdBy: "bob",
    },
  ...
  await db.insert(domain).values([
    ...
    {
      siteId: siteMap["verizon-network-access-eu"],
      fqdn: "na-eu.verizon.lepton.io",
      kind: "primary",
      dnsVerified: true,
      status: "active",
      createdBy: "system",
    ...
    {
      siteId: siteMap["walmart-smart-inventory"],
      fqdn: "inventory.walmart.lepton.io",
      kind: "primary",
      dnsVerified: true,
      status: "active",
      createdBy: "system",
  ...
  const defaultSystemSpec: SystemSpec = {
    namespace: "default",
  ...
  const componentRows: Array<{
    ...
    type: string
    systemId: string
    spec: GenericComponentSpec
  }> = [
    {
      name: "Network Access API",
      slug: "network-access-api",
      type: "service",
      systemId: systemMap["network-access"]!,
      spec: defaultComponentSpec,
    },
    {
      name: "Smart Inventory Core",
      slug: "smart-inventory-core",
      type: "service",
      systemId: systemMap["smart-inventory"]!,
      spec: defaultComponentSpec,
    },
    {
      name: "SmartOps Collector",
    ...
    },
    {
      ...
      slug: "smartmarket-engine",
      type: "service",
      systemId: systemMap["smartmarket"]!,
      spec: defaultComponentSpec,
    },
    {
      name: "Neo360 Platform",
      slug: "neo360-platform",
      type: "service",
      systemId: systemMap["neo360"]!,
      spec: defaultComponentSpec,
    },
    {
      name: "Factory API",
      slug: "factory-api-cmp",
      type: "service",
      systemId: systemMap["factory-platform"]!,
      spec: defaultComponentSpec,
    },
  ]
  const insertedComponents = await db
    ...
    .returning({ id: component.id, slug: component.slug })
  ...
  const v2SiteRows: Array<{ name: string; slug: string; spec: SiteSpec }> = [
    {
      name: "Verizon Network Access (US)",
      slug: "verizon-network-access",
      spec: { type: "dedicated", product: "network-access", status: "active" },
    },
    {
      name: "Verizon Network Access (EU)",
      slug: "verizon-network-access-eu",
    ...
    },
    {
      name: "Walmart Smart Inventory",
      slug: "walmart-smart-inventory",
      spec: { type: "dedicated", product: "smart-inventory", status: "active" },
    },
    {
      name: "BMW Trafficure",
      slug: "bmw-trafficure",
      spec: { type: "dedicated", product: "trafficure", status: "active" },
    },
    {
      name: "Target SmartMarket",
      slug: "target-smartmarket",
      spec: { type: "dedicated", product: "smartmarket", status: "active" },
    },
    {
      name: "Lepton Staging",
      slug: "lepton-staging",
      spec: { type: "shared", product: "platform", status: "active" },
    },
    {
      name: "Lepton Prod (US)",
      slug: "lepton-prod-us",
    ...
    },
    {
      name: "Deutsche Telekom SmartOps",
      slug: "dt-smartops",
      spec: { type: "dedicated", product: "smartops", status: "active" },
    },
    {
      name: "Acme Neo360",
      slug: "acme-neo360",
      spec: { type: "dedicated", product: "neo360", status: "provisioning" },
    },
    {
      name: "Sprint Smart Signal",
      slug: "sprint-smart-signal",
      spec: { type: "dedicated", product: "smart-signal", status: "active" },
    },
  ]
  const insertedV2Sites = await db
    .insert(site)
    .values(v2SiteRows)
  ...
  const sdRows: Array<{
    ...
    {
      name: "NA API @ Verizon US",
      slug: "na-api-verizon-us",
      type: "production",
      systemId: systemMap["network-access"]!,
      siteId: v2SiteMap["verizon-network-access"]!,
      runtimeId: runtimeMap["prod-us-east"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
        trigger: "manual",
    ...
    {
      name: "NA API @ Verizon EU",
      slug: "na-api-verizon-eu",
      type: "production",
      systemId: systemMap["network-access"]!,
      siteId: v2SiteMap["verizon-network-access-eu"]!,
      runtimeId: runtimeMap["prod-eu-west"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
        trigger: "manual",
    ...
    {
      name: "Inventory @ Walmart",
      slug: "inventory-walmart",
      type: "production",
      systemId: systemMap["smart-inventory"]!,
      siteId: v2SiteMap["walmart-smart-inventory"]!,
      runtimeId: runtimeMap["prod-us-east"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
    ...
    },
    {
      ...
      type: "production",
      systemId: systemMap["trafficure"]!,
      siteId: v2SiteMap["bmw-trafficure"]!,
      runtimeId: runtimeMap["prod-eu-west"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
    ...
    {
      name: "SmartMarket @ Target",
      slug: "smartmarket-target",
      type: "production",
      systemId: systemMap["smartmarket"]!,
      siteId: v2SiteMap["target-smartmarket"]!,
      runtimeId: runtimeMap["prod-us-east"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
    ...
    },
    {
      ...
      slug: "factory-staging",
      type: "staging",
      systemId: systemMap["factory-platform"]!,
      siteId: v2SiteMap["lepton-staging"]!,
      runtimeId: runtimeMap["staging-us"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
        trigger: "manual",
    ...
    {
      name: "Factory @ Prod US",
      slug: "factory-prod-us",
      type: "production",
      systemId: systemMap["factory-platform"]!,
      siteId: v2SiteMap["lepton-prod-us"]!,
      runtimeId: runtimeMap["prod-us-east"]!,
      spec: {
        status: "active",
        runtime: "kubernetes",
  ...
  const insertedSDs = await db
    .insert(systemDeployment)
    .values(sdRows)
    .returning({ id: systemDeployment.id, slug: systemDeployment.slug })
  ...
  await db.insert(componentDeployment).values([
    {
      systemDeploymentId: sdMap["na-api-verizon-us"]!,
      componentId: componentMap["network-access-api"]!,
      spec: {
        status: "running",
        replicas: 3,
        envOverrides: {},
        resourceOverrides: {},
        driftDetected: false,
      } satisfies ComponentDeploymentSpec,
    },
    {
      systemDeploymentId: sdMap["na-api-verizon-eu"]!,
      componentId: componentMap["network-access-api"]!,
      spec: {
        status: "running",
        replicas: 2,
    ...
    {
      systemDeploymentId: sdMap["inventory-walmart"]!,
      componentId: componentMap["smart-inventory-core"]!,
      spec: {
        status: "running",
        replicas: 4,
        envOverrides: {},
    ...
    {
      systemDeploymentId: sdMap["trafficure-bmw"]!,
      componentId: componentMap["trafficure-gateway"]!,
      spec: {
        status: "running",
        replicas: 2,
        envOverrides: {},
    ...
    {
      systemDeploymentId: sdMap["smartmarket-target"]!,
      componentId: componentMap["smartmarket-engine"]!,
      spec: {
        status: "running",
        replicas: 2,
        envOverrides: {},
    ...
    {
      systemDeploymentId: sdMap["factory-staging"]!,
      componentId: componentMap["factory-api-cmp"]!,
      spec: {
        status: "running",
        replicas: 1,
        envOverrides: {},
    ...
    {
      systemDeploymentId: sdMap["factory-prod-us"]!,
      componentId: componentMap["factory-api-cmp"]!,
      spec: {
        status: "running",
        replicas: 3,
    ...
    {
      systemDeploymentId: sdMap["smartops-dt"]!,
      componentId: componentMap["smartops-collector"]!,
      spec: {
        status: "running",
        replicas: 2,
        envOverrides: {},
  ...
  await db.insert(opsPreview).values([
    ...
    {
      siteId: v2SiteMap["walmart-smart-inventory"]!,
      sourceBranch: "fix/inventory-sync",
      prNumber: 87,
      phase: "building",
      spec: {
        repo: "lepton/smart-inventory",
    ...
    {
      siteId: v2SiteMap["dt-smartops"]!,
      sourceBranch: "feat/dashboard-v2",
      prNumber: 201,
      phase: "active",
      spec: {
        repo: "lepton/smartops",
    ...
    {
      siteId: v2SiteMap["bmw-trafficure"]!,
      sourceBranch: "perf/traffic-routing",
      phase: "deploying",
      spec: {
        repo: "lepton/trafficure",
        runtimeClass: "hot",
  ...
  await db.insert(infraRoute).values([
    {
      ...
      spec: {
        ...
        targets: [
          {
            tenantSlug: "verizon",
            systemDeploymentSlug: "na-api-verizon-us",
            port: 8080,
            weight: 100,
          },
        ],
    ...
    {
      name: "Walmart Inventory Ingress",
      slug: "rte-walmart-inv",
      type: "ingress",
      domain: "inventory.walmart.lepton.io",
      runtimeId: runtimeMap["prod-us-east"]!,
      spec: {
        ...
        targets: [
          {
            tenantSlug: "walmart",
            systemDeploymentSlug: "inventory-walmart",
            port: 8080,
            weight: 100,
          },
        ],
    ...
    {
      name: "BMW Trafficure Ingress",
      slug: "rte-bmw-traffic",
      type: "ingress",
      domain: "trafficure.bmw.lepton.io",
      runtimeId: runtimeMap["prod-eu-west"]!,
      spec: {
        ...
        targets: [
          {
            tenantSlug: "bmw",
            systemDeploymentSlug: "trafficure-bmw",
            port: 8080,
            weight: 100,
          },
        ],
    ...
    },
    {
      ...
      spec: {
        ...
        targets: [
          {
            tenantSlug: "lepton",
            systemDeploymentSlug: "factory-staging",
            port: 4000,
            weight: 100,
          },
        ],
    ...
    },
    {
      ...
      spec: {
        ...
        targets: [
          {
            tenantSlug: "lepton",
            systemDeploymentSlug: "factory-prod-us",
            port: 4000,
            weight: 100,
          },
        ],
  ...
  await db.insert(dnsDomain).values([
    {
      name: "na.verizon.lepton.io",
      slug: "dom-verizon-na",
      type: "primary",
      fqdn: "na.verizon.lepton.io",
      siteId: v2SiteMap["verizon-network-access"]!,
      spec: { verified: true, verifiedAt: new Date() } satisfies DnsDomainSpec,
    },
    {
      name: "inventory.walmart.lepton.io",
      slug: "dom-walmart-inv",
      type: "primary",
      fqdn: "inventory.walmart.lepton.io",
      siteId: v2SiteMap["walmart-smart-inventory"]!,
      spec: { verified: true, verifiedAt: new Date() } satisfies DnsDomainSpec,
    ...
    {
      name: "trafficure.bmw.lepton.io",
      slug: "dom-bmw-traffic",
      type: "primary",
      fqdn: "trafficure.bmw.lepton.io",
      siteId: v2SiteMap["bmw-trafficure"]!,
      spec: { verified: true, verifiedAt: new Date() } satisfies DnsDomainSpec,
    ...
    {
      name: "*.sandbox.lepton.io",
      slug: "dom-sandbox-wildcard",
      type: "wildcard",
      fqdn: "*.sandbox.lepton.io",
      spec: { verified: true, verifiedAt: new Date() } satisfies DnsDomainSpec,
    },
    {
      name: "*.tunnel.lepton.io",
      slug: "dom-tunnel-wildcard",
      type: "wildcard",
      fqdn: "*.tunnel.lepton.io",
      spec: { verified: true, verifiedAt: new Date() } satisfies DnsDomainSpec,
    },
