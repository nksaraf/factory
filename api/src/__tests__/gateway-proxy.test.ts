...
import { createGatewayServer, parseHostname } from "../modules/infra/gateway-proxy"
...
const SANDBOX_SLUG = "test-proxy"
const SANDBOX_DOMAIN = `${SANDBOX_SLUG}.sandbox.dx.dev`
const TERMINAL_DOMAIN = `${SANDBOX_SLUG}--terminal.sandbox.dx.dev`

// ---------------------------------------------------------------------------
// Mock backend: HTTP + WebSocket server simulating a sandbox service
// ---------------------------------------------------------------------------

let backendServer: ReturnType<typeof Bun.serve>
...
beforeAll(async () => {
  ...
  const mockCache = {
    get: async (domain: string) => {
      if (domain === SANDBOX_DOMAIN || domain === TERMINAL_DOMAIN) {
        return {
          kind: "sandbox",
          domain,
...
function gwFetch(path: string, opts?: RequestInit & { host?: string }) {
  const host = opts?.host ?? SANDBOX_DOMAIN
...
describe("parseHostname", () => {
  ...
  it("parses named endpoint (--terminal)", () => {
    const result = parseHostname("my-env--terminal.sandbox.dx.dev")
    expect(result).toEqual({
      family: "sandbox",
      slug: "my-env",
  ...
  it("parses port suffix (-p3000)", () => {
    const result = parseHostname("my-env-p3000.sandbox.dx.dev")
    expect(result).toEqual({
      family: "sandbox",
      slug: "my-env",
      port: 3000,
  ...
  it("strips port from host", () => {
    const result = parseHostname("my-env.sandbox.dx.dev:8080")
...
describe("HTTP proxy", () => {
  ...
  it("strips content-encoding after fetch auto-decompresses", async () => {
    ...
    // content-length from the original compressed response should not leak through
    // (Bun may re-add a correct content-length for the decompressed body, which is fine)
  ...
  it("returns 404 for unknown route", async () => {
    const res = await gwFetch("/echo", { host: "unknown.sandbox.dx.dev" })
...
describe("WebSocket proxy", () => {
  it("relays text messages bidirectionally", async () => {
    const ws = new WebSocket(
      ...
      { headers: { host: SANDBOX_DOMAIN } } as any,
    )
  ...
  it("closes browser WS when backend closes", async () => {
    // This test verifies the close propagation
    const ws = new WebSocket(
      `ws://localhost:${GATEWAY_PORT}/ws`,
      { headers: { host: SANDBOX_DOMAIN } } as any,
    )
