...
const skipTunnel = typeof globalThis.Bun === "undefined" || (!!process.env.CI && !process.env.FACTORY_URL && !process.env.RUN_TUNNEL_SMOKE)

describe.skipIf(skipTunnel)(`Tunnel Integration (${IS_PROD ? "prod" : "local"})`, () => {
  ...
  beforeAll(async () => {
    ...
    if (IS_PROD) {
      ...
      const wsUrl = FACTORY_URL!.replace(/^http/, "ws") + "/api/v1/factory/infra/tunnel-broker"
