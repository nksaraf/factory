...
import {
  RuntimeTypeSchema,
  ReverseProxyRuntimeSpecSchema,
  RouteStatusSchema,
  ResolvedTargetSchema,
  RouteSchema,
} from "@smp/factory-shared/schemas/infra";
import type { RouteTarget } from "@smp/factory-shared/schemas/infra";
import { resolveRouteTargets, type DbReader } from "../modules/infra/route-resolver";

describe("reverse-proxy runtime type", () => {
  test("RuntimeTypeSchema accepts 'reverse-proxy'", () => {
    const result = RuntimeTypeSchema.safeParse("reverse-proxy");
    expect(result.success).toBe(true);
  });

  test("ReverseProxyRuntimeSpecSchema validates proxy fields", () => {
    const result = ReverseProxyRuntimeSpecSchema.safeParse({
      engine: "traefik",
  ...
  test("ReverseProxyRuntimeSpecSchema rejects unknown engine", () => {
    const result = ReverseProxyRuntimeSpecSchema.safeParse({ engine: "haproxy" });
...
describe("route status schema", () => {
  ...
  test("RouteSchema now includes reconciliation fields", () => {
    const result = RouteSchema.safeParse({
      ...
      spec: {
        domain: "api.prod.dx.dev",
        targets: [{ tenantSlug: "acme", systemDeploymentSlug: "api", port: 8080, weight: 100 }],
      },
      status: {},
      generation: 0,
...
describe("resolveRouteTargets", () => {
  test("k8s-namespace: resolves to service DNS", async () => {
    ...
    expect(status.resolvedTargets[0].address).toBe("api-server.production.svc.cluster.local");
    expect(status.resolvedTargets[0].port).toBe(8080);
    expect(status.resolvedTargets[0].runtimeType).toBe("k8s-namespace");
  });
  ...
  test("compose-project: returns error phase", async () => {
    ...
    const mockReader: DbReader = {
      ...
      findHosts: async () => [{ id: "host_1", spec: { ipAddress: "10.0.1.10" } }],
    };
