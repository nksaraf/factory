...
// v2: subnet → substrate (type='subnet'), ipAddress fields moved to spec JSONB
import { substrate, ipAddress } from "../db/schema/infra-v2";
...
import type { NetworkDeviceAdapter, ArpEntry, DhcpLease, NetworkInterface } from "../adapters/network-device-adapter";
...
describe("IPAM Service", () => {
  ...
  beforeEach(async () => {
    ...
    // v2: ip_address is now in the infra schema, subnets are substrate entities
    await client.query(`TRUNCATE TABLE infra.ip_address RESTART IDENTITY CASCADE`);
    await client.query(`TRUNCATE TABLE infra.substrate RESTART IDENTITY CASCADE`);
  });
  ...
  describe("checkConflicts", () => {
    it("returns free for unknown addresses", async () => {
      const results = await ipamSvc.checkConflicts(db, ["10.0.1.99", "10.0.1.100"]);
    ...
    it("returns registered for existing addresses", async () => {
      const sub = await createTestSubnet();
      await ipamSvc.registerIp(db, { address: "10.0.1.50", subnetId: sub.subnetId });

      const results = await ipamSvc.checkConflicts(db, ["10.0.1.50", "10.0.1.51"]);
  ...
  describe("verifyOnNetwork", () => {
    ...
    it("detects ghost IPs from ARP table", async () => {
      const sub = await createTestSubnet();
      await ipamSvc.registerIp(db, { address: "10.0.1.10", subnetId: sub.subnetId });
      ...
      const mockAdapter: NetworkDeviceAdapter = {
        ...
        getArpTable: async () => [
          ...
          { ipAddress: "10.0.1.99", macAddress: "aa:bb:cc:dd:ee:02", hostname: "unknown-device" },
        ],
        getDhcpLeases: async () => [],
        getInterfaces: async () => [],
        ping: async () => false,
      ...
      const conflicts = await ipamSvc.verifyOnNetwork(db, mockAdapter, sub.subnetId);
    ...
    it("detects stale assignments", async () => {
      const sub = await createTestSubnet();
      const ip = await ipamSvc.registerIp(db, { address: "10.0.1.10", subnetId: sub.subnetId });
  ...
  describe("importIps", () => {
    it("imports rows and resolves subnet CIDRs", async () => {
      ...
      const result = await ipamSvc.importIps(db, [
        { address: "10.0.1.20", subnet_cidr: "10.0.1.0/24", hostname: "web-01", status: "available" },
        { address: "10.0.1.21", subnet_cidr: "10.0.1.0/24", hostname: "web-02", status: "available" },
      ]);
    ...
    it("skips conflicting addresses", async () => {
      const sub = await createTestSubnet();
      await ipamSvc.registerIp(db, { address: "10.0.1.20", subnetId: sub.subnetId });
    ...
    it("reports unknown subnet CIDRs", async () => {
      ...
      expect(result.errors.some((e) => e.includes("Unknown subnet CIDR"))).toBe(true);
    });
  });
  ...
  describe("exportIps", () => {
    ...
    it("exports as CSV with proper header", async () => {
      ...
      expect(lines[0]).toBe("address,subnet_cidr,hostname,purpose,status,assigned_to_type,assigned_to_id");
    ...
    it("filters by subnet", async () => {
      ...
      await ipamSvc.registerIp(db, { address: "10.0.1.10", subnetId: sub1.subnetId });
      await ipamSvc.registerIp(db, { address: "10.0.2.10", subnetId: sub2.subnetId });

      const result = await ipamSvc.exportIps(db, { subnetId: sub1.subnetId, format: "json" });
  ...
  describe("importFromDevice", () => {
    it("dry run returns preview without registering", async () => {
      const mockAdapter: NetworkDeviceAdapter = {
        type: "mock",
        getArpTable: async () => [
          { ipAddress: "10.0.1.50", macAddress: "aa:bb:cc:00:00:01", hostname: "device-a" },
      ...
      const result = await ipamSvc.importFromDevice(db, mockAdapter, { dryRun: true });
    ...
    it("deduplicates ARP and DHCP entries", async () => {
      const mockAdapter: NetworkDeviceAdapter = {
        ...
        getDhcpLeases: async () => [
          { ipAddress: "10.0.1.60", macAddress: "aa:bb:cc:00:00:01", status: "active" as const },
          { ipAddress: "10.0.1.61", macAddress: "aa:bb:cc:00:00:02", status: "active" as const },
        ],
        getInterfaces: async () => [],
        ping: async () => false,
      };
  ...
  describe("CSV export escaping", () => {
    it("escapes fields with commas", async () => {
      ...
      const [existing] = await db.select().from(ipAddress).where(eq(ipAddress.address, "10.0.1.10"));
      await db.update(ipAddress).set({ spec: { ...(existing.spec as Record<string, unknown>), hostname: "web,server" } as unknown as IpAddressSpec }).where(eq(ipAddress.address, "10.0.1.10"));
    ...
    it("prefixes formula-trigger characters", async () => {
      ...
      const [existing] = await db.select().from(ipAddress).where(eq(ipAddress.address, "10.0.1.10"));
      await db.update(ipAddress).set({ spec: { ...existing.spec, purpose: "=cmd|calc" } as IpAddressSpec }).where(eq(ipAddress.address, "10.0.1.10"));
