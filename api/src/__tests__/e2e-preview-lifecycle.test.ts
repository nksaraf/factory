...
describe.skipIf(!existsSync(KUBECONFIG_PATH))("E2E: Preview deployment lifecycle", () => {
  let db: Database;
  ...
  afterAll(async () => {
    try {
      if (workspaceSlug) {
        kubectlDirect(["delete", "namespace", `workspace-${workspaceSlug}`, "--ignore-not-found", "--wait=false"]);
      }
      if (previewSlug) {
  ...
  it("should seed infrastructure (substrate + runtime)", async () => {
    // Create substrate
    ...
    // Create runtime pointing to real k3d
  ...
  it("should create a workspace and reconcile it into a real K8s pod", async () => {
    ...
    const nsOutput = kubectlDirect(["get", "namespace", `workspace-${workspaceSlug}`, "-o", "name"]);
