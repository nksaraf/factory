...
import { site, systemDeployment, componentDeployment } from "../db/schema/ops";
...
describe("Reconciler", () => {
  ...
  async function seedComponentDeployment(opts?: {
    ...
    // Only create runtime if runtimeMode is kubernetes
    ...
    const [sys] = await db
      .insert(system)
      .values({ name: "billing", slug: "billing", spec: { namespace: "default", lifecycle: "production", tags: [] } })
      .returning();
    ...
    const [comp] = await db
      ...
      .values({
        ...
        spec: {
          ...
          healthcheck: { path: "/health", port: 8080, intervalSeconds: 30, timeoutSeconds: 5, failureThreshold: 3 },
          defaultReplicas: 2,
          defaultCpu: "500m",
          defaultMemory: "512Mi",
        } as ComponentSpec,
    ...
    const [siteRow] = await db
      .insert(site)
      .values({ name: "test-site", slug: "test-site", spec: { type: "shared", status: "active" } })
      .returning();
    const [sd] = await db
      ...
      .values({
        ...
        runtimeId: rtId,
        spec: {
          runtime: runtimeMode as "kubernetes" | "compose" | "systemd" | "windows_service" | "iis" | "process",
  ...
  it("sets task component deployments to completed", async () => {
    ...
    const updated = await db
      ...
      .from(componentDeployment)
      .where(eq(componentDeployment.id, cd.id));
    expect((updated[0].spec as ComponentDeploymentSpec).status).toBe("completed");
  });
  ...
  it("detects drift when actual image differs", async () => {
    ...
    const updated = await db
      ...
      .from(componentDeployment)
      .where(eq(componentDeployment.id, cd.id));
    expect((updated[0].spec as ComponentDeploymentSpec).driftDetected).toBe(true);
    expect((updated[0].spec as ComponentDeploymentSpec).actualImage).toBe("registry.dx.dev/api:v0.9.0");
  });
...
describe("generateWorkspaceResources", () => {
  ...
  it("generates resources with 3 container ports (ssh, web-terminal, web-ide)", () => {
    ...
    expect(workspace.ports.map((p) => p.name)).toEqual(["ssh", "web-terminal", "web-ide"]);
    expect(workspace.ports.map((p) => p.containerPort)).toEqual([22, 8080, 8081]);
  });
  ...
  it("uses dx-entrypoint.sh instead of sleep infinity in direct-image mode", () => {
    const resources = generateWorkspaceResources(baseWorkspace);
    const pod = resources.find((r) => r.kind === "Pod");
    const workspace = (pod!.spec as unknown as K8sPodSpec).containers.find((c) => c.name === "workspace")!;
  ...
  it("uses dx-entrypoint.sh in envbuilder init script", () => {
    ...
    const pod = resources.find((r) => r.kind === "Pod");
    const workspace = (pod!.spec as unknown as K8sPodSpec).containers.find((c) => c.name === "workspace")!;
    const initScriptEnv = workspace.env.find((e) => e.name === "ENVBUILDER_INIT_SCRIPT");
