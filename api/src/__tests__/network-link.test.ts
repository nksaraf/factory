...
describe("networkLink schemas", () => {
  ...
  test("NetworkLinkEndpointKindSchema accepts estate/host/realm", () => {
    for (const k of ["estate", "host", "realm"]) {
...
describe("traceFrom", () => {
  test("walks a linear chain A → B → C", async () => {
    ...
    const result = await traceFrom(reader, "realm", "rt_a", "outbound")
  ...
  test("inbound trace walks backwards", async () => {
    ...
    const result = await traceFrom(reader, "realm", "rt_c", "inbound")
    expect(result.hops).toHaveLength(2)
    expect(result.hops[0].entity.slug).toBe("factory-gw")
  ...
  test("throws NotFoundError when starting entity does not exist", async () => {
    ...
    await expect(
      traceFrom(reader, "realm", "rt_missing", "outbound")
    ).rejects.toThrow("Entity not found: realm/rt_missing")
  })
  ...
  test("empty graph returns no hops", async () => {
    ...
    const result = await traceFrom(reader, "realm", "rt_x", "outbound")
