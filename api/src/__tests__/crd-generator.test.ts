...
import type { ManifestRoute, ManifestDomain, ManifestV1 } from "@smp/factory-shared/types";
...
describe("CRD Generator", () => {
  ...
  describe("manifestDomainToCertificate", () => {
    it("generates Certificate CRD with correct dnsNames and issuerRef", () => {
      const crd = manifestDomainToCertificate(baseDomain, "gateway", "letsencrypt-prod");
    ...
    it("uses tlsCertRef as secretName when provided", () => {
      const domain: ManifestDomain = { ...baseDomain, tlsCertRef: "my-existing-secret" };
      const crd = manifestDomainToCertificate(domain, "gateway", "letsencrypt-prod");
  ...
  describe("manifestToMiddlewares", () => {
    it("generates deduplicated Middleware CRDs from routes", () => {
      ...
      expect((corsCrd.spec as any).headers.accessControlAllowOriginList).toEqual(["*"]);

      const rateCrd = crds.find((c) => c.metadata.name === "dx-mw-rate-limit")!;
      expect((rateCrd.spec as any).rateLimit).toEqual({ average: 100, burst: 50 });
  ...
  describe("manifestToCRDs", () => {
    ...
    it("produces correct counts for a full manifest", () => {
      ...
      for (const crd of [...result.ingressRoutes, ...result.certificates, ...result.middlewares]) {
        expect(crd.metadata.namespace).toBe("gateway");
      }
    });
  });
