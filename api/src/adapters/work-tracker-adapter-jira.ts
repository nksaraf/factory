...
export class JiraWorkTrackerAdapter implements WorkTrackerAdapter {
  ...
  private async throwOnError(res: Response, label: string): Promise<never> {
    let detail = ""
    try {
    ...
    throw new Error(`Jira ${label} failed: ${res.status}${detail}`)
  }
  ...
  async listProjects(
    ...
    while (true) {
      ...
      if (!res.ok) await this.throwOnError(res, "listProjects")
  ...
  async fetchIssues(
    ...
    while (true) {
      ...
      if (!res.ok) await this.throwOnError(res, "fetchIssues")
      const data = (await res.json()) as {
        issues: JiraIssueRaw[]
  ...
  async getIssue(
    ...
    if (!res.ok) await this.throwOnError(res, "getIssue")
    ...
    const res = await fetch(`${apiUrl}/rest/api/3/issue`, {
      ...
      body: JSON.stringify({
        fields: {
          ...
          labels: spec.labels ?? [],
          parent: spec.parentExternalId
            ? { key: spec.parentExternalId }
            : undefined,
        },
      }),
    })
    if (!res.ok) await this.throwOnError(res, "pushIssue")
  ...
  async pushIssues(
    ...
    for (const spec of specs) {
      results.push(
        await this.pushIssue(apiUrl, credentialsRef, projectId, spec)
      )
    }
    return results
  }
  ...
  async updateIssueStatus(
    ...
    if (!transRes.ok) await this.throwOnError(transRes, "getTransitions")
    ...
    if (!res.ok) await this.throwOnError(res, "updateIssueStatus")
  }
