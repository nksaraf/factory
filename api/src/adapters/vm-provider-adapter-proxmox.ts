...
import { substrate, host, ipAddress } from "../db/schema/infra-v2";
...
/** Extract Proxmox connection info from a hypervisor substrate spec. */
function getClusterConfig(hypervisor: InfraSubstrate) {
...
export class ProxmoxVmProviderAdapter implements VMProviderAdapter {
  ...
  async syncInventory(hypervisor: InfraSubstrate): Promise<SyncResult> {
    const db = this.db;
    ...
    try {
      ...
      await db
        .update(substrate)
        .set({
          spec: sql`${substrate.spec} || '{"syncStatus":"syncing","syncError":null}'::jsonb`,
          updatedAt: new Date(),
        })
        .where(eq(substrate.id, hypervisor.id));
      ...
      await db
        .update(substrate)
        .set({
          spec: sql`${substrate.spec} || ${JSON.stringify({
            syncStatus: "idle",
  ...
  private async syncNodes(
    ...
    const existingHosts = await db
      ...
      .where(
        and(
          eq(host.substrateId, hypervisor.id),
          eq(host.type, "bare-metal"),
        ),
      );
    ...
    for (const node of nodesWithStatus) {
      ...
      let nodeIpAddress: string | null =
        (existing?.spec as Record<string, unknown>)?.ipAddress as string | null || null;
      if (!nodeIpAddress && nodesWithStatus.length === 1 && hypervisorSpec.apiHost) {
        nodeIpAddress = (hypervisorSpec.apiHost as string).replace(/^https?:\/\//, "");
      }
      ...
      // Upsert IP address
      if (nodeIpAddress) {
        const hostRecord = existing || (await db
          .select()
          .from(host)
          .where(and(eq(host.name, node.node), eq(host.substrateId, hypervisor.id)))
          .limit(1)
          .then((r) => r[0]));
  ...
  private async syncVms(
    ...
    for (const pvm of vms) {
      ...
      const hostSpec = {
        ...
        ipAddress: ipAddressValue ?? ((existing?.spec as Record<string, unknown>)?.ipAddress as string | undefined) ?? undefined,
      };
  ...
  async startVm(_hypervisor: InfraSubstrate, externalId: string): Promise<void> {
    const ctx = await getVmContext(this.db, externalId);
    const upid = await ctx.client.startVm(ctx.nodeName, ctx.vmid, ctx.vmType);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  }

  async stopVm(_hypervisor: InfraSubstrate, externalId: string): Promise<void> {
    const ctx = await getVmContext(this.db, externalId);
    const upid = await ctx.client.stopVm(ctx.nodeName, ctx.vmid, ctx.vmType);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  }

  async restartVm(_hypervisor: InfraSubstrate, externalId: string): Promise<void> {
    const ctx = await getVmContext(this.db, externalId);
  ...
  async resizeVm(
    ...
    if (Object.keys(configUpdate).length > 0) {
      await ctx.client.updateVmConfig(ctx.nodeName, ctx.vmid, configUpdate, ctx.vmType);
    }

    if (spec.diskGb != null) {
      await ctx.client.resizeDisk(ctx.nodeName, ctx.vmid, "scsi0", `${spec.diskGb}G`);
    }
  }
  ...
  async migrateVm(
    ...
    const upid = await ctx.client.migrateVm(ctx.nodeName, ctx.vmid, targetNodeName, true);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  }
  ...
  async snapshotVm(
    ...
    const upid = await ctx.client.createSnapshot(ctx.nodeName, ctx.vmid, snapshotName, description);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  ...
  async restoreSnapshot(
    ...
    const ctx = await getVmContext(this.db, externalId);
    const upid = await ctx.client.rollbackSnapshot(ctx.nodeName, ctx.vmid, snapshotName);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  }
  ...
  async deleteSnapshot(
    ...
    const ctx = await getVmContext(this.db, externalId);
    const upid = await ctx.client.deleteSnapshot(ctx.nodeName, ctx.vmid, snapshotName);
    await ctx.client.waitForTask(ctx.nodeName, upid);
  }

  async destroyVm(_hypervisor: InfraSubstrate, externalId: string): Promise<void> {
    const ctx = await getVmContext(this.db, externalId);
