...
const vmProviderAdapters: Partial<
  Record<VmProviderType, (db: Database) => VMProviderAdapter>
> = {
  proxmox: (db) => new ProxmoxVmProviderAdapter(db),
}

export function getVMProviderAdapter(
  type: VmProviderType,
  db: Database
): VMProviderAdapter {
...
const observabilityAdapters: Partial<
  Record<
    ObservabilityType,
    (opts?: { lokiUrl?: string }) => ObservabilityAdapter
  >
> = {
  noop: () => new NoopObservabilityAdapter(),
  loki: (opts) =>
    new LokiObservabilityAdapter(opts?.lokiUrl ?? "http://infra-loki:3100"),
}
...
const gitHostAdapters: Partial<
  Record<GitHostType, (config: GitHostAdapterConfig) => GitHostAdapter>
> = {
...
const messagingAdapters: Partial<
  Record<MessagingType, () => MessagingAdapter>
> = {
...
const gatewayAdapters: Partial<
  Record<GatewayType, (config?: { outputDir?: string }) => GatewayAdapter>
> = {
  noop: () => new NoopGatewayAdapter(),
  file: (config) =>
    new FileGatewayAdapter(config?.outputDir ?? "./gateway-output"),
}
...
export function getSandboxAdapter(type: SandboxType = "noop"): SandboxAdapter {
...
const networkDeviceAdapters: Partial<
  Record<
    NetworkDeviceType,
    (config: NetworkDeviceAdapterConfig) => NetworkDeviceAdapter
  >
> = {
...
const identityProviderAdapters: Record<
  IdentityProviderType,
  () => IdentityProviderAdapter
> = {
