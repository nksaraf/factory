...
import { factorySettingsSchema, type FactorySettings } from "../settings.js"
...
function applyEnvFallbacks(s: FactorySettings): FactorySettings {
  ...
  if (
    envMode &&
    (envMode === "factory" || envMode === "site" || envMode === "dev")
  ) {
    mode = envMode
  }
  ...
  const siteName =
    emptyToUndefined(s.factory.site?.name) ??
    process.env.FACTORY_SITE_NAME?.trim() ??
    ""
  const siteFactoryUrl =
    emptyToUndefined(s.factory.site?.factoryUrl) ??
    process.env.FACTORY_URL?.trim() ??
    ""
  ...
  return {
    factory: {
      ...
      auth: {
        jwksUrl: jwksUrl ?? "",
        serviceUrl: s.factory.auth.serviceUrl ?? "",
      },
      log: s.factory.log,
      site: {
        ...s.factory.site,
