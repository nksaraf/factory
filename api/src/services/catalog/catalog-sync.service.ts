...
export async function syncCatalogSystem(
  ...
  for (const [compName, comp] of Object.entries(components)) {
    await upsertComponent(db, {
      ...
      healthcheck: comp.spec.healthcheck ?? null,
      replicas: comp.spec.replicas ?? 1,
      cpu: comp.spec.resources?.cpu ?? "100m",
      memory: comp.spec.resources?.memory ?? "128Mi",
  ...
  for (const c of comps) {
    components[c.name] = {
      ...
      spec: {
        ...
        healthcheck: c.healthcheck as CatalogSystem["components"][string]["spec"]["healthcheck"],
        replicas: c.replicas,
