...
import type { PipelineRunSpec, PipelineStepSpec } from "@smp/factory-shared/schemas/build";
...
type PipelineRunSpecStored = Omit<PipelineRunSpec, "startedAt" | "completedAt"> & {
  triggerEvent?: string;
  triggerRef?: string;
  triggerActor?: string;
  workflowFile?: string;
...
type PipelineStepSpecStored = Omit<PipelineStepSpec, "name" | "status" | "startedAt" | "completedAt"> & {
  name?: string;
  jobName?: string;
  stepName?: string;
  status?: string;
...
export interface CreatePipelineRunInput {
  ...
  commitSha: string;
  workflowFile?: string;
  workspaceId?: string; // v2: sandboxId → workspaceId
...
export async function createPipelineRun(db: Database, input: CreatePipelineRunInput) {
  // v2: triggerEvent, triggerRef, triggerActor, workflowFile → spec JSONB
  // trigger field maps to the PipelineRunSpec enum; default to "push" if unknown
  const triggerValue = (["push", "pull_request", "manual", "schedule", "tag"] as const)
    .find((t) => t === input.triggerEvent) ?? "push";
...
export async function listPipelineRuns(db: Database, query: ListPipelineRunsQuery = {}) {
  ...
  // v2: triggerEvent, triggerRef are in spec JSONB — filter post-query
  ...
  if (query.triggerEvent) {
    rows = rows.filter((r) => (r.spec as PipelineRunSpecStored)?.triggerEvent === query.triggerEvent);
  }
  if (query.triggerRef) {
    rows = rows.filter((r) => (r.spec as PipelineRunSpecStored)?.triggerRef === query.triggerRef);
  }
...
export async function updatePipelineRun(db: Database, id: string, input: UpdatePipelineRunInput) {
  ...
  // v2: merge updates into spec JSONB for non-column fields
  const newSpec: PipelineRunSpecStored = {
    ...
    ...(input.completedAt !== undefined ? { completedAt: input.completedAt.toISOString() } : {}),
  };
...
export async function cancelPipelineRun(db: Database, id: string) {
  ...
  const [row] = await db
    .update(pipelineRun)
    .set({ status: "cancelled", spec: toPipelineRunSpec(newSpec), updatedAt: new Date() })
    .where(
      and(
...
// ---------------------------------------------------------------------------
// Pipeline Step CRUD (v2: pipelineStepRun → pipelineStep)
// ---------------------------------------------------------------------------

export async function createStepRun(db: Database, input: CreateStepInput) {
  // v2: jobName, stepName → spec JSONB
  const [row] = await db
    .insert(pipelineStep)
    .values({
...
export async function updateStepRun(db: Database, stepId: string, input: UpdateStepInput) {
