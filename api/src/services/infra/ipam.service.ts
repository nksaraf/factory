...
// TODO: fix type — IpAddressSpec doesn't yet include hostname/purpose/assignedToType/assignedToId
// as explicit fields in the shared schema. Store them in JSONB via intersection until promoted.
...
function toIpSpec(stored: IpAddressSpecStored): IpAddressSpec {
  return stored as unknown as IpAddressSpec;
}
...
export async function createSubnet(
  ...
  const [row] = await db.insert(substrate).values({
...
export async function listAvailableIps(db: Database, subnetId?: string) {
  ...
  const rows = await db.select().from(ipAddress).where(and(...conditions));
...
export async function assignIp(
  ...
  const newSpec: IpAddressSpecStored = {
    ...
    hostname: data.hostname ?? existingSpec.hostname ?? null,
    purpose: data.purpose ?? existingSpec.purpose ?? null,
  };
...
export async function getIpamStats(db: Database, subnetId?: string) {
  const conditions = subnetId ? and(eq(ipAddress.subnetId, subnetId)) : undefined;
  ...
  }).from(ipAddress).where(conditions);
...
export async function allocateNextAvailable(
  ...
  return db.transaction(async (tx) => {
    const orderClause = data.policy === "random"
      ? sql`ORDER BY random()`
    ...
    const [existing] = await tx.select().from(ipAddress).where(eq(ipAddress.id, ipId));
    const existingSpec = (existing?.spec ?? {}) as IpAddressSpecStored;
...
export async function verifyOnNetwork(
  ...
  for (const lease of dhcpLeases) {
    ...
    if (!dbByAddress.has(lease.ipAddress) && !conflicts.some((c) => c.address === lease.ipAddress)) {
      conflicts.push({
...
export async function importIps(
  ...
  const uniqueCidrs = [...new Set(rows.map((r) => r.subnet_cidr).filter(Boolean))] as string[];
  const allSubnets = await db.select().from(substrate).where(eq(substrate.type, "subnet"));
  ...
  // Check conflicts
  const conflicts = await checkConflicts(db, rows.map((r) => r.address));
  ...
  const toAssign = rows
    .filter((r) => r.assigned_to_type && r.assigned_to_id && !conflictAddresses.has(r.address))
    .map((r) => ({
      address: r.address,
...
export async function exportIps(
  ...
  const conditions = filters?.subnetId ? eq(ipAddress.subnetId, filters.subnetId) : undefined;
  ...
  if (filters?.format === "csv") {
    const header = "address,subnet_cidr,hostname,purpose,status,assigned_to_type,assigned_to_id";
    const lines = data.map(
      (r) =>
        [r.address, r.subnet_cidr, r.hostname, r.purpose, r.status, r.assigned_to_type, r.assigned_to_id]
          .map(csvEscape)
          .join(",")
    );
...
export async function importFromDevice(
  ...
  preview: Array<{ address: string; mac: string; hostname?: string; conflict: boolean }>;
  result?: BulkResult;
}> {
  ...
  for (const entry of arpTable) {
    seen.set(entry.ipAddress, { mac: entry.macAddress, hostname: entry.hostname });
  }
  for (const lease of dhcpLeases) {
    if (lease.status === "active" && !seen.has(lease.ipAddress)) {
      seen.set(lease.ipAddress, { mac: lease.macAddress, hostname: lease.hostname });
    }
  }
