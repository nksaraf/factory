...
export const metadataCol = () =>
  jsonb("metadata").$type<EntityMetadata>().notNull().default(sql`'{}'`);
...
export const specCol = <T>() =>
  jsonb("spec").$type<T>().notNull().default(sql`'{}'`);
...
export const bitemporalCols = () => ({
  ...
  systemFrom: timestamp("system_from", { withTimezone: true }).defaultNow().notNull(),
...
export const reconciliationCols = () => ({
  status: jsonb("status").$type<Record<string, unknown>>().notNull().default(sql`'{}'`),
