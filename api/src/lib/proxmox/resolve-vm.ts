...
export async function resolveVmHost(db: Database, identifier: string) {
  ...
  const allVmHosts = await db.select().from(host).where(eq(host.type, "vm"))
  ...
  if (matches.length > 1) {
    const descriptions = matches.map((h) => {
      const spec = (h.spec ?? {}) as Record<string, unknown>
 ...
 * and the hypervisor estate (for Proxmox credentials).
 */
export async function getVmContext(
  ...
  if (isNaN(vmid)) {
    throw new Error(
      `VM host ${vmHost.id} has non-numeric external ID: ${externalId}`
    )
  }

  // The VM's estateId points to the hypervisor estate
  if (!vmHost.estateId) {
    throw new Error(`VM host ${vmHost.id} is not linked to an estate`)
  }

  // Find the hypervisor estate for credentials
  const [hypervisor] = await db
    .select()
    .from(estate)
    .where(eq(estate.id, vmHost.estateId))
    .limit(1)

  if (!hypervisor) {
    throw new Error(`Estate not found: ${vmHost.estateId}`)
  }

  const hypervisorSpec = (hypervisor.spec ?? {}) as Record<string, unknown>

  // Build client from estate spec
