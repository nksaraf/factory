...
import { emitEvent, cleanupExpiredSubscriptions } from "./workflow-events";
...
describe("emitEvent", () => {
  it("wakes a matching subscription via send()", async () => {
    await insertSub({
      ...
      matchFields: { workspaceId: "ws-123" },
    });

    await emitEvent(db, "workspace.ready", { workspaceId: "ws-123", status: "active" });

    expect(mockSend).toHaveBeenCalledOnce();
    expect(mockSend).toHaveBeenCalledWith(
      "wfr_abc",
      { workspaceId: "ws-123", status: "active" },
      "workspace.ready",
    );
  });
  ...
  it("does not match subscriptions with different event names", async () => {
    ...
    await emitEvent(db, "workspace.ready", { workspaceId: "ws-123" });
  ...
  it("does not match when matchFields are not contained in event data", async () => {
    await insertSub({
      ...
      workflowRunId: "wfr_abc",
      eventName: "workspace.ready",
      matchFields: { workspaceId: "ws-999" },
    });

    // Emit with different workspaceId
    await emitEvent(db, "workspace.ready", { workspaceId: "ws-123", status: "active" });
  ...
  it("does not match when only partial matchFields are present", async () => {
    ...
    await emitEvent(db, "pr.comment", { repoFullName: "org/repo", comment: "LGTM" });
...
describe("cleanupExpiredSubscriptions", () => {
  it("removes expired subscriptions", async () => {
    ...
    await insertSub({
      ...
      eventName: "workspace.ready",
      matchFields: { workspaceId: "ws-old" },
      expiresAt: new Date(Date.now() - 60_000), // expired 1 minute ago
    });
    await insertSub({
      id: "esub_act",
      ...
      eventName: "workspace.ready",
      matchFields: { workspaceId: "ws-new" },
