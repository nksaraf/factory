...
describe("updateRun", () => {
  ...
  it("merges state atomically without overwriting", async () => {
    ...
    // Second update: set workspaceId (should NOT lose branchName)
    await updateRun(db, "wfr_upd", { state: { workspaceId: "ws-123" } });
  ...
  it("stores error on failure", async () => {
    ...
    expect(row.error).toBe("Workspace timed out");
  });

  it("stores output on completion", async () => {
    const output = { branchName: "feat/test", prUrl: "https://github.com/org/repo/pull/1" };
