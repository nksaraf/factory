...
export function pipelineRunController(db: Database) {
  return new Elysia({ prefix: "/runs" })
    ...
    .get("/:id", async ({ params, set }) => {
      const run = await pipelineRunSvc.getPipelineRunWithSteps(db, params.id);
      if (!run) { set.status = 404; return { success: false, error: "not_found" }; }
      return { success: true, data: run };
    }, {
    ...
    .post("/:id/update", async ({ params, body, set }) => {
      ...
      if (!updated) { set.status = 404; return { success: false, error: "not_found" }; }
    ...
    .post("/:id/cancel", async ({ params, set }) => {
      const cancelled = await pipelineRunSvc.cancelPipelineRun(db, params.id);
      if (!cancelled) { set.status = 404; return { success: false, error: "not_found_or_not_cancellable" }; }
    ...
    // --- Create step run ---
    .post("/:id/steps", async ({ params, body, set }) => {
      const run = await pipelineRunSvc.getPipelineRun(db, params.id);
      if (!run) { set.status = 404; return { success: false, error: "run_not_found" }; }
