 ...
 */
function normalizeGitHubEventType(
  event: string,
  action?: string,
  payload?: any
): string {
  switch (event) {
    ...
    case "pull_request":
      switch (action) {
        case "opened":
          return "code.pr.opened"
        case "reopened":
          return "code.pr.reopened"
        case "synchronize":
          return "code.pr.updated"
        case "closed":
          return payload?.pull_request?.merged
            ? "code.pr.merged"
            : "code.pr.closed"
        default:
          return `code.pr.${action ?? "unknown"}`
      }
    case "issue_comment":
      return action === "created"
        ? "code.pr.commented"
        : `code.comment.${action ?? "unknown"}`
    case "ping":
      return "system.ping"
    default:
      return `code.${event}`
...
export function webhookController(db: Database) {
  ...
  return new Elysia({ prefix: "/webhooks" }).post(
    ...
    async ({ params, headers, body, set }) => {
      ...
      const sender = (payload as any)?.sender as
        | Record<string, unknown>
        | undefined
      const repo = (payload as any)?.repository as
        | Record<string, unknown>
        | undefined
      ...
      if (senderIdStr) {
        actorPrincipalId = await resolveActorPrincipal(
          db,
          "github",
          senderIdStr
        ).catch(() => null)
      ...
      const normalizedEventType = normalizeGitHubEventType(
        event ?? "unknown",
        action,
        payload
      )
      ...
      if (!provider) {
        wlog.warn(
          ...
          `github webhook provider not found: ${params.providerId}`
        )
        if (eventId)
      ...
      // webhook secret is a dedicated field in spec, separate from the API credentials
      ...
      if (eventId)
        await updateWebhookEventStatus(db, eventId, { status: "processing" })
      ...
      if (!result.accepted) {
        if (result.reason === "invalid_signature") {
          wlog.warn(
            ...
            `github webhook signature invalid (${event ?? "unknown"} from ${repoName ?? "unknown"})`
          )
        } else {
          wlog.info(
            {
            ...
            `github webhook rejected: ${result.reason}`
          )
        }
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
