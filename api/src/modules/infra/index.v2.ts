...
import { resolveRouteTargets, drizzleDbReader } from "./route-resolver";
import { traceFrom, drizzleGraphReader, validateEndpoints } from "./trace";
import { ok, list } from "../../lib/responses";
import { parsePagination, countRows, paginationMeta } from "../../lib/pagination";
import { z } from "zod";
...
export function infraControllerV2(db: Database) {
  return new Elysia({ prefix: "/infra" })
        ...
        actions: {
          sync: {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const [row] = await db.update(substrate)
                .set({
                  spec: { ...spec, lastSyncRequestedAt: new Date(), forceSyncRequested: parsed.force ?? false } as SubstrateSpec,
                  updatedAt: new Date(),
                })
        ...
        actions: {
          ...
          start: {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const [row] = await db.update(host)
                .set({ spec: { ...spec, lifecycle: "active", lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
          ...
          stop: {
            ...
            handler: async ({ db, entity, body }) => {
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, lifecycle: "offline", lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          restart: {
            ...
            handler: async ({ db, entity, body }) => {
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, lifecycle: "active", lastActionAt: new Date(), lastRestartAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          resize: {
            ...
            handler: async ({ db, entity, body }) => {
              const parsed = body as { cpu?: number; memoryMb?: number; diskGb?: number };
              const spec = entity.spec as HostSpec;
              const merged: HostSpec = { ...spec };
              if (parsed.cpu !== undefined) merged.cpu = parsed.cpu;
              if (parsed.memoryMb !== undefined) merged.memoryMb = parsed.memoryMb;
          ...
          migrate: {
            ...
            handler: async ({ db, entity, body }) => {
              const parsed = body as { targetSubstrateId: string; reason?: string };
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({
                  substrateId: parsed.targetSubstrateId,
                  spec: { ...spec, lastMigratedAt: new Date(), migrationReason: parsed.reason } as HostSpec,
                  updatedAt: new Date(),
                })
                .where(eq(host.id, entity.id as string))
                .returning();
          ...
          clone: {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const spec = entity.spec as HostSpec;
              const [row] = await db.insert(host)
                .values({
                  slug: parsed.slug,
                  name: parsed.name,
          ...
          snapshot: {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const spec = entity.spec as HostSpec;
              const snapshots = ((spec as Record<string, unknown>).snapshots ?? []) as Array<Record<string, unknown>>;
              snapshots.push({
              ...
              const [row] = await db.update(host)
                .set({ spec: { ...spec, snapshots } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          "restore-snapshot": {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, restoringSnapshot: parsed.snapshotId, lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          pause: {
            ...
            handler: async ({ db, entity, body }) => {
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, schedulable: false, lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          resume: {
            ...
            handler: async ({ db, entity, body }) => {
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, schedulable: true, lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          evacuate: {
            ...
            handler: async ({ db, entity, body }) => {
              const spec = entity.spec as HostSpec;
              const [row] = await db.update(host)
                .set({ spec: { ...spec, schedulable: false, evacuating: true, lastActionAt: new Date() } as HostSpec, updatedAt: new Date() })
                .where(eq(host.id, entity.id as string))
                .returning();
              return row;
            },
      ...
      ontologyRoutes(db, {
        ...
        actions: {
          upgrade: {
            ...
            handler: async ({ db, entity, body }) => {
              const parsed = body as { targetVersion: string; strategy: string };
              const spec = entity.spec as RuntimeSpec;
              const [row] = await db.update(runtime)
                .set({
                  spec: {
                    ...spec,
      ...
      ontologyRoutes(db, {
        ...
        hooks: {
          beforeCreate: async ({ parsed }) => {
            ...
            const resolved = await resolveRouteTargets(spec.targets ?? [], reader);
            return { ...parsed, status: resolved, generation: 1, observedGeneration: 1 };
          },
          beforeUpdate: async ({ entity, parsed }) => {
            ...
            const reader = drizzleDbReader(db);
            const resolved = await resolveRouteTargets(mergedSpec.targets ?? [], reader);
            const newGen = ((entity as Record<string, unknown>).generation as number ?? 0) + 1;
            return { ...parsed, spec: mergedSpec, status: resolved, generation: newGen, observedGeneration: newGen };
          },
        },
      }),
    )
        ...
        actions: {
          verify: {
            handler: async ({ db, entity }) => {
              ...
              const [row] = await db.update(dnsDomain)
                .set({ spec: { ...spec, verified: true, verifiedAt: new Date() }, updatedAt: new Date() })
        ...
        actions: {
          revoke: {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const [row] = await db.update(secret)
                .set({
                  spec: { ...spec, revokedAt: new Date(), revokeReason: parsed.reason, status: "revoked" } as SecretSpec,
                  updatedAt: new Date(),
                })
      ...
      ontologyRoutes(db, {
        ...
        hooks: {
          ...
          beforeUpdate: async ({ db: hookDb, entity, parsed }) => {
            if (parsed.sourceId || parsed.targetId || parsed.sourceKind || parsed.targetKind) {
      ...
      ontologyRoutes(db, {
        ...
        actions: {
          close: {
            ...
            handler: async ({ db, entity }) => {
              const [row] = await db.update(tunnel)
      ...
      ontologyRoutes(db, {
        ...
        actions: {
          assign: {
            ...
            handler: async ({ db, entity, body }) => {
              const parsed = body as { assignedToType: string; assignedToId: string };
              const spec = entity.spec as IpAddressSpec;
              const [row] = await db.update(ipAddress)
                .set({
                  spec: { ...spec, status: "assigned", assignedToType: parsed.assignedToType, assignedToId: parsed.assignedToId },
                  updatedAt: new Date(),
                })
          ...
          release: {
            handler: async ({ db, entity }) => {
              ...
              const [row] = await db.update(ipAddress)
                .set({
                  spec: { ...spec, status: "available", assignedToType: undefined, assignedToId: undefined },
                  updatedAt: new Date(),
                })
                .where(eq(ipAddress.id, entity.id as string))
                .returning();
    ...
    .post("/trace", async ({ body }) => {
      ...
      if (!parsed.success) {
        throw new ValidationError("Invalid trace request", parsed.error.flatten());
      }
      const { entityKind, entityId, direction } = parsed.data;
      const reader = drizzleGraphReader(db);
      const result = await traceFrom(reader, entityKind, entityId, direction);
      return ok(result);
    }, {
      detail: { tags: ["infra/trace"], summary: "Trace network path from entity" },
    })

    // ── IPAM: Allocate next available IP ─────────────────
    .post("/ip-addresses/allocate", async ({ body }) => {
      const parsed = AllocateIpBody.safeParse(body);
      if (!parsed.success) {
        throw new ValidationError("Invalid allocate request", parsed.error.flatten());
      }
      const { subnetId, assignedToType, assignedToId, strategy } = parsed.data;
      ...
      detail: { tags: ["infra/ip-addresses"], summary: "Allocate next available IP from subnet" },
    })

    // ── IPAM: Lookup by address ──────────────────────────
    .post("/ip-addresses/lookup", async ({ body }) => {
      ...
      detail: { tags: ["infra/ip-addresses"], summary: "Lookup IP by address" },
    })

    // ── IPAM: Stats ──────────────────────────────────────
    .get("/ip-addresses/stats", async ({ query }) => {
      ...
      detail: { tags: ["infra/ip-addresses"], summary: "Get IPAM statistics" },
    })

    // ── Access: Resolve SSH target ───────────────────────
    .get("/access/resolve/:slug", async ({ params }) => {
      const { resolveTarget } = await import("../../services/infra/access.service");
      const target = await resolveTarget(db, params.slug);
      if (!target) throw new NotFoundError(`SSH target '${params.slug}' not found`);
      return ok(target);
    }, {
      detail: { tags: ["infra/access"], summary: "Resolve SSH target by slug" },
    })

    // ── Access: List all SSH targets ─────────────────────
    .get("/access/targets", async ({ query }) => {
      const { listTargets } = await import("../../services/infra/access.service");
      ...
      detail: { tags: ["infra/access"], summary: "List all SSH-connectable targets" },
    })

    // ── Assets: Aggregated infrastructure summary ────────
    .get("/assets", async () => {
      const [substrateCount] = await db.select({ count: sql<number>`count(*)::int` }).from(substrate);
      const [hostCount] = await db.select({ count: sql<number>`count(*)::int` }).from(host);
      const [runtimeCount] = await db.select({ count: sql<number>`count(*)::int` }).from(runtime);
      const [routeCount] = await db.select({ count: sql<number>`count(*)::int` }).from(route);
      const [tunnelCount] = await db.select({ count: sql<number>`count(*)::int` }).from(tunnel);
      const [ipCount] = await db.select({ count: sql<number>`count(*)::int` }).from(ipAddress);
      const [secretCount] = await db.select({ count: sql<number>`count(*)::int` }).from(secret);
      const [linkCount] = await db.select({ count: sql<number>`count(*)::int` }).from(networkLink);
      ...
      detail: { tags: ["infra/assets"], summary: "Get infrastructure asset summary" },
    })
