/**
 * v2 Messaging controller.
 *
 * Maps legacy messaging routes to v2 ontology tables:
        ...
        actions: {
          ...
          "sync-users": {
            handler: async ({ db, entity }) => {
              const spec = entity.spec as Record<string, unknown>;
              const [row] = await db.update(messagingProvider)
          ...
          "map-channel": {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const spec = entity.spec as Record<string, unknown>;
              const channels = (spec.channelMappings ?? []) as Array<Record<string, unknown>>;
              channels.push({ externalChannelId: b.externalChannelId, externalChannelName: b.externalChannelName, teamId: b.teamId, mappedAt: new Date() });
              const [row] = await db.update(messagingProvider)
                .set({ spec: { ...spec, channelMappings: channels } as any, updatedAt: new Date() })
                .where(eq(messagingProvider.id, entity.id as string))
                .returning();
              return row;
            },
          ...
          "link-user": {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const spec = entity.spec as Record<string, unknown>;
              const links = (spec.userMappings ?? []) as Array<Record<string, unknown>>;
              links.push({ externalUserId: b.externalUserId, principalId: b.principalId, linkedAt: new Date() });
              const [row] = await db.update(messagingProvider)
                .set({ spec: { ...spec, userMappings: links } as any, updatedAt: new Date() })
                .where(eq(messagingProvider.id, entity.id as string))
                .returning();
              return row;
            },
