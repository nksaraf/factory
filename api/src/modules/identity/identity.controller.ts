    ...
    new Elysia({ prefix: "/org" })
      ...
      .get(
        ...
        async (ctx) => {
          ...
          const identities = await svc.getLinkedIdentities(principal.id)
      ...
      .post(
        ...
        async ({ body, ...ctx }) => {
          ...
          const profile = await svc.updateProfileOverrides(principal.id, body)
      ...
      .post(
        ...
        async ({ params, ...ctx }) => {
          ...
          const row = await svc.revokeToolCredential(principal.id, params.id)
          if (!row) {
            ctx.set.status = 404
      ...
      .get(
        ...
        async ({ query, ...ctx }) => {
          ...
          const result = await svc.queryToolUsage(principal.id, query)
