...
export class IdentityService {
  ...
  async findPrincipalByExternalLogin(login: string, excludeProvider?: string) {
    const conditions = [sql`${identityLink.spec}->>'externalUsername' = ${login}`];
    if (excludeProvider) {
  ...
  async findPrincipalMultiSignal(opts: {
    ...
    externalUserId: string;
    provider: string;
  }): Promise<{ principal: typeof principal.$inferSelect; matchSignal: string } | null> {
    ...
    if (opts.login) {
      const found = await this.findPrincipalByExternalLogin(opts.login, opts.provider);
      if (found) return { principal: found, matchSignal: "cross-provider-login" };
    }
  ...
  async refreshPrincipalProfile(principalId: string) {
    ...
    const userOverrides =
      (found.metadata as Record<string, unknown>)?.userOverrides as
    ...
    for (const provider of [...PROFILE_MERGE_PRIORITY].reverse()) {
      ...
      const data = (link.spec as Record<string, unknown>)?.profileData as Record<string, unknown> | undefined;
      if (!data) continue;
      ...
      if (data.bio) merged.bio = data.bio as string;
