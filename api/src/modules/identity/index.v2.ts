  ...
  return new Elysia({ prefix: "/org" })
    ...
    .use(
      ontologyRoutes(db, {
        ...
        deletable: "bitemporal",
        bitemporal: { validTo: principal.validTo, systemTo: principal.systemTo },
        relations: {
          identities: {
            path: "identities",
            table: identityLink,
        ...
        actions: {
          "link-identity": {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const [link] = await db.insert(identityLink)
                .values({
                  id: newId("idlk"),
          ...
          "add-ssh-key": {
            ...
            handler: async ({ db, entity, body }) => {
              ...
              const [key] = await db.insert(sshKey)
                .values({
                  id: newId("sshk"),
                  principalId: entity.id as string,
                  type: b.type,
    ...
    .get("/sync/identities/export/:provider", async ({ params }) => {
      const provider = params.provider as import("../../adapters/identity-provider-adapter").IdentityProviderType;
      const secrets = new PostgresSecretBackend(db);
      const svc = new IdentitySyncService(db, secrets);
    ...
    .post("/sync/identities/import/:provider", async ({ params, body }) => {
      const provider = params.provider as import("../../adapters/identity-provider-adapter").IdentityProviderType;
      const { data } = body as { data: import("../../adapters/identity-provider-adapter").ExternalIdentityUser[] };
      const secrets = new PostgresSecretBackend(db);
      const svc = new IdentitySyncService(db, secrets);
