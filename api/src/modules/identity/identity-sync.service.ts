...
import { identityLink, principal, messagingProvider } from "../../db/schema/org-v2";
...
export class IdentitySyncService {
        ...
        try {
          const result = await Promise.race([
            ...
            new Promise<never>((_, reject) =>
              setTimeout(() => reject(new Error(`${provider} sync timed out after ${PROVIDER_TIMEOUT_MS}ms`)), PROVIDER_TIMEOUT_MS),
            ),
          ]);
          results.push(result);
          logger.info(
   ...
   */
  async exportUsers(providerType: IdentityProviderType): Promise<ExternalIdentityUser[]> {
   ...
   */
  async runCrossProviderSync(skipProviders?: Set<IdentityProviderType>): Promise<IdentitySyncResult[]> {
    const results: IdentitySyncResult[] = [];
  ...
  async importUsers(
    ...
    for (const extUser of externalUsers) {
      ...
      try {
        ...
        if (match) {
          ...
          await this.identityService.linkIdentity(match.principal.id, providerType, {
            externalUserId: extUser.externalUserId,
          ...
          const created_principal = await this.identityService.resolveOrCreatePrincipal({
            authUserId: `pending:${providerType}:${extUser.externalUserId}`,
            email: extUser.email,
            name: extUser.displayName ?? extUser.login ?? extUser.email.split("@")[0],
            provider: providerType,
            externalUserId: extUser.externalUserId,
            externalLogin: extUser.login ?? undefined,
            profileData: extUser.profileData,
          ...
          let existingPrincipal = extUser.login
            ? await this.identityService.findPrincipalByExternalLogin(extUser.login, providerType)
            : null;

          if (existingPrincipal) {
            await this.identityService.linkIdentity(existingPrincipal.id, providerType, {
              externalUserId: extUser.externalUserId,
              externalLogin: extUser.login ?? undefined,
              profileData: extUser.profileData,
            });
            ...
            const displayName = extUser.displayName ?? extUser.login ?? extUser.externalUserId;
            const created_principal = await this.identityService.resolveOrCreatePrincipal({
              authUserId: `pending:${providerType}:${extUser.externalUserId}`,
              name: displayName,
              provider: providerType,
        ...
        logger.error(
          { err, provider: providerType, externalUserId: extUser.externalUserId },
    ...
    return { provider: providerType, linked, created, skipped, deactivated, errors };
  }
  ...
  private async handleDepartedUser(
    extUser: { externalUserId: string; login: string | null; email: string | null },
    providerType: IdentityProviderType,
  ): Promise<void> {
    ...
    logger.info(
      { provider: providerType, externalUserId: extUser.externalUserId, principalId },
  ...
  private async runFuzzyNameMerge(): Promise<{ merged: number; skipped: number }> {
    ...
    for (const [, provMap] of nameMap) {
      ...
      const principalIds = [...new Set([...provMap.values()].map((ids) => ids[0]))];
  ...
  private async runProfileRefresh(skipProviders?: Set<IdentityProviderType>): Promise<void> {
    // In v2, syncStatus is inside spec JSONB
    const links = await this.db
      .select()
      .from(identityLink)
      .where(sql`${identityLink.spec}->>'syncStatus' = 'idle' OR ${identityLink.spec}->>'syncStatus' IS NULL`);
    ...
    if (skippedCount > 0) {
      logger.info({ skippedCount, skipProviders: [...(skipProviders ?? [])] }, "skipping profile refresh for failed providers");
    }

    if (refreshableLinks.length === 0) return;

    logger.info({ count: refreshableLinks.length }, "refreshing identity link profiles");
  ...
  private async gatherProviderConfigs(): Promise<
    Array<{ provider: IdentityProviderType; config: IdentityProviderConfig; sourceId: string }>
  > {
  ...
  private async buildRefreshConfig(
    ...
    const expiresAt = spec.expiresAt ? new Date(spec.expiresAt as string) : undefined;
    ...
    const providerConfigs = this.cachedProviderConfigs ?? await this.gatherProviderConfigs();
