  ...
  return new Elysia()
    ...
    .post("/vars", async ({ body, set }) => {
      ...
      await db.insert(configVar).values({
        ...
        target: [configVar.slug, configVar.scopeType, configVar.scopeId, configVar.environment],
        set: {
          name: name ?? slug,
          value,
          ...(spec !== undefined ? { spec } : {}),
    ...
    // --- Get ---
    .get("/vars/:slug", async ({ params, query, set }) => {
    ...
    // --- Delete ---
    .delete("/vars/:slug", async ({ params, query, set }) => {
      const err = validateScopeType(query.scopeType)
      if (err) { set.status = 400; return { success: false, error: err } }
      ...
      params: t.Object({ slug: t.String() }),
      query: ScopeQuery,
      detail: { tags: ["Config Vars"], summary: "Delete a config variable" },
    })

    // --- Resolve (merge all scopes for a context) ---
    .post("/vars/resolve", async ({ body }) => {
      ...
      body: ResolveBody,
      detail: { tags: ["Config Vars"], summary: "Resolve all config variables for a context" },
    })
}
