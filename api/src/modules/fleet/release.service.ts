...
export async function getRelease(db: Database, version: string) {
  ...
  const [bySlug] = await db.select().from(release).where(eq(release.slug, version)).limit(1);
  const row = bySlug ?? (await db.select().from(release)).find(
    (r) => (r.spec as any)?.version === version,
  );
  if (!row) return null;
  return { ...row, modulePins: (row.spec as any)?.systemPins ?? [], releaseId: row.id };
}

export async function promoteRelease(db: Database, version: string, target: string) {
  return await db.transaction(async (tx) => {
    // Try slug lookup first (indexed), fall back to full scan for spec.version match
    const [bySlug] = await tx.select().from(release).where(eq(release.slug, version)).limit(1);
    const rows = bySlug ? undefined : await tx.select().from(release);
    const row = bySlug ?? rows?.find((r) => (r.spec as any)?.version === version);
...
export async function addModulePin(db: Database, releaseId: string, moduleVersionId: string) {
  const [row] = await db.select().from(release).where(eq(release.id, releaseId)).limit(1);
...
export async function removeModulePin(db: Database, releaseId: string, moduleVersionId: string) {
  const [row] = await db.select().from(release).where(eq(release.id, releaseId)).limit(1);
  if (!row) return null;

  const newPins = ((row.spec as any)?.systemPins ?? []).filter((p: string) => p !== moduleVersionId);
  const [updated] = await db
    .update(release)
