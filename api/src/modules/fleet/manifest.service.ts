...
import { listRoutes, listDomains } from "../infra/gateway.service";
...
export async function siteCheckin(
  ...
  const [s] = await db.select().from(site).where(eq(site.name, siteName)).limit(1);
  if (!s) throw new Error(`Site not found: ${siteName}`);

  await db
    .update(site)
    .set({
      spec: { ...(s.spec as any), lastCheckinAt: new Date().toISOString() } as any,
    })
...
export async function assignReleaseToSite(
  ...
  const rel = allReleases.find(
    (r) => (r.spec as any)?.version === releaseVersion || r.slug === releaseVersion,
  );
  ...
  const previousVersion =
    existingManifests.length > 0
      ? Math.max(...existingManifests.map((m) => (m.spec as any)?.manifestVersion ?? 0))
      : 0;
