...
export function observabilityController(adapter: ObservabilityAdapter) {
  return (
    new Elysia({ prefix: "/observability" })
          ...
          return new Response(
            new ReadableStream({
              start(controller) {
                ...
                const send = (
                  entry: import("@smp/factory-shared/observability-types").LogEntry
                ) => {
                  try {
                    controller.enqueue(
                ...
                adapter
                  .streamLogs(logQuery, send, ac.signal)
                  .then(() => {
                    try {
                      controller.close()
                    } catch {
      ...
      .get(
        "/traces/find",
        ({ query }) => svc.findTrace(adapter, query as TraceFindQuery),
        {
      ...
      .get("/alerts/:id", ({ params }) => svc.getAlert(adapter, params.id), {
      ...
      .post(
        "/alerts/:id/resolve",
        ({ params, body }) => svc.resolveAlert(adapter, params.id, body.reason),
        {
          params: ObservabilityModel.alertIdParams,
      ...
      .post("/alerts/silence", ({ body }) => svc.silenceAlerts(adapter, body), {
