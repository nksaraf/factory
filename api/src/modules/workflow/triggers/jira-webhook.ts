...
function normalizeJiraEventType(webhookEvent: string, changelog?: any): string {
  switch (webhookEvent) {
    ...
    case "jira:issue_updated": {
      ...
      const hasAssignee = changelog.items.some(
        (i: any) => i.field === "assignee"
      )
...
export function jiraWebhookTrigger(db: Database) {
  return new Elysia({ prefix: "/webhooks" }).post(
    ...
    async ({ params, headers, body, set }) => {
      ...
      const jiraUser = (jiraPayload as any)?.user as
      ...
      const jiraIssueKey =
        ((jiraPayload as any)?.issue?.key as string) ?? undefined
      ...
      if (jiraAccountId) {
        jiraActorPrincipalId = await resolveActorPrincipal(
          db,
          "jira",
          jiraAccountId
        ).catch(() => null)
      ...
      const normalizedEventType = normalizeJiraEventType(
        jiraEventType,
        (jiraPayload as any)?.changelog
      )
      ...
      if (!provider) {
        wlog.warn(
          { source: "jira", providerId: params.providerId },
          "webhook provider not found"
        )
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
            status: "ignored",
      ...
      if (adapter.verifyWebhook) {
        ...
        if (!verification.valid) {
          wlog.warn(
            ...
            `jira webhook signature invalid (${jiraEventType.replace("jira:", "")}${jiraIssueKey ? ` ${jiraIssueKey}` : ""})`
          )
          if (eventId)
            await updateWebhookEventStatus(db, eventId, {
              status: "failed",
        ...
        wlog.info(
          {
            ...
            event: webhookEvent,
            issue: jiraIssueKey,
            reason: "not_a_status_transition",
          },
          `jira ${webhookEvent.replace("jira:", "")} ignored — not a status transition${jiraIssueKey ? ` (${jiraIssueKey})` : ""}`
        )
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
            status: "ignored",
      ...
      if (!statusChange) {
        wlog.info(
          ...
          `jira issue_updated ignored — no status change${jiraIssueKey ? ` (${jiraIssueKey})` : ""}`
        )
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
            status: "ignored",
      ...
      const toStatus = (
        ((statusChange as Record<string, unknown>)["toString"] as string) ?? ""
      ).toLowerCase()
        ...
        wlog.info(
          {
            ...
            event: webhookEvent,
            issue: jiraIssueKey,
            reason: `status_changed_to_${toStatus}`,
          },
          `jira status → ${toStatus} ignored — not "in progress"${jiraIssueKey ? ` (${jiraIssueKey})` : ""}`
        )
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
            status: "ignored",
      ...
      if (!repoFullName) {
        wlog.info(
          ...
          `jira ${issueKey} ignored — no repo mapping configured`
        )
        if (eventId)
          await updateWebhookEventStatus(db, eventId, {
            status: "ignored",
    ...
    },
    {
      detail: {
        tags: ["Webhooks"],
        summary: "Jira webhook — triggers workflows on issue transitions",
      },
    }
  )
}
