...
export function workflowController(db: Database) {
  return new Elysia({ prefix: "/workflow" })
    ...
    .get("/definitions", () => {
      ...
      detail: { tags: ["Workflow"], summary: "List registered workflow definitions" },
    })
    ...
    .post("/runs", async ({ body, set }) => {
      ...
      if (!parsed.success) {
        set.status = 400;
        return { success: false, error: "Invalid input", details: parsed.error.format() };
      }
    ...
    .post("/runs/:id/cancel", async ({ params, set }) => {
      ...
      if (row.status !== "running") {
        set.status = 409;
        return { success: false, error: `Cannot cancel run in status: ${row.status}` };
      }
    ...
    .post("/events", async ({ body }) => {
      ...
      detail: { tags: ["Workflow"], summary: "Emit a workflow event (for testing)" },
    })
    ...
    .get("/subscriptions", async ({ query }) => {
      ...
      if (query.workflowRunId) {
        conditions.push(eq(eventSubscription.workflowRunId, query.workflowRunId));
      }
      ...
      detail: { tags: ["Workflow"], summary: "List pending event subscriptions" },
    });
}
