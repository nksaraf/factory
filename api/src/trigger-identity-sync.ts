...
try {
  ...
  if (importIdx !== -1) {
    ...
    console.log(
      `Importing ${users.length} ${provider} users from ${filePath}...\n`
    )

    const result = await service.importUsers(provider, users)
    console.log(
      `  ${result.provider}: linked=${result.linked} created=${result.created} skipped=${result.skipped} deactivated=${result.deactivated} errors=${result.errors}`
    )

    // Skip profile refresh for the imported provider (its API is likely unreachable)
    const skipProviders = new Set([
      provider,
    ] as import("./adapters/identity-provider-adapter").IdentityProviderType[])
    console.log(
      "\nRunning cross-provider sync (name merge + profile refresh, skipping " +
        provider +
        ")..."
    )
    ...
    for (const r of crossResults) {
      console.log(
        `  ${r.provider}: linked=${r.linked} created=${r.created} skipped=${r.skipped} deactivated=${r.deactivated} errors=${r.errors}`
      )
    }
