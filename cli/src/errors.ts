...
> = {
  ...
  AUTH_DENIED: {
    ...
    suggestions: [
      {
        action: "dx factory login",
        description:
          "Sign in with email/password; session stored in ~/.config/dx/session.json",
      },
      {
        action: "dx whoami",
        description: "Verify stored session against the auth service",
      },
      {
        action: "config",
        description: "Set authUrl and authBasePath in ~/.config/dx/config.yaml",
      },
    ],
  },
  API_UNREACHABLE: {
    ...
    suggestions: [
      {
        action: "dx status",
        description: "Confirm API URL in ~/.config/dx/config.yaml",
      },
  ...
  NOT_FOUND: {
    ...
    suggestions: [
      { action: "list", description: "List resources to validate names/ids" },
    ],
  },
}
