...
import ora from "ora";
import { styleBold, styleInfo, styleSuccess, styleError, styleWarn, styleMuted } from "../cli-style.js";
...
async function checkDefaults(): Promise<DoctorResult> {
  ...
  const scan = await collectDefaults((role as "workbench" | "site" | "factory") || "workbench");
...
async function checkLocal(): Promise<DoctorResult> {
  ...
  const checks: Array<{ name: string; passed: boolean; detail?: string; fix?: string }> = [];
  ...
  if (k3dVersion.exitCode === 0) {
    ...
    checks.push({ name: "k3d installed", passed: false, fix: "brew install k3d" });
  }

  // 2. Docker running
  const dockerInfo = await capture(["docker", "info", "--format", "{{.ServerVersion}}"]);
  if (dockerInfo.exitCode === 0) {
    checks.push({ name: "Docker running", passed: true, detail: dockerInfo.stdout.trim() });
  } else {
    checks.push({ name: "Docker running", passed: false, fix: "Start Docker Desktop or the Docker daemon" });
  }
  ...
  let clusterJson: any[] = [];
  if (k3dVersion.exitCode === 0) {
    const clusters = await capture(["k3d", "cluster", "list", "--output", "json"]);
    if (clusters.exitCode === 0) {
      try { clusterJson = JSON.parse(clusters.stdout); } catch {}
  ...
  if (clusterExists) {
    ...
    checks.push({ name: "dx-local cluster exists", passed: false, fix: "dx cluster create --local" });
  }
  ...
  if (clusterExists) {
    ...
    const kubeconfigPath = path.join(os.homedir(), ".config", "dx", "kubeconfig-dx-local.yaml");
    ...
    if (apiReachable) {
      ...
      checks.push({ name: "k3d API reachable", passed: false, fix: "k3d cluster start dx-local" });
    }
    ...
    if (apiReachable) {
      const sc = await capture([
        "kubectl", "--kubeconfig", kubeconfigPath,
        "get", "sc", "local-path", "--no-headers",
      ]);
      if (sc.exitCode === 0) {
        checks.push({ name: "local-path storage class", passed: true });
      } else {
        checks.push({ name: "local-path storage class", passed: false, fix: "Recreate k3d cluster (ships with local-path by default)" });
      }
    }
    ...
    if (hasPortRange) {
      ...
      checks.push({ name: "NodePort range 30000-30200 mapped", passed: false, fix: "Recreate cluster with: k3d cluster delete dx-local && dx cluster create --local" });
    }
  }
  ...
  if (daemonRunning) {
    ...
    checks.push({ name: "Local daemon running", passed: false, fix: "Daemon starts automatically on first API call" });
  }

  // 8. Daemon health endpoint
  if (daemonRunning) {
    try {
      const res = await fetch("http://localhost:4100/health", { signal: AbortSignal.timeout(3000) });
      if (res.ok) {
        checks.push({ name: "Daemon health endpoint", passed: true });
      } else {
        checks.push({ name: "Daemon health endpoint", passed: false, detail: `HTTP ${res.status}`, fix: "Check ~/.config/dx/daemon.log" });
      }
    } catch {
      checks.push({ name: "Daemon health endpoint", passed: false, fix: "Check ~/.config/dx/daemon.log" });
    }
  }
  ...
  if (fs.existsSync(pgliteDir)) {
    ...
    checks.push({ name: "PGlite data exists", passed: false, detail: "Will be created on daemon start" });
  }

  // Print results
  for (const c of checks) {
    if (c.passed) {
      console.log(`  ${styleSuccess("✔")} ${c.name}${c.detail ? ` (${c.detail})` : ""}`);
    } else {
      console.log(`  ${styleError("✖")} ${c.name}${c.detail ? ` — ${c.detail}` : ""}`);
      if (c.fix) {
  ...
  return {
    ...
    passed: allPassed,
    details: {
      checks: checks.map((c) => ({ name: c.name, passed: c.passed, detail: c.detail, fix: c.fix })),
    },
  };
}
