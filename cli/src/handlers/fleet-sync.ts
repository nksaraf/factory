/**
 * Fleet sync — compare Factory's recorded state against live docker compose state.
 *
  ...
  for (const dep of factoryDeployments) {
    if (!liveProjects.has(dep.project)) {
      stacks.push({
        ...
        issues: [`Deployment ${dep.slug} exists in Factory but no compose project found on host`],
      });
    }
  }
...
async function syncStack(
  ...
  const deployment = await getEntity(rest, "fleet", "system-deployments", deploymentSlug);
  ...
  const composeRuntimeSlug = `${hostSlug}--${projectName}`;
  const composeRuntime = await getEntity(rest, "infra", "runtimes", composeRuntimeSlug);
  if (composeRuntime) {
    // We could check component deployments here in the future
    // For now, just verify the runtime exists
  }
...
async function listFactoryDeployments(
  ...
  try {
    const res = await rest.listEntities("fleet", "system-deployments");
