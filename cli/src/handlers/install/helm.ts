...
export async function helmInstall(opts: HelmInstallOptions): Promise<string> {
  ...
  run("kubectl", ["apply", "-f", "-", "--kubeconfig", getKubeconfig()])
  ...
  runOrThrow(
    "helm",
    [baseArgs[0], baseArgs[1], chartRef, ...baseArgs.slice(2)],
    {
...
export function getInstalledDxPlatformChartVersion(verbose?: boolean): string {
  const result = run(
    "helm",
    ["list", "-n", DX_NAMESPACE, "--kubeconfig", getKubeconfig(), "-o", "json"],
    { verbose }
  )
  if (result.status !== 0) {
  ...
  try {
    rows = JSON.parse(result.stdout || "[]") as Array<{
      name: string
      chart: string
    }>
  } catch {
  ...
  if (!rel) {
    throw new Error(
      "dx-platform Helm release not found — cannot resume past phase 4"
    )
  }
...
async function waitForPods(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (result.status === 0 && result.stdout.length > 0) {
      ...
      const allRunning =
        phases.length > 0 &&
        phases.every((p) => p === "Running" || p === "Succeeded")
      if (allRunning) {
