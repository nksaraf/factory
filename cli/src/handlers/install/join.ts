...
export async function runJoin(opts: JoinOptions): Promise<void> {
  ...
  const exitCode = runInherit(
    "sh",
    ["-c", "curl -sfL https://get.k3s.io | sh -"],
    {
      env: {
        K3S_URL: opts.server,
...
async function waitForNodeJoin(
  verbose?: boolean,
  timeoutMs = 120_000
): Promise<void> {
...
async function appendNodeToManifest(
  server: string,
  verbose?: boolean
): Promise<void> {
  ...
  spawnSync("kubectl", ["apply", "-f", "-", "--kubeconfig", getKubeconfig()], {
    input: configMapJson,
    encoding: "utf8",
  })
}
