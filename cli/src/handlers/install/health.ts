...
async function runAllChecks(
  ...
  if (opts.role === "factory") {
    ...
    checks.push(await checkFactoryPlane("ops", opts.domain, opts.verbose))
...
async function checkPods(verbose?: boolean): Promise<ServiceHealth> {
  ...
  return {
    ...
    detail: allGood
      ? `${phases.length} pod(s) running`
      : `Some pods not ready: ${phases.join(", ")}`,
  }
}
...
async function checkTls(
  ...
  return {
    name: "tls",
    status: "degraded",
    detail: "TLS not yet valid (may still be provisioning)",
  }
}
