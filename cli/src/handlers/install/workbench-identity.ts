...
import { hostname as osHostname, networkInterfaces, platform, arch } from "node:os";
import { join } from "node:path";

import type { WorkbenchConfig, WorkbenchType } from "@smp/factory-shared/install-types";
import { getMachineId } from "./machine-id.js";

const VALID_TYPES: WorkbenchType[] = ["developer", "ci", "agent", "sandbox", "build", "testbed"];

/** Generate a deterministic workbench ID from machine ID + hostname. */
export function generateWorkbenchId(machineId: string, hostname: string): string {
  const hash = createHash("sha256").update(machineId + hostname).digest("hex");
...
export function detectWorkbenchType(explicit?: string): WorkbenchType {
  ...
  if (process.env.CONDUCTOR_WORKSPACE || process.env.DEVIN_SESSION || process.env.DX_AGENT) {
    return "agent";
  }
...
export function writeWorkbenchConfig(root: string, config: WorkbenchConfig): void {
  const dir = join(root, ".dx");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "workbench.json"), JSON.stringify(config, null, 2) + "\n", {
    mode: 0o600,
  });
}
