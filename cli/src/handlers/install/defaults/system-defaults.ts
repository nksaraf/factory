...
const LIMITS_CONTENT =
  ["*    soft    nofile    65536", "*    hard    nofile    65536"].join("\n") +
  "\n"
...
async function detectLinux(): Promise<ConfigChange[]> {
  ...
  if (!isWSL) {
    ...
    const sysctlMatch =
      sysctlExists && readFileSync(SYSCTL_CONF_PATH, "utf8") === SYSCTL_CONTENT
  ...
  const limitsMatch =
    limitsExists && readFileSync(LIMITS_CONF_PATH, "utf8") === LIMITS_CONTENT
