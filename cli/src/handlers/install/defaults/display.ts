...
export function displayScan(scan: DefaultsScanResult): void {
  ...
  for (const [category, changes] of grouped) {
    ...
    if (pending.length === 0) {
      console.log(
        `  ${styleSuccess("✓")} ${label}: ${styleMuted(`${applied.length} configured`)}`
      )
      continue
    }

    const sudoNote = pending.some((c) => c.requiresSudo)
      ? styleMuted(" (requires sudo)")
      : ""
    console.log(
      `  ${styleWarn("●")} ${styleBold(label)} — ${pending.length} to apply${sudoNote}`
    )
    ...
    for (const change of changes) {
      if (change.alreadyApplied) {
        ...
        console.log(
          `    ${styleWarn("~")} ${change.description} ${styleMuted(`(was: ${change.currentValue})`)}`
        )
      } else {
...
export function displayApplyResult(result: ApplyResult): void {
  ...
  if (result.skipped.length > 0)
    parts.push(`${result.skipped.length} already set`)
  if (result.failed.length > 0) parts.push(`${result.failed.length} failed`)
  if (result.backedUp.length > 0)
    parts.push(`${result.backedUp.length} files backed up`)
...
export function displayCheckSummary(scan: DefaultsScanResult): void {
  ...
  for (const [category, changes] of grouped) {
    ...
    if (pending.length === 0) {
      console.log(
        `  ${styleSuccess("✓")} ${label}: ${changes.length}/${changes.length} configured`
      )
    } else {
      console.log(
        `  ${styleWarn("⚠")} ${label}: ${changes.length - pending.length}/${changes.length} configured (${pending.length} pending)`
      )
    }
  }
  ...
  if (scan.pending.length > 0) {
    ...
    console.log(
      styleMuted(
        `  ${scan.pending.length} changes pending. Run \`dx setup\` to apply.`
      )
    )
  }
}
