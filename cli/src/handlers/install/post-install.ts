...
export async function runPostInstall(
  ...
  if (config.role === "factory" && !config.factoryUrl) {
    console.log(
      "Factory self-bootstrap: creating self-referencing site record..."
    )
...
async function writeManifestConfigMap(
  ...
  const proc = spawnSync(
    "kubectl",
    ["apply", "-f", "-", "--kubeconfig", getKubeconfig()],
    {
      input: configMapJson,
      encoding: "utf8",
    }
...
async function registerWithFactory(
  ...
  const result = run("curl", [
    ...
    "-X",
    "POST",
    `${factoryUrl}/api/v1/factory/ops/sites/${siteName}/install-manifest`,
    "-H",
    "Content-Type: application/json",
    "-d",
  ...
  if (result.status !== 0) {
    console.warn(
      "Factory registration failed (non-fatal, will retry on next heartbeat)."
    )
    if (verbose) console.error(result.stderr)
  } else {
