/**
 * SSH into hosts, find docker compose projects, fetch compose files, parse results.
 */
    ...
    for (const line of lines) {
      try {
        ...
        containers.push({
          ...
          health: c.Health ?? "",
          ports:
            c.Ports ?? (c.Publishers ? formatPublishers(c.Publishers) : ""),
        })
      } catch {
...
function parseComposeProject(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), `dx-ops-${project.name}-`))
...
async function listComposeProjectsAsync(
  target: MachineTarget
