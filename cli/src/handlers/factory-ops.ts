import { styleBold, styleMuted, styleSuccess, styleError, styleWarn } from "../cli-style.js";
...
function formatStatus(status: string): string {
  switch (status) {
    case "succeeded": return styleSuccess(status);
    case "failed": return styleError(status);
    case "running": return styleWarn(status);
    default: return styleMuted(status);
  }
}
...
export async function runFactoryOps(
  ...
  if (name && args?.trigger) {
    try {
      ...
      if (flags.json) {
        ...
        console.log(`${styleSuccess("Triggered")} ${styleBold(name)} -> ${styleMuted(result.runId)}`);
      }
    } catch (err) {
  ...
  if (name) {
    try {
      ...
      console.log(`${styleBold(detail.name)}  ${styleMuted(`every ${formatDuration(detail.intervalMs)}`)}`);
      console.log("");
      ...
      for (const run of detail.runs) {
        const cols = [
          ...
          run.error ? styleError(run.error.slice(0, 40)) : summarize(run.summary),
        ];
  ...
  try {
    ...
    for (const op of ops) {
      ...
      const cols = [
        ...
        last ? formatStatus(last.status.padEnd(12)) : styleMuted("-".padEnd(12)),
        formatDuration(last?.durationMs ?? null).padEnd(10),
        last?.error ? styleError(last.error.slice(0, 40)) : summarize(last?.summary ?? null),
      ];
      console.log(cols.join("  "));
    }
  } catch (err) {
