...
const A = (api: Awaited<ReturnType<typeof getFactoryClient>>) => api.api.v1.factory.infra;
...
export function buildSshArgs(target: MachineTarget): string[] {
  ...
  if (target.jumpHost) {
    args.push("-J", formatJumpSpec(target.jumpHost, target.jumpUser, target.jumpPort));
  }
...
export function needsSync(composeFilePath: string): boolean {
  ...
  try {
    ...
    for (const svcVal of Object.values(doc.services as Record<string, unknown>)) {
      ...
      if (Array.isArray(svc.volumes)) {
        for (const v of svc.volumes) {
          const vol = typeof v === "string" ? v : (v && typeof v === "object" ? (v as Record<string, unknown>).source : undefined);
    ...
    for (const section of [doc.configs, doc.secrets] as unknown[]) {
      ...
      for (const entryVal of Object.values(section as Record<string, unknown>)) {
  ...
  try {
    ...
    for (const svcVal of Object.values(doc.services as Record<string, unknown>)) {
      ...
      if (svc.build) {
        ...
        const ctxRaw = typeof buildVal === "string" ? buildVal : (buildVal as Record<string, unknown>).context;
        ...
        if (typeof buildVal === "object" && (buildVal as Record<string, unknown>).dockerfile) {
          const df = resolve(composeDir, String((buildVal as Record<string, unknown>).dockerfile));
      ...
      if (Array.isArray(svc.volumes)) {
        for (const v of svc.volumes) {
          const vol = typeof v === "string" ? v.split(":")[0] : (v && typeof v === "object" ? (v as Record<string, unknown>).source : undefined);
          if (typeof vol === "string" && (vol.startsWith("./") || vol.startsWith("../"))) {
...
export function syncAndRunCompose(
  ...
  try {
    ...
    if (err && typeof err === "object" && "status" in err && (err as Record<string, unknown>).status != null) {
