/**
 * dx pkg install — cross-language smart install for monorepo packages.
 *
 * Groups discovered packages by type and runs the appropriate installer:
 *   npm   → pnpm install (at monorepo root)
...
export async function pkgInstall(
  ...
  if (opts.json) {
    ...
    for (const s of summary) {
      ...
      console.log(
        `${icon} ${s.type}: ${s.count} package(s) ${styleMuted(s.status)}`
      )
    }
  }
}
