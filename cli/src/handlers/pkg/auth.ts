...
export async function pkgAuth(root: string, opts: AuthOptions): Promise<void> {
  ...
  console.log(
    "Credentials saved to ~/.config/dx/secrets.json (local secret store)"
  )
...
async function reauthFromGlobal(root: string): Promise<void> {
  ...
  if (!saJson) {
    throw new Error(
      "Stored credentials are invalid — run dx pkg auth --key-file <path> to reconfigure"
    )
  }

  const email = extractEmail(saJson)
  console.log(
    `Re-authenticating registries using global key: ${email ?? "unknown"}\n`
  )
...
async function checkCredentials(root: string): Promise<void> {
  ...
  const universalSecret = localSecretGet(
  ...
  if (saJsonForProbe) {
    ...
    for (const r of results) {
      ...
      console.log(
        `  ${r.label.padEnd(20)} ${statusTag}${httpPart} — ${r.message}`
      )
      if (r.hint) {
