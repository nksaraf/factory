...
import { type DiagnosticReport, formatReport } from "./diagnostics.js"
...
async function checkPeerDeps(ws: MonorepoTopology): Promise<DiagnosticIssue[]> {
  const issues: DiagnosticIssue[] = []
    ...
    for (const entry of data) {
      ...
      for (const [dep, info] of Object.entries(
        ...
        if (
          ...
          typeof info === "object" &&
          "missing" in info &&
          (info as Record<string, unknown>).missing
        ) {
          issues.push({
            check: "peer-deps",
            severity: "warning",
...
async function checkLockfile(ws: MonorepoTopology): Promise<DiagnosticIssue[]> {
  const issues: DiagnosticIssue[] = []
...
async function checkSecurity(ws: MonorepoTopology): Promise<DiagnosticIssue[]> {
  const issues: DiagnosticIssue[] = []
  if (!existsSync(join(ws.root, "pnpm-workspace.yaml"))) return issues
