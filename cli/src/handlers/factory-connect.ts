...
import { styleError, styleInfo, styleMuted, styleSuccess } from "../cli-style.js";
import { dxConfigStore, readConfig, resolveFactoryUrl, resolveFactoryMode, LOCAL_FACTORY_URL } from "../config.js";
...
export async function runFactoryConnect(
  ...
  if (!targetUrl) {
    ...
    if (flags.json) {
      console.log(
        JSON.stringify(
          { success: true, data: { factoryUrl: modeInfo.url, factoryMode: modeInfo.mode, envOverride: modeInfo.envOverride, role: config.role } },
          null,
          2
        )
      );
    ...
    console.log(`Mode:         ${modeInfo.mode === "local" ? styleSuccess(modeInfo.label) : modeInfo.label}`);
  ...
  if (targetUrl === "local") {
    ...
    try {
      ...
      exitWithError(flags, `Failed to start local daemon: ${detail}`, ExitCodes.CONNECTION_FAILURE);
    }
    ...
    try {
      ...
      apiData = res.ok ? (await res.json()) as HealthBody | undefined : undefined;
    } catch { /* health check failed — daemon may still be starting */ }
    ...
    if (flags.json) {
      console.log(JSON.stringify({
        ...
        data: {
          ...
          factoryMode: "local",
          api: apiData ? { status: apiData.status, service: apiData.service } : undefined,
        },
      }, null, 2));
      return;
    }
    ...
    if (apiData?.status) {
      console.log(styleMuted(`  Health:   ${apiData.status} (${apiData.service ?? "factory-api"})`));
    }
    console.log();
    console.log(styleMuted(`  Tip: if workspace creation fails with "No cluster registered",`));
  ...
  try {
    ...
    const data = res.ok ? (await res.json()) as HealthBody | undefined : undefined;
