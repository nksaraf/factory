...
import {
  componentLabels,
  labelsToYaml,
} from "../../templates/compose-labels.js"
...
export async function runAdd(opts: AddOptions): Promise<AddResult> {
  ...
  if (opts.image) {
    ...
    if (!json) {
      ...
      console.log(
        styleMuted(
          `  Detected: ${ports.length} port(s), ${vols.length} volume(s), ${envCount} env var(s)`
        )
      )
  ...
  if (opts.from) {
    ...
    const name =
      target ||
      gitResult.composeFiles[0]?.replace("compose/", "").replace(".yml", "") ||
      "unknown"
  ...
  if (target && isResourceName(target) && !type) {
    ...
    if (existing.some((p) => p.includes(`${resourceName}.yml`))) {
      throw new Error(
        `Resource "${resourceName}" is already included in docker-compose.yaml`
      )
    }
    if (existsSync(join(projectRoot, composePath))) {
      throw new Error(`File ${composePath} already exists`)
    ...
    const files = generateResource(resourceName, {
      owner: resolvedOwner,
      projectName,
    })
  ...
  const composeFile = generateComponentCompose(
    name,
    type,
    runtime,
    resolvedOwner,
...
function readOwnerFromCompose(projectRoot: string): string | undefined {
  ...
  const files = readdirSync(composeDir).filter((f: string) =>
    f.endsWith(".yml")
  )
  for (const file of files) {
