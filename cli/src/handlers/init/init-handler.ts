...
export async function runInit(opts: InitOptions): Promise<void> {
  ...
  if (!existsSync(gitDir)) {
    ...
    if (gitInit.status === 0) {
      ...
      spawnSync("git", ["config", "core.hooksPath", ".dx/hooks"], {
        cwd: targetDir,
        stdio: "ignore",
      })
      ...
      spawnSync("git", ["commit", "-m", "feat: scaffold project"], {
        cwd: targetDir,
        stdio: "ignore",
      })
    }
  ...
  if (type === "project") {
    ...
    console.log(
      `  .dx/hooks/              Git hooks (conventional commits, lint-staged, quality checks)`
    )
