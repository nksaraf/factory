/**
 * Import discovered compose stacks into Factory entities.
 *
 * Creates entities in order: infra (host, realm) → software (system, component)
...
import type { DiscoveredStack, DiscoveryResult } from "./ops-discover.js"

// ─── Types ────────────────────────────────────────────────────

export interface ImportPlan {
  host: HostPlan
  dockerRealm: RealmPlan
  stacks: StackPlan[]
}
...
export async function buildImportPlan(
  ...
  const [hostExists, dockerExists] = await Promise.all([
    ...
    entityExists(rest, "infra", "realms", dockerSlug),
  ])
  ...
  const existsChecks = await Promise.all(
    stackMeta.map(async (m) => {
      const [sys, site, compose, dep] = await Promise.all([
        ...
        entityExists(rest, "ops", "sites", m.siteSlug),
        entityExists(rest, "infra", "realms", m.composeSlug),
        entityExists(rest, "ops", "system-deployments", m.deploymentSlug),
      ])
      return {
        systemExists: sys,
        siteExists: site,
        composeExists: compose,
  ...
  const stacks: StackPlan[] = stackMeta.map((m, i) => {
    ...
    return {
      ...
      deployment: {
        ...
        name: `${m.projectName} on ${hostSlug}`,
        type:
          inferEnvironment(m.projectName) === "development"
            ? "dev"
            : "production",
...
export async function executeImportPlan(
  ...
  // 2. Docker engine realm
  if (plan.dockerRealm.action === "create") {
    await upsert(rest, result, "infra", "realms", plan.dockerRealm.slug, {
      slug: plan.dockerRealm.slug,
      name: plan.dockerRealm.name,
      type: "docker-engine",
      hostId,
      spec: { status: "ready" },
    })
  ...
  const dockerRealmId = await resolveId(
    rest,
    "infra",
    "realms",
    plan.dockerRealm.slug
  )

  // 3. Per-stack entities
  for (const stack of plan.stacks) {
    // 3a. Compose-project realm
    if (stack.composeRealm.action === "create") {
      await upsert(rest, result, "infra", "realms", stack.composeRealm.slug, {
        slug: stack.composeRealm.slug,
        name: stack.composeRealm.name,
        type: "compose-project",
        hostId,
        parentRealmId: dockerRealmId,
        spec: { status: "ready" },
      })
    }

    const composeRealmId = await resolveId(
      rest,
      "infra",
      "realms",
      stack.composeRealm.slug
    )
    ...
    const systemId = await resolveId(
      rest,
      "product",
      "systems",
      stack.system.slug
    ...
    if (stack.site.action === "create") {
      await upsert(rest, result, "ops", "sites", stack.site.slug, {
    ...
    const siteId = await resolveId(rest, "ops", "sites", stack.site.slug)
...
export function inferSystemSlug(stack: DiscoveredStack): string {
  ...
  if (images.some((i) => i.includes("martin") || i.includes("tileserv")))
    return "tile-server"
...
function mapCatalogType(catalogType: string | undefined): string {
  ...
  if (t === "elasticsearch" || t === "meilisearch" || t === "typesense")
    return "search"
  if (t === "nginx" || t === "traefik" || t === "kong" || t === "envoy")
    return "gateway"
  return "service"
}
