...
export async function agentSkillSync(opts: {
  ...
  if (!root) {
    throw new Error("Could not find monorepo root (no pnpm-workspace.yaml or .git found).");
  }

  const skillsDir = join(root, "skills");
  if (!existsSync(skillsDir)) {
    throw new Error(`No skills/ directory found at ${skillsDir}. Create it and add SKILL.md files.`);
  }
  ...
  if (opts.json) {
    console.log(JSON.stringify({ success: true, synced: synced.length, skills: synced }));
  } else {
