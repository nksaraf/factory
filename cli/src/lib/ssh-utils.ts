 ...
 */
export function formatJumpSpec(host: string, user?: string, port?: number): string {
...
export function generateSshConfigBlocks(targets: Array<{
  slug: string;
  host: string;
  user: string;
  port?: number;
  ...
  for (const t of targets) {
    ...
    if (t.jumpHost) {
      lines.push(`  ProxyJump ${formatJumpSpec(t.jumpHost, t.jumpUser, t.jumpPort)}`);
    }
    if (t.identityFile) {
