...
export function loadDxProjectConfig(dir: string): DxProjectConfig | null {
  ...
  return {
    version:
      typeof dx.version === "string" ? dx.version : DEFAULT_CONFIG.version,
    type: isValidType(dx.type) ? dx.type : DEFAULT_CONFIG.type,
    team: typeof dx.team === "string" ? dx.team : DEFAULT_CONFIG.team,
    conventions: {
      commits:
        dx.conventions?.commits === "none"
          ? "none"
          : DEFAULT_CONFIG.conventions.commits,
      branching:
        dx.conventions?.branching === "gitflow"
          ? "gitflow"
          : DEFAULT_CONFIG.conventions.branching,
    },
    deploy: {
...
function isValidType(value: unknown): value is DxProjectType {
  return (
    typeof value === "string" &&
    ["service", "frontend", "library", "monorepo"].includes(value)
  )
}
