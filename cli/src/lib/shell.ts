...
async function loadSecretEnv(
  ...
  _secretEnvPromise = (async () => {
    ...
    try {
      const { loadLocalVars } = await import("../handlers/var-local-store.js")
    ...
    try {
      const { loadLocalSecrets } =
        await import("../handlers/secret-local-store.js")
...
export function resolveEnvSync(opts?: ShellOptions): Record<string, string> {
  const base = { ...process.env } as Record<string, string>
 ...
 */
export async function shell(cmd: string[], opts?: ShellOptions): Promise<void> {
...
export async function shellCapture(
  ...
  const env = await resolveEnv(opts)
  const result = await apply(bun$`${cmd}`.quiet().throws(false), opts?.cwd, env)
  return {
...
export async function shellCaptureOrThrow(
  ...
  if (result.exitCode !== 0) {
    ...
    throw new DxError(`${cmd[0]} ${cmd[1] ?? ""} failed: ${detail.trim()}`, {
