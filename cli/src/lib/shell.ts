...
async function loadSecretEnv(
  ...
  _secretEnvPromise = (async () => {
    ...
    try {
      ...
      // Local store unavailable — continue
    }

    // 2. Remote secrets (if connected to Factory)
    try {
      ...
      if (factoryUrl && token) {
        const res = await fetch(
          `${factoryUrl}/api/v1/factory/secrets/resolve`,
          {
            method: "POST",
            headers: {
      ...
      // Remote unavailable — continue with local secrets only
    }
