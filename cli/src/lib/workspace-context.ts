/**
 * Workspace discovery — finds and indexes all packages in a monorepo.
 *
...
import { detectPkgType } from "../handlers/pkg/detect.js";
import {
  readNpmVersion,
  readPythonVersion,
  readMavenVersion,
...
function discoverNpmPackages(root: string): WorkspacePackage[] {
...
function discoverPythonPackages(root: string): WorkspacePackage[] {
  const pkgs: WorkspacePackage[] = [];
...
function discoverJavaPackages(root: string): WorkspacePackage[] {
  const pkgs: WorkspacePackage[] = [];

  // Look for parent pom with <modules>
  const parentPomDirs = [
    join(root, "packages", "java"),
    root,
  ];
...
/**
 * Walk up from `cwd` to find the workspace root, then discover all packages.
 *
