...
command -v dx >/dev/null 2>&1 || { echo "dx not found. Install: curl -fsSL https://get.dx.rio.software | sh"; exit 1; }
...
export function verifyHooks(projectDir: string): HookVerification {
  ...
  for (const hookName of HOOK_NAMES) {
    ...
    if (!existsSync(hookPath)) {
      ...
      hooks[hookName] = hashContent(existing) === hashContent(expected) ? "ok" : "outdated";
    }
  }
...
function setHooksPath(projectDir: string, path: string): void {
  spawnSync("git", ["config", "core.hooksPath", path], { cwd: projectDir, stdio: "ignore" });
}

function getHooksPath(projectDir: string): string | null {
  const result = spawnSync("git", ["config", "--get", "core.hooksPath"], { cwd: projectDir, encoding: "utf-8" });
