 ...
 *   Workbench → "where am I working" (worktree / container / VM)
...
import { loadPackageJson, loadPackageScripts } from "./dx-project-config.js"
...
export interface ConductorLayout {
  ...
  reposDir: string
  /** Base directory for worktree checkouts (e.g., ~/conductor/workspaces). */
  worktreesDir: string
}
...
// ---------------------------------------------------------------------------
// Tier 3: Workbench — where am I working
// ---------------------------------------------------------------------------

/** The isolation kind of a workbench checkout. */
export type WorkbenchKind = "main" | "worktree" | "container" | "vm"

/** Workbench-level context — an isolation unit within a project. */
export interface WorkbenchContextData {
  /** Workbench name (e.g., "colombo", or project name for main checkout). */
  name: string
  /** Absolute path to this workbench's working directory. */
  dir: string
  /** Isolation tier. */
  kind: WorkbenchKind
  /** Current git branch. */
  ...
  mainRepoDir: string
  /** Docker compose project name (worktree name for worktrees, project name for main). */
...
export interface DxContext {
  ...
  /** Available when inside a workbench checkout. Null outside any workbench. */
  workbench: WorkbenchContextData | null
...
/**
 * Get the effective config by merging workbench-local overrides into the global config.
...
export function effectiveConfig(ctx: DxContext): DxConfig {
  if (!ctx.workbench || Object.keys(ctx.workbench.localConfig).length === 0) {
    return ctx.host.config
  }
  return { ...ctx.host.config, ...ctx.workbench.localConfig } as DxConfig
}
...
/** Context with workbench tier guaranteed non-null. */
export interface DxContextWithWorkbench extends DxContext {
  project: ProjectContextData
  workbench: WorkbenchContextData
}
...
export type ContextNeed = "host" | "project" | "workbench" | "package"
 ...
 *   workbench — resolved if cwd is inside a git repo (worktree or main checkout); exposed as `ctx.workbench`
 ...
 */
export async function resolveDxContext(opts: {
  need: "host"
  cwd?: string
}): Promise<DxContext>
export async function resolveDxContext(opts: {
  need: "project"
  cwd?: string
}): Promise<DxContextWithProject>
export async function resolveDxContext(opts: {
  need: "workbench"
  cwd?: string
}): Promise<DxContextWithWorkbench>
export async function resolveDxContext(opts: {
  need: "package"
  cwd?: string
}): Promise<DxContextWithPackage>
export async function resolveDxContext(opts: {
  need: ContextNeed
  cwd?: string
}): Promise<DxContext> {
  ...
  if (_cachedCwd === cwd && _cachedCtx) {
    ...
    // Phase 3: Workbench — detect from git plumbing
    const workbench = project ? resolveWorkbenchContextData(cwd, project) : null
    ...
    ctx = { host, project, workbench, package: pkg }
    _cachedCwd = cwd
    _cachedCtx = ctx
  }
...
async function resolveHostContext(): Promise<HostContext> {
  // Global config (no project-local merge — that belongs to the workbench tier)
  ...
  const layout: ConductorLayout = {
    reposDir: config.workbenchReposDir || `${home}/conductor/repos`,
    worktreesDir:
      config.workbenchWorktreesDir || `${home}/conductor/workspaces`,
  }
...
function resolveProjectContextData(cwd: string): ProjectContextData | null {
  ...
  try {
    ...
    // Not a monorepo or no pnpm-workspace.yaml — fine
  }
...
function resolveWorkbenchContextData(
  cwd: string,
  project: ProjectContextData
): WorkbenchContextData | null {
  ...
  try {
    ...
    if (existsSync(workbenchPath)) {
      ...
      if (
        typeof workbench.authProfile === "string" &&
        workbench.authProfile.length > 0
      ) {
...
function runtimeToType(
  runtime: DetectedToolchain["runtime"]
): PackageContextData["type"] {
...
function resolvePackageContextData(
  cwd: string,
  project: ProjectContextData
): PackageContextData | null {
