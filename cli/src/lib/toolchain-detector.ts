...
function detectTestRunner(
  ...
  if (runtime === "node") {
    const vitestConfig = firstExisting(dir, [
      "vitest.config.ts",
      "vitest.config.js",
      "vitest.config.mts",
      "vitest.config.mjs",
    ...
    const jestConfig = firstExisting(dir, [
      "jest.config.ts",
      "jest.config.js",
      "jest.config.mjs",
      "jest.config.cjs",
  ...
  if (runtime === "python") {
    const pytestConfig = firstExisting(dir, [
      "pytest.ini",
      "conftest.py",
      "setup.cfg",
    ])
...
function detectLinter(
  ...
  if (runtime === "node") {
    const eslintConfig = firstExisting(dir, [
      "eslint.config.js",
      "eslint.config.mjs",
      "eslint.config.ts",
      ".eslintrc.js",
    ...
    if (fileExists(dir, "biome.json") || fileExists(dir, "biome.jsonc"))
      return {
        ...
        runCmd: "biome check --fix",
        source: "auto-detect",
      }

    const oxlintConfig = firstExisting(dir, [
      "oxlint.config.json",
      ".oxlintrc.json",
    ])
    if (oxlintConfig || hasDependency(pkg ?? {}, "oxlint"))
      return {
        tool: "oxlint",
        configFile: oxlintConfig ?? "package.json",
        runCmd: "oxlint .",
        source: "auto-detect",
      }
  }
...
function detectMigrationTool(
  ...
  if (fileExists(dir, "prisma/schema.prisma"))
    return {
      ...
      runCmd: "prisma migrate deploy",
      source: "auto-detect",
    }

  // Drizzle
  const drizzleConfig = firstExisting(dir, [
    "drizzle.config.ts",
    "drizzle.config.js",
    "drizzle.config.json",
  ])
  ...
  if (runtime === "python" && fileExists(dir, "alembic.ini"))
    return {
      tool: "alembic",
      configFile: "alembic.ini",
      runCmd: "alembic upgrade head",
      source: "auto-detect",
    }
  ...
  if (runtime === "python" && fileExists(dir, "manage.py"))
    return {
      ...
      runCmd: "python manage.py migrate",
      source: "auto-detect",
    }
  ...
  if (runtime === "go" && fileExists(dir, "migrations"))
    return {
      tool: "golang-migrate",
      configFile: "migrations/",
      runCmd: "migrate -path migrations -database $DATABASE_URL up",
      source: "auto-detect",
    }
  ...
  if (
    runtime === "java" &&
    (fileExists(dir, "src/main/resources/db/migration") ||
      fileExists(dir, "flyway.conf"))
  )
    return {
      tool: "flyway",
      configFile: "flyway.conf",
      runCmd: "flyway migrate",
      source: "auto-detect",
    }
...
function detectCodegen(
  ...
  if (drizzleConfig)
    tools.push({
      tool: "drizzle-kit",
      configFile: drizzleConfig,
      runCmd: "drizzle-kit generate",
      source: "auto-detect",
    })

  // GraphQL codegen
  const gqlConfig = firstExisting(dir, [
    "codegen.ts",
    "codegen.yml",
    "codegen.yaml",
    "graphql.config.ts",
  ...
  if (gqlConfig)
    tools.push({
      ...
      configFile: gqlConfig,
      runCmd: "graphql-codegen",
      source: "auto-detect",
    })

  // OpenAPI TypeScript
  if (pkg && hasDependency(pkg, "openapi-typescript")) {
    const specFile = firstExisting(dir, [
      "api/openapi.yaml",
      "openapi.yaml",
      "openapi.json",
    ])
    if (specFile)
      tools.push({
        ...
        configFile: specFile,
        runCmd: `openapi-typescript ${specFile} -o src/types/api.ts`,
        source: "auto-detect",
      })
  }
  ...
  if (fileExists(dir, "sqlc.yaml") || fileExists(dir, "sqlc.yml"))
    tools.push({
      tool: "sqlc",
      configFile: "sqlc.yaml",
      runCmd: "sqlc generate",
      source: "auto-detect",
    })
...
function detectFramework(
  ...
  if (runtime === "node") {
    ...
    if (
      firstExisting(dir, ["app.config.ts"]) &&
      pkg &&
      hasDependency(pkg, "vinxi")
    )
      return "vinxi"
    if (
      firstExisting(dir, [
      ...
      return "vite"
    if (firstExisting(dir, ["astro.config.mjs", "astro.config.ts"]))
      return "astro"
  ...
  if (runtime === "go") {
    if (fileExists(dir, "air.toml") || fileExists(dir, ".air.toml"))
      return "air"
  }
