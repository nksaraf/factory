...
export type ResourceCatalogType = "database" | "cache" | "queue" | "gateway" | "storage" | "search";

export type ResourceName = "postgres" | "redis" | "temporal" | "kafka" | "rabbitmq" | "minio" | "mailpit" | "auth" | "gateway";
...
export const RESOURCE_CATALOG: ResourceEntry[] = [
  { name: "postgres",  label: "PostgreSQL", description: "Relational database",     catalogType: "database" },
  { name: "redis",     label: "Redis",      description: "In-memory cache",         catalogType: "cache" },
  { name: "temporal",  label: "Temporal",   description: "Workflow orchestration",   catalogType: "queue" },
  { name: "kafka",     label: "Kafka",      description: "Event streaming",         catalogType: "queue" },
  { name: "rabbitmq",  label: "RabbitMQ",   description: "Message broker",          catalogType: "queue" },
  { name: "minio",     label: "MinIO",      description: "S3-compatible storage",   catalogType: "storage" },
  { name: "mailpit",   label: "Mailpit",    description: "Email testing",           catalogType: "gateway" },
  { name: "auth",      label: "Auth",       description: "Authentication service (Better Auth)", catalogType: "gateway" },
  { name: "gateway",   label: "Gateway",    description: "API gateway (APISIX)",   catalogType: "gateway" },
];
...
const generators: Record<ResourceName, (opts: ResourceOpts) => GeneratedFile[]> = {
...
export function generateResource(name: ResourceName, opts: ResourceOpts): GeneratedFile[] {
