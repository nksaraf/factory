    ...
    .run(async ({ flags }) => {
      ...
      if (!ctx.package)
        return exitWithError(
          f,
          "No project found (no package.json in parent directories)."
        )
      ...
      if (!tool) {
        return exitWithError(
          f,
          'No linter detected. Add a "lint" script to package.json or install eslint/biome.'
        )
      }
