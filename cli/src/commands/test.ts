    ...
    .run(async ({ args, flags }) => {
      ...
      if (project && !args.components?.length) {
        ...
        if (pkg?.toolchain.testRunner) {
          return runToolchainTest(pkg.toolchain.testRunner, pkg.dir, pkg.scripts, variant, f);
        }
      ...
      if (!pkg) return exitWithError(f, "No project found (no package.json or docker-compose).");

      if (!pkg.toolchain.testRunner) {
        return exitWithError(f, 'No test runner detected. Add a "test" script to package.json or install vitest/jest.');
      }
...
function runToolchainTest(
  testRunner: { tool: string; configFile: string; runCmd: string; source: "auto-detect" | "package.json" },
  rootDir: string,
  ...
  if (variant) {
    ...
    if (resolved) {
      ...
      console.log(`  No ${variant} variant found for ${testRunner.tool}, running default.`);
    }
  }

  if (!f.quiet) {
    const sourceLabel = tool.source === "package.json"
  ...
  for (const name of names) {
    ...
    if (variant) {
      ...
      if (compToolchain?.testRunner) {
        ...
        const resolved = resolveVariant("test", variant, compToolchain.testRunner, packageJson);
        if (resolved) {
