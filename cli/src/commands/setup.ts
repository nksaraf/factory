...
export function setupCommand(app: DxBase) {
  return app
    ...
    .flags({
      ...
      mode: { type: "string", short: "m", description: "Factory mode: local, dev, or prod (required with --role factory)" },
      ...
      check: { type: "boolean", description: "Dry-run: show what would change without applying (exits 1 if pending, 0 if configured)" },
      "skip-defaults": { type: "boolean", description: "Skip the configure-defaults phase" },
    })
    .run(async ({ flags }) => {
      ...
      try {
        ...
        let resolvedFactoryMode = flags.mode as "local" | "dev" | "prod" | undefined;

        // Validate --mode flag
        if (role === "factory" && resolvedFactoryMode && !["local", "dev", "prod"].includes(resolvedFactoryMode)) {
          exitWithError(f, `Invalid factory mode: ${resolvedFactoryMode}. Must be one of: local, dev, prod`);
        }

        // Interactive mode picker for factory role (before any work starts)
        if (role === "factory" && !resolvedFactoryMode) {
          if (flags.yes) {
            exitWithError(f, "Factory mode is required in non-interactive mode. Use --mode local, --mode dev, or --mode prod");
          }
          const { select } = await import("@crustjs/prompts");
          resolvedFactoryMode = await select<"local" | "dev" | "prod">({
            message: "Factory mode",
            choices: [
              { value: "local", label: "Local — k3d + embedded daemon (runs from anywhere)" },
              { value: "dev", label: "Dev — k3d + docker-compose (requires factory repo)" },
              { value: "prod", label: "Production — k3s + Helm (requires factory repo)" },
            ],
          });
        }
        ...
        if (role === "workbench" || role === "factory") {
          ...
          successLine(`Workbench configured — ${result.workbenchId}`, Date.now() - totalStart);
          console.log();
        }
        ...
        if (role === "factory" && resolvedFactoryMode) {
          ...
          if (prevMode && prevMode !== resolvedFactoryMode) {
            ...
            if (prevMode === "local") {
              ...
              items.push("docker-compose stack (port 4100)", "k3d cluster dx-dev");
            }
            if (items.length > 0) {
              ...
              if (proceed) {
                ...
                if (prevMode === "local") {
                  const { stopLocalDaemon } = await import("../local-daemon/lifecycle.js");
                  await stopLocalDaemon();
                  try {
                    const { deleteK3dCluster } = await import("../handlers/cluster/k3d.js");
                  ...
                  try {
                    ...
                    composeDown(ctx.project.composeFiles, {
                      projectName: basename(ctx.project.rootDir),
                      volumes: true,
                    });
                  } catch {}
                  try {
        ...
        if (role === "factory" && resolvedFactoryMode === "local") {
            ...
            const { getK3dKubeconfig } = await import("../handlers/cluster/k3d.js");
            const { seedLocalInfra } = await import("../handlers/cluster/register.js");

            // 1. Start embedded daemon (also ensures k3d cluster internally)
            let s = phase(1, 2, "Factory daemon + cluster");
            let start = Date.now();
            await dxConfigStore.update((prev) => ({ ...prev, factoryUrl: "local", factoryMode: "local" }));
            await ensureLocalDaemon();
            phaseSucceed(s, 1, 2, "Embedded daemon + k3d cluster dx-local", start);
            ...
            if (f.json) {
              console.log(JSON.stringify({ success: true, data: { mode: "local", cluster: "dx-local" } }, null, 2));
            }
            return;
        }
        ...
        if (role === "factory" && resolvedFactoryMode === "dev") {
          ...
          try {
            ...
            exitWithError(f, "Factory --mode dev must be run from inside a factory project directory (needs docker-compose.yaml).");
            return;
          }

          const { ensureLocalCluster } = await import("../local-daemon/ensure-cluster.js");
          ...
          start = Date.now();
          const { basename } = await import("node:path");
          const { composeUp } = await import("../lib/docker.js");
          ...
          start = Date.now();
          const factoryUrl = process.env.DX_FACTORY_URL ?? "http://localhost:4100";
          let healthy = false;
          for (let i = 0; i < 45; i++) {
            try {
          ...
          if (!healthy) {
            ...
            exitWithError(f, `Factory API at ${factoryUrl} not healthy after 90s`);
            return;
          }
          ...
          await dxConfigStore.update((prev) => ({ ...prev, factoryUrl, factoryMode: "dev" }));
          ...
          phaseSucceed(s, 4, 4, "Cluster registered with compose factory", start);
          ...
          if (f.json) {
            console.log(JSON.stringify({ success: true, data: { mode: "dev", cluster: "dx-dev", factoryUrl } }, null, 2));
          }
          return;
        }
        ...
        if (role === "factory" && resolvedFactoryMode === "prod") {
          ...
          try {
            ...
            exitWithError(f, "Factory --mode prod must be run from inside a factory project directory.");
            return;
          }
        }
    ...
    .command("restore", (c) =>
      c
        .meta({ description: "Restore config files from backups made during dx setup" })
        .args([{ name: "category", type: "string", description: "Filter by filename prefix (e.g. '.npmrc', '.gitconfig') or 'all'" }])
        .run(async ({ args, flags }) => {
          const f = toDxFlags(flags);
          try {
            const { listBackups, restoreBackup, getBackupDir } = await import("../handlers/install/defaults/backup.js");
            ...
            if (backups.length === 0) {
              console.log(`  No backups found${prefix ? ` matching "${prefix}"` : ""} in ${getBackupDir()}`);
              return;
            }
            ...
            if (latestByFile.size === 0) {
              console.log(`  Found ${backups.length} backup(s) but none have recorded original paths.`);
            ...
            for (const [origPath, b] of entries) {
              console.log(`    ${origPath}  ← ${b.name}  (${b.mtime.toLocaleString()})`);
            }
            console.log();

            const { select: selectPrompt, multiselect } = await import("@crustjs/prompts");
            const action = await selectPrompt({
              message: `Restore ${entries.length} file(s) to their original locations?`,
              choices: [
                { value: "all", label: `Restore all ${entries.length} files` },
