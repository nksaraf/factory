...
const C = (api: any) => api.api.v1.factory.infra.clusters;
      ...
      c
        ...
        .run(async ({ args, flags }) => {
          ...
          if (isLocal) {
            ...
            const res = await C(api)({ id: name }).delete();
      ...
      c
        ...
        .run(async ({ args, flags }) => {
          ...
          if (isLocal) {
            ...
            const cluster = clusters.find((c: any) => c.name === name);
            if (!cluster) {
            ...
            const servers = cluster.nodes?.filter((n: any) => n.role === "server") ?? [];
            const agents = cluster.nodes?.filter((n: any) => n.role === "agent") ?? [];
          ...
          if (json) {
            ...
            console.log(`${styleBold(d.name ?? name)}`);
            console.log(`  Slug:     ${d.slug ?? ""}`);
            console.log(`  Status:   ${colorStatus(d.status ?? "")}`);
            console.log(`  Provider: ${d.providerSlug ?? d.providerId ?? ""}`);
            if (d.endpoint) console.log(`  Endpoint: ${d.endpoint}`);
            if (d.createdAt) console.log(`  Created:  ${timeAgo(d.createdAt)}`);
          }
        })
    )
          ...
          if (isLocal) {
            ...
            if (json) {
              ...
              for (const c of clusters) {
                const agents = c.nodes?.filter((n: any) => n.role === "agent")?.length ?? 0;
                const servers = c.nodes?.filter((n: any) => n.role === "server")?.length ?? 0;
