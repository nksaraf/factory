...
const C = (api: Awaited<ReturnType<typeof getApi>>) =>
  api.api.v1.factory.infra.realms
        ...
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            if (isLocal) {
              ...
              const servers =
                cluster.nodes?.filter((n) => n.role === "server") ?? []
              const agents =
                cluster.nodes?.filter((n) => n.role === "agent") ?? []
              ...
              if (json) {
                ...
                for (const node of cluster.nodes ?? []) {
                  ...
                  console.log(
                    `  ${styleMuted("node")} ${node.name}  role=${node.role}  state=${state}`
                  )
                }
              }
              return
            ...
            if (json) {
              ...
              if (d) {
                ...
                console.log(
                  `  Status:   ${colorStatus(String(d.status ?? ""))}`
                )
                console.log(`  Estate: ${d.parentEstateId ?? ""}`)
                if (spec.endpoint) console.log(`  Endpoint: ${spec.endpoint}`)
                if (d.createdAt)
                  console.log(`  Created:  ${timeAgo(String(d.createdAt))}`)
              }
            }
          })
      )
            ...
            if (isLocal) {
              ...
              if (json) {
                ...
                for (const c of clusters) {
                  const agents =
                    c.nodes?.filter((n) => n.role === "agent")?.length ?? 0
                  const servers =
                    c.nodes?.filter((n) => n.role === "server")?.length ?? 0
                  console.log(
                    `  ${styleBold(c.name)}  servers=${servers} agents=${agents}`
                  )
                }
