...
export function syncCommand(app: DxBase) {
  return app
    .sub("sync")
    .meta({ description: "Sync local state: hooks, dependencies, env, migrations" })
    .run(async ({ flags }) => {
      ...
      const results: { step: string; status: "ok" | "updated" | "warning" | "error"; detail?: string }[] = [];

      // 1. Git hooks
      const hookVerification = verifyHooks(rootDir);
      const needsHookUpdate = !hookVerification.hooksPathSet ||
      ...
      if (needsHookUpdate) {
        ...
        if (changed.length > 0) {
          results.push({ step: "Hooks", status: "updated", detail: `${changed.length} hooks ${hookResult.installed.length > 0 ? "installed" : "updated"}` });
        }
      } else {
        results.push({ step: "Hooks", status: "ok", detail: "core.hooksPath set to .dx/hooks" });
      }
      ...
      for (const [lockFile, installCmd] of Object.entries(lockFiles)) {
        ...
        if (existsSync(lockPath)) {
          ...
          if (needsInstall) {
            ...
            const result = spawnSync(bin!, args, { cwd: rootDir, stdio: quiet ? "ignore" : "inherit" });
            if (result.status === 0) {
              results.push({ step: "Dependencies", status: "updated", detail: `${installCmd} (lockfile changed)` });
            } else {
              results.push({ step: "Dependencies", status: "error", detail: `${installCmd} failed` });
            }
          } else {
      ...
      if (project.composeFiles.length > 0) {
        const pullResult = spawnSync("docker", ["compose", "pull", "--quiet", "--ignore-pull-failures"], {
          cwd: rootDir,
          stdio: "ignore",
          timeout: 30_000,
        });
      ...
      if (toolchain?.codegen && toolchain.codegen.length > 0) {
        results.push({ step: "Codegen", status: "ok", detail: `${toolchain.codegen.length} generator(s) available` });
      }

      // 5. Database — check for pending migrations
      if (toolchain?.migrationTool) {
        results.push({ step: "Database", status: "ok", detail: `${toolchain.migrationTool.tool} detected (migrations run on next dx dev)` });
      }
      ...
      if (!quiet) {
        ...
        for (const r of results) {
          const icon = r.status === "ok" ? "✓" : r.status === "updated" ? "✓" : r.status === "warning" ? "⚠" : "✗";
