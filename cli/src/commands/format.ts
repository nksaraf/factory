...
export function formatCommand(app: DxBase) {
  return app
    ...
    .run(async ({ flags }) => {
      ...
      if (!tool) {
        return exitWithError(f, 'No formatter detected. Add .prettierrc/biome.json or a "format" script to package.json.');
      }

      if (!f.quiet) {
        const sourceLabel = tool.source === "package.json" ? `Using: package.json → "format"` : `Detected: ${tool.tool} (from ${tool.configFile})`;
      ...
      if (flags.check && tool.source === "auto-detect") {
        ...
        cmd = cmd.replace("--write", "--check").replace("format .", "format --check .");
