...
export function upgradeCommand(app: DxBase) {
  return app
    ...
    .run(async ({ flags }) => {
      ...
      const staleHooks = Object.entries(hookStatus.hooks).filter(([, s]) => s !== "ok");
      if (staleHooks.length > 0) {
        for (const [name, status] of staleHooks) {
          upgrades.push({ file: `.dx/hooks/${name}`, action: status === "missing" ? "add" : "update" });
        }
      }
      ...
      if (upgrades.length === 0) {
        if (!f.quiet) console.log("  ✓ Project is up to date (dx template v" + dxConfig.version + ")");
      ...
      if (checkOnly) {
        ...
        if (f.json) {
          console.log(JSON.stringify({ command: "upgrade", upToDate: false, upgrades }));
        }
        process.exit(1);
      }
      ...
      if (staleHooks.length > 0 || !hookStatus.hooksPathSet) {
        ...
        if (!f.quiet) {
          for (const name of result.installed) console.log(`    + .dx/hooks/${name}`);
          for (const name of result.updated) console.log(`    ~ .dx/hooks/${name}`);
        }
      }
