    ...
    .command("run", (c) =>
      c
        ...
        .flags({
          ...
          platform: {
            type: "string",
            description: "Act platform image override (e.g. ubuntu-latest=catthehacker/ubuntu:act-latest)",
          },
          event: {
            type: "string",
            alias: "e",
        ...
        .run(async ({ flags }) => {
          const { detectEnvironment } = await import("../lib/ci/environment.js");
          const env = detectEnvironment();

          if (env === "github-actions") {
            console.log("Already running in GitHub Actions — workflows execute natively.");
            console.log("Use `dx ci status` to check run status from the API.");
            return;
          }
    ...
    .command("status", (c) =>
      c
        ...
        .flags({
          ...
          status: {
            type: "string",
            description: "Filter by status (pending, running, success, failure, cancelled)",
          },
          limit: {
            type: "number",
            alias: "n",
        ...
        .run(async ({ flags }) => {
          ...
          const data = await apiCall(flags, () =>
            (api as any).api.v1.factory.build.runs.get({ query })
          );

          tableOrJson(
            flags,
            data,
            ["ID", "Status", "Event", "Ref", "SHA", "Workflow", "Started", "Duration"],
            (run: Record<string, unknown>) => {
              ...
              if (run.startedAt && run.completedAt) {
                const ms = new Date(run.completedAt as string).getTime() - new Date(run.startedAt as string).getTime();
