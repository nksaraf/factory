...
export function dockerCommand(app: DxBase) {
  return app
    ...
    .run(async ({ args, flags }) => {
      const raw = args.cmd ?? [];
      const dockerArgs = Array.isArray(raw) ? raw as string[] : [raw as string];
      const slug = flags.on as string | undefined;

      if (!slug && dockerArgs.length === 0) {
        console.log(styleBold("dx docker") + " — Run Docker commands on remote machines\n");
        console.log("Usage:");
        console.log("  dx docker <command> --on <machine>     Proxy a docker command to a remote machine");
        console.log("  dx docker compose <args> --on <machine> Run docker compose on a remote machine");
        console.log("  dx docker connect <machine>            Open a shell connected to a remote Docker daemon");
        console.log("  dx docker env <machine>                Print DOCKER_HOST export commands");
        console.log("  dx docker setup <machine>              Install Docker on a remote machine");
        console.log("  dx docker add <name> --host <ip>       Register a machine locally");
        console.log("  dx docker remove <name>                Remove a locally registered machine");
        console.log("");
        console.log("Machine resolution: Factory API → ~/.ssh/config → ~/.config/dx/machines.json");
        console.log("");
      ...
      console.log(
        styleMuted(`Targeting ${styleBold(target.name)} (${target.kind}) via ${target.dockerHost}`)
      );
    ...
    .command("compose", (c) =>
      c
        ...
        .flags({
          ...
          sync: {
            type: "boolean",
            description: "Force sync of compose files to remote (auto-detected by default, use --no-sync to disable)",
          },
          file: {
            type: "string",
            short: "f",
        ...
        .run(async ({ args, flags }) => {
          ...
          const composeArgs = Array.isArray(rawCmd) ? rawCmd as string[] : [rawCmd as string];
          const slug = flags.on as string | undefined;
          ...
          if (!slug) {
            ...
            checkLocalDocker();
            try {
              execFileSync("docker", ["compose", ...composeArgs], { stdio: "inherit" });
            } catch (err: any) {
              if (err.status != null) process.exit(err.status);
              throw err;
            }
          ...
          const composeFile = (flags.file as string) ?? findComposeFile() ?? undefined;
          ...
          if (shouldSync && composeFile) {
            console.log(
              styleMuted("Build context or local volumes detected — syncing files to remote...")
            );
    ...
    .command("connect", (c) =>
      c
        .meta({ description: "Open a shell connected to a remote Docker daemon" })
        .args([
          {
            name: "machine",
            type: "string",
        ...
        .run(async ({ args }) => {
          ...
          console.log(
            styleSuccess(`Connected to ${styleBold(target.name)} (${target.kind})`)
          );
          console.log(styleMuted(`  DOCKER_HOST=${target.dockerHost}`));
          console.log(styleMuted(`  All docker commands in this shell target ${slug}.`));
    ...
    .command("env", (c) =>
      c
        .meta({ description: "Print DOCKER_HOST export commands for a machine" })
        .args([
          {
            name: "machine",
            type: "string",
        ...
        .run(async ({ args }) => {
          ...
          console.error(styleMuted(`# Disconnect: unset DOCKER_HOST DX_DOCKER_TARGET`));
        })
    )
    ...
    .command("setup", (c) =>
      c
        .meta({ description: "Install Docker and Docker Compose on a remote machine" })
        .args([
          {
            name: "machine",
            type: "string",
    ...
    .command("add", (c) =>
      c
        .meta({ description: "Register a machine in ~/.config/dx/machines.json" })
        .args([
          {
            name: "name",
            type: "string",
        ...
        .run(async ({ args, flags }) => {
          ...
          if (!name || !host) {
            console.error("Usage: dx docker add <name> --host <ip> [--user <user>] [--port <port>]");
            process.exit(1);
          }
    ...
    .command("remove", (c) =>
      c
        .meta({ description: "Remove a machine from ~/.config/dx/machines.json" })
        .args([
          {
            name: "name",
            type: "string",
        ...
        .run(async ({ args }) => {
          ...
          if (removed) {
            ...
            console.error(styleError(`Machine "${name}" not found in ~/.config/dx/machines.json.`));
            process.exit(1);
          }
        })
    );
