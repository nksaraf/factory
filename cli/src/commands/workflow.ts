...
setExamples("workflow", [
  ...
  '$ dx workflow emit workspace.ready --data \'{"workspaceId":"ws-123","status":"active"}\'  Emit event',
  ...
  return app
    ...
    .command("list", (c) =>
      c
        ...
        .run(async ({ flags }) => {
          ...
          const res = await client.request<{ data: Array<{ name: string; description: string; triggerTypes: string[] }> }>(
            "GET",
            "/api/v1/factory/workflow/definitions",
          );
    ...
    .command("start", (c) =>
      c
        ...
        .flags({
          input: { type: "string", short: "i", description: "JSON input for the workflow" },
        })
        .run(async ({ args, flags }) => {
          ...
          if (res.success) {
            console.log(styleSuccess(`Workflow started: ${res.workflowRunId}`));
          } else {
    ...
    .command("runs", (c) =>
      c
        ...
        .flags({
          status: { type: "string", short: "s", description: "Filter by status (running, succeeded, failed, cancelled)" },
          workflow: { type: "string", short: "w", description: "Filter by workflow name" },
        ...
        .run(async ({ flags }) => {
          ...
          if (flags.workflow) params.set("workflowName", flags.workflow as string);
      ...
      c
        ...
        .run(async ({ args, flags }) => {
          ...
          console.log(printKeyValue({
            ...
            "Completed": r.completedAt ? formatDate(r.completedAt) : undefined,
    ...
    .command("cancel", (c) =>
      c
        ...
        .run(async ({ args, flags }) => {
          const json = flags.json as boolean | undefined;
          const client = await getFactoryRestClient();
          const res = await client.request<{ success: boolean; error?: string }>(
            "POST",
            `/api/v1/factory/workflow/runs/${args.id}/cancel`,
          );
    ...
    .command("emit", (c) =>
      c
        .meta({ description: "Emit a workflow event (for testing / manual progression)" })
        .args([
          { name: "eventName", type: "string", required: true, description: "Event name (e.g. workspace.ready, pr.opened)" },
        ])
        .flags({
          data: { type: "string", short: "d", description: "JSON event data" },
        })
        .run(async ({ args, flags }) => {
          const json = flags.json as boolean | undefined;
          ...
          const client = await getFactoryRestClient();
          const res = await client.request<{ success: boolean; eventName: string }>(
            "POST",
            "/api/v1/factory/workflow/events",
            { eventName: args.eventName, data },
          );
    ...
    .command("subs", (c) =>
      c
        ...
        .flags({
          "run-id": { type: "string", short: "r", description: "Filter by workflow run ID" },
        })
        .run(async ({ flags }) => {
          const json = flags.json as boolean | undefined;
          const params = new URLSearchParams();
          if (flags["run-id"]) params.set("workflowRunId", flags["run-id"] as string);
...
function formatStatus(status: string): string {
  switch (status) {
    case "running": return "\x1b[34mrunning\x1b[0m";
    case "succeeded": return "\x1b[32msucceeded\x1b[0m";
