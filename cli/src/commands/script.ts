...
export function scriptCommand(app: DxBase) {
  return app
    .sub("script")
    .meta({
      description: "Run a TypeScript script with the embedded Bun runtime",
    })
    .args([
      {
        name: "file",
    ...
    .flags({
      ...
      secrets: {
        type: "boolean",
        description:
          "Inject secrets into the script environment (use --no-secrets to disable)",
      },
    })
    .run(async ({ args, flags }) => {
      console.error(
        "\x1b[2m(dx script is now dx run — dx script still works)\x1b[0m"
      )
