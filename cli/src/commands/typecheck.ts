...
export function typecheckCommand(app: DxBase) {
  return app
    .sub("typecheck")
    .meta({
      description: "Run type checker (auto-detected or from package.json)",
    })
    .run(async ({ flags }) => {
      ...
      if (!tool) {
        ...
        if (toolchain.runtime === "go" || toolchain.runtime === "rust") {
          if (!f.quiet)
            console.log("  Type checking handled by compiler — skipping.")
          process.exit(0)
        }
        return exitWithError(
          f,
          'No type checker detected. Add tsconfig.json or a "typecheck" script to package.json.'
        )
      }
