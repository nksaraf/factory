        ...
        const handle = await openTunnel(
          ...
          },
          {
            onRegistered(info) {
              if (f.json) {
                console.log(
                  JSON.stringify({ success: true, data: info }, null, 2)
                )
              } else {
                console.log(`\nTunnel active!`)
            ...
            onReconnecting(attempt, delayMs) {
              console.log(
                `Reconnecting (attempt ${attempt}, ${Math.round(delayMs / 1000)}s delay)...`
              )
            },
            onReconnected(info) {
              console.log(`Reconnected! URL: ${info.url}`)
            },
            onError(err) {
      ...
      .command("list", (c) =>
        c
          ...
          .run(async ({ flags }) => {
            ...
            if (f.json) {
              console.log(
                JSON.stringify({ success: true, data: tunnels }, null, 2)
              )
              return
            }
            if (tunnels.length === 0) {
            ...
            console.log(
              printTable(["ID", "Subdomain", "Local", "Status"], rows)
            )
          })
      )
      ...
      .command("close", (c) =>
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            actionResult(
              flags,
              { closed: true, tunnelId: id },
              styleSuccess(`Tunnel ${id} closed.`)
            )
          })
      )
  )
