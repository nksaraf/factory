...
import { getCurrentBranch, gitCommit, stageAll } from "../lib/git.js"
...
export function shipCommand(app: DxBase) {
  return app
    ...
    .flags({
      ...
      base: {
        type: "string",
        description: "PR base branch (defaults to repo default)",
      },
    ...
    .run(async ({ args, flags }) => {
      ...
      try {
        ...
        const conventions =
          dxCtx.project?.conventions ?? defaultConventionsConfig()
        ...
        try {
          ...
          if (existing) {
            ...
            const createRes = await rest.request<{
              data?: Record<string, unknown>
            }>("POST", pullsBase, {
              title: prTitle,
              body: "",
        ...
        if (f.json) {
          ...
          if (prNumber && prUrl) {
            console.log(
              `Shipped ${sha.slice(0, 8)} \u2192 PR #${prNumber}: ${prUrl}`
            )
          } else if (prNumber) {
