...
function classifyService(
  ...
  if (INFRA_NAME_PATTERNS.some((n) => lower === n || lower.startsWith(`${n}-`))) {
    return "resource";
  }
  return "component";
}
...
function extractContainerPorts(svc: Record<string, unknown>): number[] {
  const rawPorts = svc.ports as Array<string | number | Record<string, unknown>> | undefined;
  if (!rawPorts) return [];
  const result: number[] = [];
  for (const p of rawPorts) {
    if (typeof p === "number") {
      ...
      const resolved = p.replace(/\$\{[^}]+:-([^}]+)\}/g, "$1").replace(/\$\{[^}]+\}/g, "0");
...
function diagnoseService(
  ...
  check("catalog.lifecycle", "production", "Lifecycle stage (production, development, experimental, deprecated)", false);

  if (kind === "resource") {
    const inferredType = image ? inferResourceType(image) : null;
    check("catalog.type", inferredType, "Resource type (database, cache, queue, gateway, storage, search, other)", false);
  } else {
    check("catalog.type", "service", "Component type (service, worker, library)", false);
  }
  ...
  if (kind === "component") {
    check("catalog.api.provides", null, "APIs this service provides (comma-separated)", false);
    check("catalog.api.consumes", null, "APIs this service consumes (comma-separated)", false);
  }

  // Kind override — only suggest if classification might be wrong
  if (kind === "resource" && svc.build) {
    check("catalog.kind", "Component", "Classification override (Component or Resource)", false);
  }
...
function renderDiagnosis(d: ServiceDiagnosis): void {
  const kindBadge = d.kind === "component"
    ? styleInfo("Component")
    : styleWarn("Resource");
  ...
  if (missingRequired.length > 0) {
    for (const m of missingRequired) {
      ...
      console.log(`  ${styleError("✗")} ${styleBold(m.key)}${inf} — ${m.description}`);
    }
  }
  ...
  if (missingOptional.length > 0) {
    for (const m of missingOptional) {
      const inf = m.inferred ? styleMuted(` (default: ${m.inferred})`) : "";
      console.log(`  ${styleWarn("○")} ${styleMuted(m.key)}${inf} — ${m.description}`);
    }
  }
...
export async function runCatalogDoctor(flags: Record<string, unknown>, f: DxFlags): Promise<void> {
  const cwd = process.cwd();

  // Find compose file
  const composePath = flags.file
    ? String(flags.file)
    : findComposeFile(cwd);
  ...
  if (serviceFilter && serviceNames.length === 0) {
    ...
    console.error(
      styleError(`Service "${serviceFilter}" not found. Available: ${available}`),
    );
    process.exit(1);
  }
  ...
  for (const d of diagnoses) {
    ...
    for (const m of d.missing) {
      ...
      if (useDefaults) {
        if (m.inferred) {
          value = m.inferred;
          console.log(`  ${styleSuccess("+")} ${m.key}: ${value} ${styleMuted("(default)")}`);
        } else if (m.required) {
          console.log(`  ${styleWarn("⚠")} ${m.key}: ${styleMuted("skipped (no default, needs manual input)")}`);
        } else {
          console.log(`  ${styleMuted("–")} ${m.key}: ${styleMuted("skipped (optional, no default)")}`);
        }
      } else {
