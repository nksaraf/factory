    ...
    app
          ...
          .command("list", (c) =>
            c
              ...
              .flags({
                ...
                sort: {
                  type: "string",
                  description: "Sort by: name, type, status (default: name)",
                },
              })
              .run(async ({ flags }) => {
            ...
            c
              ...
              .flags({
                ...
                type: {
                  type: "string",
                  required: true,
                  description: "Host type (github, gitlab, gitea, bitbucket)",
                },
                authMode: {
                  type: "string",
              ...
              .run(async ({ flags }) => {
                ...
                const result = await apiCall(flags, () =>
                  api.api.v1.factory.build["git-host-provider"].post({
                    ...
                    apiBaseUrl:
                      (flags.apiBaseUrl as string) ?? "https://api.github.com",
                ...
                actionResult(
                  flags,
                  result,
                  styleSuccess(`Git host provider "${flags.name}" created.`)
                )
              })
          )
          ...
          .command("sync", (c) =>
            c
              ...
              .run(async ({ args, flags }) => {
                const api = await getApi()
                const result = await apiCall(flags, () =>
                  api.api.v1.factory.build["git-host-provider"]({
                    id: args.id,
                  }).sync.post()
                )
                actionResult(
                  flags,
                  result,
                  styleSuccess(`Sync started for provider ${args.id}.`)
                )
              })
          )
            ...
            c
              ...
              .run(async ({ args, flags }) => {
                ...
                actionResult(
                  flags,
                  undefined,
                  styleSuccess(`Git host provider ${args.id} deleted.`)
                )
              })
          )
          ...
          .command("update", (c) =>
            c
              ...
              .run(async ({ args, flags }) => {
                ...
                const result = await apiCall(flags, () =>
                  api.api.v1.factory.build["git-host-provider"][args.id].put(
                    body
                  )
                )
                actionResult(
                  flags,
                  result,
                  styleSuccess(`Git host provider ${args.id} updated.`)
                )
              })
          )
      )
          ...
          .command("list", (c) =>
            c
              ...
              .flags({
                ...
                sort: {
                  type: "string",
                  description: "Sort by: name, kind (default: name)",
                },
              })
              .run(async ({ flags }) => {
                const api = await getApi()
            ...
            c
              ...
              .run(async ({ flags }) => {
                ...
                actionResult(
                  flags,
                  result,
                  styleSuccess(`Repo "${name}" created.`)
                )
              })
          )
      )
      ...
      .command("clone", (c) =>
        c
          .meta({
            description:
              "Clone a repo (URL, factory slug, or interactive picker)",
          })
          .args([
            {
              name: "target",
              type: "string",
              description:
                "Git URL, factory repo slug, or omit for interactive picker",
            },
          ])
          .flags({
            dir: {
            ...
            try {
              ...
              if (target && /^(https?:\/\/|git@|ssh:\/\/)/.test(target)) {
                ...
                if (!target) {
                  ...
                  const repos = listRes.data?.data as
                    | Array<{ name: string; kind?: string; gitUrl?: string }>
                    | undefined
                  ...
                  const res = await api.api.v1.factory.build.repos[target].get()
