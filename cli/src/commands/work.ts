      ...
      .command("start", (c) =>
        c
          ...
          .flags({
            ...
            ttl: { type: "string", description: "Workbench TTL (default: 4h)" },
          })
          .run(async ({ args, flags }) => {
            ...
            if (!repo) {
              console.error(
                styleError("--repo is required (e.g. --repo org/repo)")
              )
              process.exit(1)
            }
            ...
            const input: Record<string, unknown> = {
              ...
              workbenchTtl: (flags.ttl as string) ?? "4h",
            ...
            if (res.success) {
              ...
              console.log(
                styleMuted(
                  `Track with: dx workflow status ${res.workflowRunId}`
                )
              )
            } else {
      ...
      .command("status", (c) =>
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            if (!run) {
              console.log(
                styleMuted(`No workflow run found for ${args.issueKey}`)
              )
              return
            }
      ...
      .command("cancel", (c) =>
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            if (!run) {
              console.log(
                styleMuted(`No running workflow found for ${args.issueKey}`)
              )
              return
            }
            ...
            if (cancelRes.success) {
              console.log(
                styleSuccess(
                  `Workflow ${run.workflowRunId} cancelled for ${args.issueKey}`
                )
              )
            } else {
