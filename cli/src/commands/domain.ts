      ...
      .command("add", (c) =>
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            if (f.json) {
              console.log(
                JSON.stringify({ success: true, data: resultData }, null, 2)
              )
            } else {
              console.log(styleSuccess(`Domain registered: ${fqdn}`))
              console.log(`\nTo verify ownership, add these DNS records:`)
              console.log(
                `  CNAME ${fqdn} → ${resultData?.siteId ?? "factory"}.sites.dx.dev`
              )
              console.log(
                `  TXT   _dx-verify.${fqdn} = ${resultData?.verificationToken}`
              )
      ...
      .command("verify", (c) =>
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            const domains = (listRes?.data ?? []) as Array<
              Record<string, unknown>
            >
            ...
            if (!dom) {
              exitWithError(
                f,
                `Domain ${fqdn} not found. Register it first with: dx domain add ${fqdn}`
              )
            }
            ...
            const result = await rest.request<Record<string, unknown>>(
              "POST",
              `/api/factory/infra/dns-domains/${dom.id}/verify`,
              {}
            )
      ...
      .command("list", (c) =>
        c
          ...
          .flags({
            ...
            sort: {
              type: "string",
              description: "Sort by: fqdn, kind, status (default: fqdn)",
            },
            limit: {
              type: "number",
              alias: "n",
          ...
          .run(async ({ flags }) => {
            ...
            if (f.json) {
              console.log(
                JSON.stringify({ success: true, data: domains }, null, 2)
              )
              return
            }
            if (domains.length === 0) {
        ...
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            actionResult(
              flags,
              { removed: true, fqdn },
              styleSuccess(`Domain ${fqdn} removed.`)
            )
          })
      )
  )
