...
import { buildSshArgs, buildKubectlExecArgs, clearStaleHostKey, formatJumpSpec, generateSshConfigBlocks, mergeSshConfig } from "../lib/ssh-utils.js";
...
setExamples("ssh", [
  "$ dx ssh my-workspace              SSH into a machine by name",
    ...
    .run(async ({ args, flags }) => {
      ...
      if (!target) {
        ...
        try {
          ...
          console.log(styleMuted("Searches workspaces and hosts by slug.\n"));
          console.log("  dx ssh my-workspace");
      ...
      if (!entity) {
        ...
        console.log(styleMuted("\nSearched workspaces and hosts. Try:"));
...
export async function connectToEntity(entity: ResolvedEntity, flags: Record<string, unknown>, remoteCmd: string[] = []) {
