  ...
  return app
    ...
    .run(async ({ args, flags }) => {
      ...
      if (editorPref && !["cursor", "code"].includes(editorPref)) {
        console.error(styleError(`Unsupported editor "${editorPref}". Use 'cursor' or 'code'.`));
        process.exitCode = 1;
        return;
      }
      ...
      if (!target) {
        ...
        if (!local && !remote) {
          console.error(
            styleError(`Workspace "${target}" not found.`),
          );
          console.log(
            styleMuted("Try: dx workspace list"),
          );
          process.exitCode = 1;
          return;
        }
...
async function openLocal(
  ...
  if (opts.terminal) {
    console.log(
      styleMuted(`Opening shell in ${styleBold(ws.name)} → ${dir}`),
    );
  ...
  console.log(
    styleMuted(
      `Opening ${styleBold(ws.name)} in ${editorCmd} → ${dir}`,
    ),
  );
...
async function pickWorkspace(): Promise<LocalWorkspaceInfo | ResolvedEntity> {
  ...
  for (const e of remotes) {
    choices.push({
      label: `${e.slug.padEnd(24)} ${styleMuted((e.runtimeType ?? e.type).padEnd(10))} ${styleMuted(e.status)}`,
      value: e,
    });
  }
