...
export function generateCommand(app: DxBase) {
  return app
    ...
    .meta({
      description:
        "Run detected code generators (prisma, drizzle, graphql, openapi, sqlc)",
    })
    .run(async ({ flags }) => {
      ...
      for (const gen of generators) {
        if (!f.quiet) {
          const sourceLabel =
            gen.source === "package.json"
              ? `Using: package.json → "generate"`
              : `Detected: ${gen.tool} (from ${gen.configFile})`
