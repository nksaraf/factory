...
const PREVIEW_BASE = "/api/v1/factory/fleet/previews";

async function getPreviewApi() {
  const api = await getFactoryClient();
  // Routes: /api/v1/factory/fleet/previews/...
  return api.api.v1.factory.fleet.previews;
}
    ...
    .command("deploy", (c) =>
      c
        ...
        .run(async ({ flags }) => {
          ...
          const previewData = (resultData?.preview && typeof resultData.preview === "object"
            ? resultData.preview : undefined) as Record<string, unknown> | undefined;
          if (!previewData?.slug) {
          ...
          const domain = ((resultData?.route && typeof resultData.route === "object"
            ? resultData.route : undefined) as Record<string, unknown> | undefined)?.domain as string | undefined;
