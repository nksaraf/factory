...
export function fleetCommand(app: DxBase) {
  return app
    .sub("fleet")
    .meta({ description: "Discover and manage docker compose stacks across hosts" })

    .run(() => {
      console.log(styleBold("dx fleet") + " — Discover and manage compose stacks\n");
      console.log("Commands:");
      console.log("  dx fleet discover --on <host>           Discover compose stacks");
      console.log("  dx fleet import --on <host> --dry-run   Preview import plan");
      console.log("  dx fleet import --on <host>             Import into Factory");
      console.log("  dx fleet sync --on <host>               Check for drift");
      console.log("");
      console.log("Examples:");
      console.log(styleMuted("  dx fleet discover --on lepton-59 --user lepton"));
      console.log(styleMuted("  dx fleet import --on lepton-59 --user lepton --dry-run"));
    })

    // ── dx fleet discover ──
    .command("discover", (c) =>
      c
        .meta({ description: "Discover docker compose stacks on remote hosts" })
        .flags({
    ...
    .command("import", (c) =>
      c
        ...
        .flags({
          on: { type: "string", description: "Target machine slug" },
          user: { type: "string", description: "Override SSH user" },
          "dry-run": { type: "boolean", description: "Preview without creating" },
        ...
        .run(async ({ flags }) => {
          ...
          if (dryRun) {
            console.log("");
            console.log(styleMuted("Dry run — no changes made. Remove --dry-run to import."));
            return;
          }
          ...
          if (importResult.created.length > 0) {
            console.log(styleSuccess(`Created ${importResult.created.length} entities:`));
            for (const e of importResult.created) {
              console.log(`  ${styleSuccess("+")} ${e.entity} ${styleBold(e.slug)}`);
            }
          }

          if (importResult.updated.length > 0) {
            console.log(styleWarn(`Updated ${importResult.updated.length} entities:`));
            for (const e of importResult.updated) {
              console.log(`  ${styleWarn("~")} ${e.entity} ${styleBold(e.slug)}`);
            }
          }
          ...
          if (importResult.errors.length > 0) {
            ...
            for (const e of importResult.errors) {
              console.log(`  ${styleError("✗")} ${e.entity} ${e.slug}: ${e.error}`);
            }
          }
    ...
    .command("sync", (c) =>
      c
        .meta({ description: "Check for drift between Factory and live state" })
        .flags({
          on: { type: "string", description: "Target machine slug" },
          user: { type: "string", description: "Override SSH user" },
        })
...
function formatStackRow(s: DiscoveredStack): string[] {
  ...
  const statusColored =
    status === "running" ? styleSuccess("● running")
    : status === "exited" ? styleError("● exited")
    : styleWarn(`● ${status}`);

  const unhealthyCount = s.containers.filter((c) => c.health === "unhealthy").length;
  const health = unhealthyCount > 0
    ? styleError(`⚠ ${unhealthyCount} unhealthy`)
    : styleSuccess("✓ healthy");

  return [s.project.name, String(s.containers.length), statusColored, health, styleMuted(inferSystemType(s))];
}
...
function printHealthWarnings(stacks: DiscoveredStack[]) {
  const unhealthy = stacks.filter((s) => s.containers.some((c) => c.health === "unhealthy"));
  if (unhealthy.length > 0) {
    console.log("");
    console.log(styleWarn("Health issues:"));
    for (const s of unhealthy) {
      const bad = s.containers.filter((c) => c.health === "unhealthy").map((c) => c.service || c.name);
      console.log(`  ${styleBold(s.project.name)}: ${bad.join(", ")} ${styleError("unhealthy")}`);
    }
  }
}
...
function printImportPlan(plan: ImportPlan) {
  ...
  // Host + docker runtime
  const hostAction = plan.host.action === "create" ? styleSuccess("+ create") : styleMuted("  exists");
  console.log(`${hostAction}  infra.host          ${styleBold(plan.host.slug)} (${plan.host.type}, ${plan.host.ip})`);

  const dockerAction = plan.dockerRuntime.action === "create" ? styleSuccess("+ create") : styleMuted("  exists");
  console.log(`${dockerAction}  infra.runtime       ${styleBold(plan.dockerRuntime.slug)} (docker-engine)`);
  ...
  for (const stack of plan.stacks) {
    ...
    const siteAction = stack.site.action === "create" ? styleSuccess("+") : styleMuted("=");
    console.log(`    ${siteAction} site              ${stack.site.slug} (${stack.site.type}, ${stack.site.env})`);

    const sysAction = stack.system.action === "create" ? styleSuccess("+") : styleMuted("=");
    console.log(`    ${sysAction} system            ${stack.system.slug}`);

    const rtAction = stack.composeRuntime.action === "create" ? styleSuccess("+") : styleMuted("=");
    console.log(`    ${rtAction} runtime           ${stack.composeRuntime.slug}`);

    const depAction = stack.deployment.action === "create" ? styleSuccess("+") : styleMuted("=");
    ...
    if (newComps.length > 0 || existComps.length > 0) {
      const parts = [];
      if (newComps.length > 0) parts.push(styleSuccess(`+${newComps.length} new`));
      if (existComps.length > 0) parts.push(styleMuted(`${existComps.length} exist`));
      console.log(`    ${newComps.length > 0 ? styleSuccess("+") : styleMuted("=")} components       ${parts.join(", ")}`);
    }
  ...
  const totalNew =
    ...
    (plan.dockerRuntime.action === "create" ? 1 : 0) +
    plan.stacks.reduce(
      (sum, s) =>
        sum +
        (s.site.action === "create" ? 1 : 0) +
        (s.system.action === "create" ? 1 : 0) +
        (s.composeRuntime.action === "create" ? 1 : 0) +
  ...
  const totalExist =
    ...
    (plan.dockerRuntime.action === "exists" ? 1 : 0) +
    plan.stacks.reduce(
      (sum, s) =>
        sum +
        (s.site.action === "exists" ? 1 : 0) +
        (s.system.action === "exists" ? 1 : 0) +
        (s.composeRuntime.action === "exists" ? 1 : 0) +
        (s.deployment.action === "exists" ? 1 : 0) +
        s.components.filter((c) => c.action === "exists").length,
      0,
    );

  console.log(`${styleSuccess(`${totalNew} to create`)}, ${styleMuted(`${totalExist} already exist`)}`);
}
...
function printSyncResult(result: {
  ...
  const rows = result.stacks.map((s) => {
    const statusStr =
      s.status === "in-sync" ? styleSuccess("● in-sync")
      : s.status === "drifted" ? styleError("● drifted")
      : s.status === "missing-in-factory" ? styleWarn("● not imported")
  ...
  if (drifted.length > 0) {
    console.log("");
    console.log(styleError(`${drifted.length} stacks drifted from Factory state.`));
  }
  if (missing.length > 0) {
    console.log(styleWarn(`${missing.length} stacks not yet imported. Run: dx fleet import --on ${result.host}`));
  }
...
function buildJsonOutput(result: DiscoveryResult) {
  return {
    host: result.host,
    target: { host: result.target.host, user: result.target.user, port: result.target.port },
    projects: result.stacks.map((s) => ({
      ...
      containers: s.containers.map((c) => ({
        name: c.name, image: c.image, service: c.service,
        status: c.status, health: c.health, ports: c.ports,
      })),
      components: s.catalog ? Object.keys(s.catalog.components ?? {}).length : 0,
      resources: s.catalog ? Object.keys(s.catalog.resources ?? {}).length : 0,
      error: s.error,
    })),
    summary: {
      projects: result.stacks.length,
      containers: result.stacks.reduce((sum, s) => sum + s.containers.length, 0),
      unhealthy: result.stacks.reduce((sum, s) => sum + s.containers.filter((c) => c.health === "unhealthy").length, 0),
    },
  };
}
