...
describe("toolchain-detector", () => {
  ...
  describe("runtime detection", () => {
    ...
    it("detects Python from pyproject.toml", () => {
      expect(detectToolchain(fixture("python-fullstack")).runtime).toBe(
        "python"
      )
    })
  ...
  describe("package manager detection", () => {
    it("detects pnpm from packageManager field", () => {
      expect(detectToolchain(fixture("node-fullstack")).packageManager).toBe(
        "pnpm"
      )
    })
    ...
    it("defaults to pnpm when no lockfile", () => {
      expect(detectToolchain(fixture("node-minimal")).packageManager).toBe(
        "pnpm"
      )
    })
  })
  ...
  describe("Node.js full-stack project", () => {
    ...
    it("detects tsc as type checker", () => {
      ...
      expect(t.typeChecker!.tool).toBe("tsgo")
  ...
  describe("package.json script overrides", () => {
    it("prefers test script from package.json over auto-detect", () => {
      ...
      expect(tc.testRunner!.runCmd).toBe(
        "vitest run --pool=forks --reporter=verbose"
      )
    })
  ...
  describe("framework detection", () => {
    ...
    it("detects FastAPI from pyproject.toml", () => {
      expect(detectToolchain(fixture("python-fastapi")).framework).toBe(
        "fastapi"
      )
    })

    it("detects Spring Boot from pom.xml", () => {
      expect(detectToolchain(fixture("java-spring")).framework).toBe(
        "spring-boot"
      )
    })
  })
  ...
  describe("resolveVariant", () => {
    ...
    it("prefers package.json script over variant adaptation", () => {
      ...
      const pkg = {
        scripts: { "test:watch": "vitest --ui --reporter=verbose" },
      }
