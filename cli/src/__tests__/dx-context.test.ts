...
// Mock workspace-context.ts to avoid the `import { Glob } from "bun"` issue
...
describe("dx-context", () => {
  ...
  describe("WorkspaceContext", () => {
    ...
    it("resolves as kind=main for a regular git repo", async () => {
      ...
      expect(ctx.workspace!.kind).toBe("main");
      expect(ctx.workspace!.dir).toBe(fixture.rootDir);
      expect(ctx.workspace!.mainRepoDir).toBe(fixture.rootDir);
      expect(ctx.workspace!.branch).toBe("main");
    });
    ...
    it("resolves as kind=worktree inside a git worktree", async () => {
      ...
      expect(ctx.workspace).not.toBeNull();
      expect(ctx.workspace!.kind).toBe("worktree");
      expect(ctx.workspace!.name).toBe("test-wt");
      expect(ctx.workspace!.branch).toBe("feature/test");
      // Use realpathSync for macOS /var → /private/var symlink normalization
      expect(realpathSync(ctx.workspace!.mainRepoDir)).toBe(realpathSync(fixture.rootDir));
      expect(ctx.workspace!.composeProjectName).toBe("test-wt");
    });
    ...
    it("has isolated compose project name per worktree", async () => {
      ...
      const mainCtx = await resolveDxContext({ need: "host", cwd: fixture.rootDir });
      const mainComposeProject = mainCtx.workspace!.composeProjectName;

      // Worktree uses its own name
      worktreeDir = createWorktree(fixture.rootDir, "isolated-wt", "feature/isolated");
      writeFileSync(
        join(worktreeDir, "docker-compose.yaml"),
        stringify({ services: { api: { build: { context: "./api" } } } }),
        "utf8",
      ...
      expect(wtCtx.workspace!.composeProjectName).toBe("isolated-wt");
      expect(wtCtx.workspace!.composeProjectName).not.toBe(mainComposeProject);
    });
    ...
    it("returns null when not inside a git repo", async () => {
      ...
      if (ctx.workspace) {
        expect(ctx.workspace.kind).toMatch(/^(main|worktree)$/);
      }
    });
    ...
    it("reads local config from .dx/config.json", async () => {
      ...
      const ctx = await resolveDxContext({ need: "host", cwd: fixture.rootDir });
      expect(ctx.workspace).not.toBeNull();
      expect(ctx.workspace!.localConfig.factoryUrl).toBe("http://localhost:4100");
    });
    ...
    it("reads auth profile from .dx/workbench.json", async () => {
      ...
      const ctx = await resolveDxContext({ need: "host", cwd: fixture.rootDir });
      expect(ctx.workspace).not.toBeNull();
      expect(ctx.workspace!.authProfile).toBe("staging");
    });
    ...
    it("defaults auth profile to 'default' when no workbench.json", async () => {
      ...
      const ctx = await resolveDxContext({ need: "host", cwd: fixture.rootDir });
      expect(ctx.workspace!.authProfile).toBe("default");
    });
  });
  ...
  describe("cross-tier resolution", () => {
    ...
    it("host + project + workspace resolve, package is null when no package.json", async () => {
      fixture = createProject();
