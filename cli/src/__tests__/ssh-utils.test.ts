...
import { formatJumpSpec, buildSshArgs } from "../lib/ssh-utils.js";
...
describe("formatJumpSpec", () => {
  ...
  it("prepends user@ when user is provided", () => {
    expect(formatJumpSpec("bastion.example.com", "admin")).toBe("admin@bastion.example.com");
  });

  it("appends :port when port is provided", () => {
    expect(formatJumpSpec("bastion.example.com", undefined, 2222)).toBe("bastion.example.com:2222");
  });

  it("includes both user and port", () => {
    expect(formatJumpSpec("bastion.example.com", "admin", 2222)).toBe("admin@bastion.example.com:2222");
  });

  it("omits port when port is 0 (falsy)", () => {
    expect(formatJumpSpec("bastion.example.com", "admin", 0)).toBe("admin@bastion.example.com");
  });
});
