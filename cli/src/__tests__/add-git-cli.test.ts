...
function createGitFixtureWithComposeDir(files: Record<string, string>): string {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), "dx-git-fixture-"))
...
`,
    ...
    const { status } = runDxInDir(["add", "--from", fixture], dir, home)
...
`
  ...
  it("fails when no compose files found in repo", () => {
    ...
    const { status, stderr } = runDxInDir(["add", "--from", repoDir], dir, home)
