...
import { DataTable, type Column, statusColor, timeAgo } from "../../components/data-table.js"
...
const HIDDEN_KEYS = new Set([
  "id", "spec", "metadata", "status",
  "validFrom", "validTo", "systemFrom", "systemTo",
  "changedBy", "changeReason", "updatedAt",
  "generation", "observedGeneration",
])

const PRIORITY_KEYS = ["name", "slug", "type", "version", "lifecycle", "createdAt"]
...
function deriveColumns(rows: Record<string, unknown>[]): Column[] {
  ...
  // Name or slug first
  const nameKey = allKeys.includes("name") ? "name" : allKeys.includes("slug") ? "slug" : null
  if (nameKey) {
    const slugKey = nameKey === "name" && allKeys.includes("slug") ? "slug" : undefined
    columns.push({ header: nameKey === "name" ? "Name" : "Slug", key: nameKey, slugKey })
  }
    ...
    columns.push({
      ...
      format: (v: unknown) => {
        if (typeof v === "object" && v !== null) {
          ...
          return (obj.phase as string) ?? (obj.state as string) ?? JSON.stringify(v)
        }
  ...
  for (const key of PRIORITY_KEYS) {
    ...
    columns.push({
      ...
      key,
      ...(key === "createdAt" ? { format: (v: unknown) => timeAgo(v as string), width: 14 } : {}),
    })
  }
...
function filterRows(rows: Record<string, unknown>[], filter: string): Record<string, unknown>[] {
...
export function EntityList({ entity, focused, onBack, onSelect, onCreate }: EntityListProps) {
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      <Box paddingX={1} gap={2}>
        ...
        <Text dimColor>({loading ? "loading..." : `${rows.length}${filter ? `/${allRows.length}` : ""} rows`})</Text>
        {filtering || filter ? (
          <Box>
      ...
      <DataTable
        ...
        onSelect={onSelect}
        emptyMessage={loading ? "Loading..." : filter ? `No rows matching "${filter}"` : "No records."}
      />
    </Box>
  )
}
