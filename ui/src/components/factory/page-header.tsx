...
const PAGE_GROUP_CONFIG: Record<PageGroup, { color: string; icon: string }> = {
...
export function PageHeader({
  ...
  const config = PAGE_GROUP_CONFIG[pageGroup]
  const displayIcon = icon ?? config.icon

  return (
    <div className={cn("flex items-start justify-between", className)}>
      <div className="flex items-center gap-3">
        <span className={cn(displayIcon, "text-4xl shrink-0", config.color)} />
        <div>
