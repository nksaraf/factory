...
export default function UtilizationPage() {
  ...
  const totalMemGb = (hosts ?? []).reduce(
    (sum, h) => sum + Math.round(h.memoryMb / 1024),
    0
  )
  const totalDiskGb = (hosts ?? []).reduce((sum, h) => sum + h.diskGb, 0)

  const allocatedCpu = (vms ?? []).reduce((sum, v) => sum + v.cpu, 0)
  const allocatedMemGb = (vms ?? []).reduce(
    (sum, v) => sum + Math.round(v.memoryMb / 1024),
    0
  )
  ...
  const memPct =
    totalMemGb > 0 ? Math.round((allocatedMemGb / totalMemGb) * 100) : 0
  const diskPct =
    totalDiskGb > 0 ? Math.round((allocatedDiskGb / totalDiskGb) * 100) : 0
  ...
  return (
    <div className="space-y-6 p-6">
      <PlaneHeader
        ...
        description="Aggregate resource allocation across all hosts"
      />
        ...
        <>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            <div className="space-y-2 rounded-lg border bg-card p-4">
              ...
              <HealthGauge
                value={cpuPct}
                label={`${allocatedCpu} / ${totalCpu} cores`}
              />
            </div>
            <div className="space-y-2 rounded-lg border bg-card p-4">
              ...
              <HealthGauge
                value={memPct}
                label={`${allocatedMemGb} / ${totalMemGb} GB`}
              />
            </div>
            <div className="space-y-2 rounded-lg border bg-card p-4">
              ...
              <HealthGauge
                value={diskPct}
                label={`${allocatedDiskGb} / ${totalDiskGb} GB`}
              />
            </div>
          </div>
        </>
