...
export function ProviderTypeIcon({ type, className }: { type: string; className?: string }) {
  const config = PROVIDER_ICONS[type] ?? { icon: "icon-[ph--cloud-duotone]", color: "text-zinc-400" }
  return <Icon icon={config.icon} className={cn("h-5 w-5", config.color, className)} />
}
