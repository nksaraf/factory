...
async function extractTables(): Promise<TableInfo[]> {
  ...
  if (isV2) {
    ...
    // V1 schemas
    const agent = await import("../api/src/db/schema/agent");
    const build = await import("../api/src/db/schema/build");
    const catalog = await import("../api/src/db/schema/catalog");
    const commerce = await import("../api/src/db/schema/commerce");
    const fleet = await import("../api/src/db/schema/fleet");
    const gateway = await import("../api/src/db/schema/gateway");
    const infra = await import("../api/src/db/schema/infra");
    const org = await import("../api/src/db/schema/org");
    const product = await import("../api/src/db/schema/product");
  ...
  tables.sort((a, b) => a.schema.localeCompare(b.schema) || a.tableName.localeCompare(b.tableName));
  return tables;
}

function extractFromModule(mod: Record<string, unknown>, schemaLabel: string, tables: TableInfo[]) {
