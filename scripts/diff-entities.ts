...
if (!v1Path || !v2Path) {
  console.error("Usage: bun run scripts/diff-entities.ts <v1-entities.json> <v2-entities.json>");
  process.exit(1);
}
...
const TABLE_RENAMES: Record<string, string> = {
  ...
  sandbox: "workspace",
  sandbox_snapshot: "workspace_snapshot",
  cluster: "runtime",
  provider: "substrate",
  subnet: "__removed__",
...
for (const [v1Name, v1Table] of v1Tables) {
  ...
  for (const v1Col of v1Table.columns) {
    ...
    if (["created_at", "updated_at", "deleted_at"].includes(v1Col.sqlName)) continue;
