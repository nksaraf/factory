...
if (!v1Path || !v2Path) {
  console.error("Usage: bun run scripts/diff-api-surface.ts <v1-spec.json> <v2-spec.json>");
  process.exit(1);
}
...
function loadRoutes(filePath: string): Map<string, RouteEntry> {
  ...
  if (raw.paths) {
    ...
    console.error("This looks like an entity inventory, not an API surface spec.");
    process.exit(1);
  } else {
    routes = raw.routes ?? [];
  }
...
for (const [key, v1] of v1Routes) {
  ...
  if (!v2) {
    ...
    if (JSON.stringify(v1.parameters?.sort()) !== JSON.stringify(v2.parameters?.sort())) {
...
if (missing.length === 0) {
  ...
  console.log("  These must be in MIGRATION.md 'Intentionally removed' or it's a bug:");
...
console.log(`NEW ENDPOINTS: ${added.length} (in v2 only — expected for actions, relations)`);
...
console.log();
console.log(`Summary: v1=${v1Routes.size} routes, v2=${v2Routes.size} routes, missing=${missing.length}, changed=${changed.length}, new=${added.length}`);
