...
export function loadConfig(): LegacyDxConfig {
  ...
  const factoryUrl = (
    parsed.factoryUrl || "https://factory.rio.software"
  ).replace(/\/$/, "")
...
export async function runWorkbenchSetup(opts: {
  ...
  try {
    ...
    result.context = new URL(opts.factoryUrl).hostname
    console.log(
      `  ${styleMuted("Context setup skipped — configure with `dx context set`")}`
    )
  }
  ...
  if (dockerResult.status === 0) {
    ...
    console.log(
      `  ${styleMuted("Docker not found — optional, needed for dx dev")}`
    )
  }
...
if (config.install.mode === "connected" && config.install.factoryUrl) {
  await registerWithFactory(
    config.install.factoryUrl,
    config.site.name,
    manifest,
    opts.verbose
  )
...
import { configExists, dxConfigStore, readConfig } from "../config.js"
      ...
      .run(async ({ flags }) => {
        ...
        try {
          ...
          if (hasExistingConfig && config.role) {
            ...
            // Interactive wizard
            const { runWizard } =
              await import("../handlers/install/interactive-setup.js")
          ...
          const role: InstallRole =
            (flags.role as InstallRole) || (config.role as InstallRole)
          ...
          if (role === "workbench") {
            ...
            successLine(
              `Workbench ready — ${new URL(config.factoryUrl).hostname}`,
              Date.now() - totalStart
            )
