...
async function resolveByInventoryGroup(
  group: string
...
export async function runRecipe(opts: RunRecipeOpts): Promise<void> {
  ...
  await checkAndApplyDependencies(
    recipe,
    opts.targets,
    opts.paramEnv,
    new Set()
  ...
  if (isMulti) {
    console.log(
      `Running ${styleBold(recipe.manifest.name)} on ${opts.targets.length} machine(s)...\n`
    )
  }
  ...
  for (const target of opts.targets) {
    ...
    try {
      ...
      if (isMulti) {
        console.log(
          `  ${target.name}  ${styleSuccess("✓")}  applied (${elapsed}s)`
        )
      }
    } catch (err: any) {
      const elapsed = ((Date.now() - start) / 1000).toFixed(1)
      ...
      if (msg === "__already_applied__") {
        ...
        if (isMulti) {
          console.log(
            `  ${target.name}  ${styleSuccess("✓")}  already applied (skipped)`
          )
        } else {
          console.log(
            styleMuted(
              `Recipe "${recipe.manifest.name}" is already applied on ${target.name}. Use --force to re-apply.`
            )
          )
        }
        continue
      ...
      if (isMulti) {
        console.log(
          `  ${target.name}  ${styleError("✗")}  failed (${elapsed}s): ${msg}`
        )
      } else {
        throw err
      }
...
async function runRecipeOnMachine(
  ...
  if (recipe.verifyScript) {
    ...
    if (dryRun) {
      if (verifyResult.status === 0) {
        console.log(
          styleSuccess(
            `[dry-run] ${recipe.manifest.name} is already applied on ${target.name}`
          )
        )
      } else {
        console.log(
          styleMuted(
            `[dry-run] ${recipe.manifest.name} needs to be applied on ${target.name}`
          )
        )
      }
      return
    ...
    console.log(
      styleMuted(
        `[dry-run] ${recipe.manifest.name} has no verify.sh — cannot determine state on ${target.name}`
      )
    )
    return
  }
  ...
  // Install step
  console.log(
    `Applying ${styleBold(recipe.manifest.name)} on ${target.name}...`
  )
  ...
  try {
    ...
    if (err.status != null) {
      throw new Error(
        `Recipe "${recipe.manifest.name}" failed on ${target.name} (exit code ${err.status})`
      )
    }
    throw err
  }
  ...
  console.log(
    styleSuccess(`Recipe "${recipe.manifest.name}" applied on ${target.name}`)
  )
}
    ...
    if (depRecipe.verifyScript) {
      for (const target of targets) {
        ...
        if (result.status !== 0) {
          console.log(
            styleMuted(
              `Dependency "${depName}" not satisfied on ${target.name} — applying...`
            )
          )
...
import { styleBold, styleMuted, styleSuccess } from "./list-helpers.js"
...
export function runCommand(app: DxBase) {
  return (
    app
      ...
      .meta({
        description:
          "Run scripts, recipes, and playbooks locally or on remote machines",
      })
      .args([
        {
          name: "input",
      ...
      .flags({
        on: {
          type: "string",
          description:
            "Target machine(s): slug, comma-separated, tag:<name>, or @inventory:<group>",
        },
        set: {
          type: "string",
      ...
      .run(async ({ args, flags }) => {
        ...
        if (!input) {
          ...
          console.log("Usage:")
          console.log(
            "  dx run <script.ts>                    Run TypeScript/JavaScript locally"
          )
          console.log(
            "  dx run <script.sh> --on <machine>     Run shell script on remote machine"
          )
          console.log(
            "  dx run @dx/<tool> --on <machine>      Run built-in recipe"
          )
          console.log(
            "  dx run <recipe> --on <machine>        Run custom recipe"
          )
          console.log(
            "  dx run list                           List available recipes"
          )
          ...
          console.log("Flags:")
          console.log(
            "  --on <target>     Machine(s): slug, slug1,slug2, tag:<name>, @inventory:<group>"
          )
          ...
          console.log(
            "  --watch           Re-run on file changes (scripts only)"
          )
          return
        }
        ...
        try {
          ...
          const { resolveRecipe, resolveParams } =
          ...
          switch (inputType) {
            ...
            case "script-sh": {
              ...
              for (const target of targets) {
                console.log(
                  styleMuted(`Running on ${styleBold(target.name)}...`)
                )
      ...
      .command("list", (c) =>
        c
          ...
          .run(async ({ flags }) => {
            ...
            if (f.json) {
              console.log(
                JSON.stringify({ success: true, data: recipes }, null, 2)
              )
              return
            }
        ...
        c
          ...
          .run(async ({ args, flags }) => {
            ...
            try {
              ...
              console.log(
                styleMuted(
                  `Has uninstall.sh: ${recipe.uninstallScript ? "yes" : "no"}`
                )
              )
            } catch (err) {
              exitWithError(f, err instanceof Error ? err.message : String(err))
...
export function setupCommand(app: DxBase) {
  return app
    .sub("setup")
    .meta({
      description: "Set up the dx platform or install tools on remote machines",
    })
    .args([
      {
        name: "tool",
        type: "string",
        description:
          "Tool to install (docker, node, caddy, etc.) or omit for platform setup",
      },
    ])
    .flags({
      on: {
      ...
      role: {
        type: "string",
        description: "Installation role: workbench (default), site, or factory",
      },
      bundle: {
        type: "string",
        short: "b",
    ...
    .run(async ({ args, flags }) => {
      ...
      if (!tool) {
        ...
        console.log(
          styleMuted("Running platform setup (same as dx install)...\n")
        )
        ...
        console.log(
          "For platform installation, use: " + styleBold("dx install")
        )
        ...
        console.log(
          styleMuted(
            "dx setup <tool> --on <machine> is equivalent to dx run @dx/<tool> --on <machine>"
          )
        )
        return
      }
