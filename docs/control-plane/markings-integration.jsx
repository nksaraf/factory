...
const EVAL_STEPS = [
  {
    ...
    failBehavior:
      "Resource is completely invisible. Not even listed in search results.",
    notes:
      "Already embedded in slot permissions via (... & org->access). No extra call needed — SpiceDB evaluates this as part of every has_slot_N check.",
  },
  {
    step: 2,
  ...
  AND NOT EXISTS (
    ...
    failBehavior:
      "Resource invisible. Even if principal is Owner/Admin on the parent project, markings block access. Mandatory > discretionary.",
    notes:
      "Pure PostgreSQL. No SpiceDB involvement. Binary AND-logic: must hold EVERY marking. One missing = denied. Typical check: <1ms with indexed lookups.",
  },
  {
    step: 3,
    name: "Scope Check",
    ...
    failBehavior:
      "Action denied, but resource remains visible if view scope passes.",
    notes:
      "Only checked if resource_type_scope_binding exists for this (resource_type, scope_type, action) combination. Scope slot numbering is independent from resource slot numbering.",
  },
  {
    step: 4,
    ...
    notes:
      "Uses _cascade or _local variant based on registry. Implies graph walked for fallback. Requires checked as AND-intersection. BulkCheckPermission for batching.",
  },
  {
    step: 5,
...
// If fails: casualty_details = null in response`,
    failBehavior:
      "Property value masked to null. Object still visible, but sensitive fields hidden.",
    notes:
      "Only applies to ontology objects (domain data), NOT platform resources (folders, projects). Uses clearance slots on ontology_object definition (separate from resource slots).",
  },
]
    ...
    components: [
      {
        name: "Org Membership",
        detail:
          "SpiceDB (embedded in slot permissions via org->access). Binary: member/guest or not.",
      },
      {
        name: "Marking Clearance",
        detail:
          "PostgreSQL. Resource carries markings. Principal carries clearances. Must hold ALL. AND-logic.",
      },
      {
        name: "Marking Propagation",
        detail:
          "Write-time. Container→children. Lineage: input→output. Can't launder markings through ETL.",
      },
    ],
  },
  {
    ...
    components: [
      {
        name: "Scope Check",
        detail:
          "SpiceDB scope_node slots. Which region/department/topology can you operate in?",
      },
      {
        name: "Resource Permission",
        detail:
          "SpiceDB resource slots. 8 slots × cascade/local. Implies/requires DAG.",
      },
      {
        name: "Resource Type Registry",
        detail:
          "PostgreSQL. Maps action names → slot numbers. Defines cascade and implication rules.",
      },
    ],
  },
  {
    ...
    components: [
      {
        ...
        detail:
          "4 slots on ontology_object definition. Mapped to marking requirements by Ontology Registry.",
      },
      {
        name: "Null Masking",
        detail:
          "Application layer. If principal lacks clearance for property group, value = null in response.",
      },
      {
        name: "Not on resources",
        detail:
          "Folders, projects, documents don't have property-level security. Only ontology objects (domain data).",
      },
    ],
  },
]
...
const EXAMPLES = [
  {
    ...
    scenario:
      "Samsung has a traffic_plan marked CONFIDENTIAL. Sonu has edit permission but lacks CONFIDENTIAL clearance.",
    markings: [
    ...
    steps: [
      {
        ...
        detail: "organization:samsung#access@principal:sonu → ✓",
        pass: true,
      },
      {
        check: "Marking clearance",
        detail:
          "tp_001 has marking: confidential (level=3). Sonu has clearance: internal (level=2). 2 < 3 → ✗ DENIED",
        pass: false,
      },
      {
        check: "—",
        detail: "Steps 3-5 never reached. Resource is invisible to sonu.",
        pass: null,
      },
    ],
    result:
      "✗ DENIED at marking layer. Sonu can't even see tp_001 exists, despite having slot_2 (edit) and slot_3 (simulate) grants.",
  },
  {
    id: "marking_pass",
    title: "Marking Passes, Slot Check Follows (Samsung)",
    scenario:
      "Vinay has CONFIDENTIAL clearance and edit+approve grants on tp_001.",
    markings: [
      "iam.principal_marking: vinay → confidential (level=3)",
    ...
    steps: [
      {
        check: "Org membership",
        detail: "organization:samsung#access@principal:vinay → ✓",
        pass: true,
      },
      {
        check: "Marking clearance",
        detail: "confidential(3) ≥ confidential(3) → ✓",
        pass: true,
      },
      {
        check: "Scope (region)",
        detail:
          "scope_node:region_korea#has_slot_2@principal:vinay → ✓ (manage on korea)",
        pass: true,
      },
      {
        check: "Resource slot",
        detail:
          "resource:tp_001#has_slot_4_local@principal:vinay → ✓ (approve)",
        pass: true,
      },
      {
        check: "Property filter",
        detail: "N/A — traffic_plan is a resource, not ontology_object",
        pass: true,
      },
    ],
    result:
      "✓ ALLOWED. Vinay passes mandatory (marking) + discretionary (scope + slot) checks.",
  },
  {
    id: "propagation_container",
    title: "Container Propagation (Lepton)",
    scenario:
      "folder1 is marked INTERNAL. project1 is inside folder1. Does project1 inherit the INTERNAL marking?",
    markings: [
    ...
    steps: [
      ...
      {
        check: "project1 marking",
        detail: "INTERNAL (inherited from folder1)",
        pass: null,
      },
      {
        check: "Principal without INTERNAL",
        detail: "Cannot see project1, even if they have edit slot on it",
        pass: false,
      },
      {
        check: "Remove parent marking?",
        detail:
          "If folder1's INTERNAL is removed, project1's inherited marking is also removed (cascade)",
        pass: null,
      },
    ],
    result:
      "Markings propagate down the containment hierarchy. Inherited markings cannot be removed by resource owners.",
  },
  {
    id: "propagation_lineage",
    title: "Lineage Propagation (Data Pipeline)",
    scenario:
      "Dataset A is marked PII + SUBSCRIBER_DATA. Pipeline transforms A → B. Does B inherit markings?",
    markings: [
    ...
    steps: [
      {
        check: "Dataset A markings",
        detail: "PII + SUBSCRIBER_DATA (direct)",
        pass: null,
      },
      {
        check: "Pipeline A → B",
        detail: "Output inherits ALL input markings",
        pass: null,
      },
      {
        check: "Dataset B markings",
        detail: "PII + SUBSCRIBER_DATA (inherited_lineage from A)",
        pass: null,
      },
      {
        check: "Stop propagation?",
        detail:
          "Marking admin can run stop_propagation(dataset_B, pii) with audit log + approval",
        pass: null,
      },
    ],
    result:
      "You can't launder markings through ETL. Classification travels with the data, not the container.",
  },
  {
    id: "property_level",
    title: "Property-Level Security (Ontology Object)",
    scenario:
      "Incident object has description (unclassified), casualty_details (FORENSIC clearance), responding_officers (OFFICER_PII clearance).",
    markings: [
    ...
    steps: [
      {
        check: "arjun views inc_001",
        detail:
          "Object visible (passes base view). casualty_details = null. responding_officers = null.",
        pass: null,
      },
      {
        check: "forensic_analyst views",
        detail: "casualty_details = visible ✓. responding_officers = null ✗.",
        pass: null,
      },
      {
        check: "hr_admin views",
        detail: "casualty_details = null ✗. responding_officers = visible ✓.",
        pass: null,
      },
      {
        check: "Both clearances",
        detail: "A principal with BOTH sees all properties.",
        pass: true,
      },
    ],
    result:
      "Same object, different views per principal. SpiceDB resolves via view_clearance_N. Application null-masks denied properties.",
  },
  {
    id: "multi_marking",
    title: "Multiple Markings (AND Logic)",
    scenario:
      "A resource has THREE markings: CONFIDENTIAL + PII + EU_GDPR. Principal must hold ALL three.",
    markings: [
    ...
    steps: [
      {
        check: "Principal A",
        detail: "Has 2/3 markings. Missing eu_gdpr. → ✗ DENIED (must hold ALL)",
        pass: false,
      },
      {
        check: "Principal B",
        detail: "Has 3/3 markings. → ✓ PASSED",
        pass: true,
      },
      {
        check: "SQL check",
        detail:
          "NOT EXISTS (marking on resource WHERE principal doesn't hold it)",
        pass: null,
      },
    ],
    result:
      "AND-logic. One missing marking = invisible. No partial access. No OR-exceptions (except CBAC for government use cases).",
  },
]
...
const WHY_NOT_SPICEDB = [
  {
    ...
    detail:
      'SpiceDB checks "does principal have relation R to object O?" (existential). Markings require "for EVERY marking M on resource R, does principal hold M?" (universal). SpiceDB has no native universal quantifier. You\'d need N separate checks where N = number of markings on the resource.',
  },
  {
    problem: "Set membership, not graph traversal",
    detail:
      "Marking checks are simple set membership (does principal hold marking X?). No hierarchy walking, no cascading, no relationship traversal. PostgreSQL handles this in microseconds with indexed lookups. SpiceDB's graph engine is overkill.",
  },
  {
    problem: "Hierarchical markings need comparison",
    detail:
      "For hierarchical marking types (PUBLIC < INTERNAL < CONFIDENTIAL), the check is numeric: principal.level ≥ resource.level. SpiceDB has no numeric comparison. PostgreSQL does this trivially.",
  },
  {
    problem: "Dynamic marking count per resource",
    detail:
      "A resource can have 0 to N markings. If modeled as SpiceDB relations, you'd need dynamic-length relation lists. The slot pattern (clearance_1 through clearance_4) works for property-level gating (bounded, known count per type). But object-level markings are unbounded.",
  },
  {
    problem: "Write-time propagation",
    detail:
      "Marking propagation (container → children, lineage: input → output) is a WRITE-TIME concern. When a resource is created inside a marked folder, the inherited marking is written immediately. This is a PostgreSQL trigger or application-level hook, not a SpiceDB graph traversal at read time.",
  },
]
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        >
          PLATFORM FABRIC — MANDATORY ACCESS CONTROL
        </div>
        ...
        <div style={{ fontSize: "11px", color: "#52525b", marginTop: "3px" }}>
          Mandatory before discretionary · Markings in PostgreSQL · Clearance
          slots in SpiceDB · 5-step evaluation
        </div>
      </div>
              ...
              >
                <span
                  style={{
                    ...
                    fontFamily: mono,
                    backgroundColor:
                      step.type === "mandatory" ? "#ef444415" : "#3b82f615",
              ...
              <div style={{ padding: "10px 14px" }}>
                ...
                {layer.components.map((c, j) => (
                  <div
                    ...
                    style={{
                      padding: "6px 10px",
                      marginBottom: j < layer.components.length - 1 ? "4px" : 0,
                      borderRadius: "4px",
...
No role, no cascade, no implies chain can override a mandatory denial.`}
      ...
      {tab === "markings" && (
        <div>
          ...
          >
            Markings live in PostgreSQL, NOT SpiceDB. They're checked before any
            SpiceDB call. A marking is a binary access requirement — you either
            hold it or you don't. Multiple markings on a resource use AND-logic:
      ...
      {tab === "ontology" && (
        <div>
          ...
          >
            <strong>
              Property-level security only applies to ontology objects
            </strong>{" "}
            (domain data), NOT platform resources (folders, projects).
            Object-level markings (PostgreSQL) gate the whole object. Clearance
            slots (SpiceDB) gate individual properties. These are two different
            mechanisms.
          </div>
          ...
          <div style={{ marginTop: "12px" }}>
            ...
            >
              ...
              >
                <span>DIMENSION</span>
                <span>RESOURCE (folder, project)</span>
                <span>ONTOLOGY OBJECT (road segment, incident)</span>
              </div>
              {[
                [
                  "Object-level marking",
                  "✓ PostgreSQL (AND-logic)",
                  "✓ PostgreSQL (AND-logic)",
                ],
                [
                  "Property-level gating",
                  "✗ Not supported",
                  "✓ SpiceDB clearance slots (4 slots)",
                ],
                [
                  "Permission slots",
                  "8 slots (cascade/local)",
                  "3 fixed: owner/editor/viewer",
                ],
                [
                  "Parent cascade",
                  "✓ via resource#parent",
                  "✗ (inherits from dataset→project)",
                ],
                [
                  "Scope binding",
                  "✓ via resource_type_scope_binding",
                  "✓ via scope relation",
                ],
                [
                  "Containment",
                  "resource → resource (folder→project)",
                  "ontology_object → dataset → resource",
                ],
                [
                  "User-defined actions",
                  "✓ via registry (8 slots)",
                  "✗ fixed: view/edit/admin",
                ],
                [
                  "Lineage propagation",
                  "✗ (not a data pipeline output)",
                  "✓ markings propagate through ETL",
                ],
              ].map(([dim, res, onto], i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <span
                    style={{
                      color: res.startsWith("✓") ? "#86efac" : "#71717a",
                    }}
                  >
                    {res}
                  </span>
                  <span
                    style={{
                      color: onto.startsWith("✓") ? "#86efac" : "#71717a",
                    }}
                  >
                    {onto}
                  </span>
          ...
          {currentExample && (
            ...
            >
              ...
              >
                ...
                <Code maxHeight="180px">
                  {currentExample.markings.join("\n")}
                </Code>
              </div>
              ...
              {currentExample.steps.map((step, i) => (
                <div
                  ...
                  style={{
                    padding: "5px 14px",
                    borderBottom:
                      i < currentExample.steps.length - 1
                        ? "1px solid #111114"
                        : "none",
                    display: "flex",
                    gap: "10px",
              ...
              <div
                style={{
                  padding: "8px 14px",
                  backgroundColor: currentExample.result.startsWith("✓")
                    ? "#0a120a"
                    : currentExample.result.startsWith("✗")
                      ? "#120a0a"
                      : "#0e0c08",
                  ...
                  fontWeight: 700,
                  lineHeight: "1.6",
                  color: currentExample.result.startsWith("✓")
                    ? "#86efac"
                    : currentExample.result.startsWith("✗")
                      ? "#fca5a5"
                      : "#fde68a",
      ...
      {tab === "why" && (
        <div>
          ...
          >
            ...
            <div
              style={{ fontSize: "11px", color: "#86efac", lineHeight: "1.7" }}
            >
              Property-level clearances on ontology objects DO use SpiceDB
              (clearance_1_holder through clearance_4_holder). This works
              because: (a) the slot count is bounded per object type (max 4),
              (b) the check composes with base view permission via AND
              (view_clearance_N = view & clearance_N_holder), and (c) SpiceDB's
              graph engine adds value here because clearance can be granted via
              roles (role#member). Object-level markings = PostgreSQL.
              Property-level clearances = SpiceDB. Different problems, different
              tools.
            </div>
          </div>
        </div>
      ...
      {tab === "code" && (
        <div>
          ...
          >
            The full authorize() function showing the 5-step chain. Note how
            marking check (PostgreSQL) runs BEFORE any SpiceDB call. If markings
            fail, we never hit SpiceDB — saving the network round-trip.
          </div>
